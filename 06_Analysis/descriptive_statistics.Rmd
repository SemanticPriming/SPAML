...
for (i in 1:length(p.files)){
  p.files[[i]]$language <- as.character(p.files[[i]]$language)
}
...
Please see XX LINK XX for a summarized list of all native languages.
...
Please see XX LINK XX for a summarized list of all native languages for participants used in analysis.
...
Please see XX LINK XX for a summarized list of all native languages for rescored accuracy in native language.
...
Please see summary report located at: <https://osf.io/mh9ny> which includes gender, education, age, and sample size counts for a summary. This table also includes information about the ethics submissions and data collection procedures for each lab.

Native Language by Lab:

Operating System by Lab:

Web Browser by Lab:

Language Locale by Lab:

```{r separate-by-lab}
p.df %>% 
  group_by(language, native_language_translated) %>% 
  summarize(frequency = n(), .groups = "keep") %>% 
  mutate(language = paste0("lab_", language)) %>% 
  pivot_wider(data = ., 
              names_from = language, 
...
Please see summary report located at: <https://osf.io/ztkfg>. This summary includes gender, education, and age information by language.

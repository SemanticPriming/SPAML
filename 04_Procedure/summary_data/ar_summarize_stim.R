...
# original word lists
ar_words <- import("/var/www/html/ar/ar_words.csv")

# collected data
ar_data_all <-
  bind_rows(processData("/var/www/html/ar/data/data.sqlite") %>%
...
list_ar_data <- lapply(list.files(path = "/var/www/html/summary_data",
...
write.csv(ar_merged, "/var/www/html/summary_data/ar_summary.csv", row.names = F)
...
write.csv(p_lab, "/var/www/html/summary_data/ar_participants.csv", row.names = F)

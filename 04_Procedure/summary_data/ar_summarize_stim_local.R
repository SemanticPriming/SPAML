...
# original word lists
ar_words <- import("./04_Procedure/ar/ar_words.csv")

# collected data
ar_data_all <-
  bind_rows(processData("./04_Procedure/ar/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab))) %>% unique()

# delete stuff before we started
# ar_data_all <- ar_data_all %>%
#   filter(timestamp > as.POSIXct("2022-12-04"))
...
# Clean Up ----------------------------------------------------------------
  ...
  number_trials <- ar_data_all %>% #data frame
...
# grab only real trials ----
  real_trials <- ar_data_all %>% #data frame
    filter(sender == "Stimulus Real") %>%  #filter out only the real stimuli
    select(observation, fix_sender, response, response_action, ended_on, duration,
           colnames(ar_data_all)[grep("^time", colnames(ar_data_all))],
...
# figure out trial type ----
  ...
  ar_real_wide <- ar_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                    "class.x", "correct.x", "trial_code.x",
                                    "duration.y", "word.y", "class.y", "correct.y",
                                    "Z_RT.y", "keep.y", "keep_participant.y",
                                    "ended_on.x", "ended_on.y")]
  # good names
  colnames(ar_real_wide) <- c("unique_trial", "observation", "cue_word",
  ...
  ar_focus <- subset(ar_real_wide, target_type == "word" & cue_type == "word")
  ar_focus$word_combo <- paste0(ar_focus$cue_word, ar_focus$target_word)

  # add if it's related or unrelated
  ar_words$word_combo <- paste0(ar_words$ar_cue, ar_words$ar_target)
  ar_focus <- merge(ar_focus, ar_words[ , c("type", "word_combo")],
                    by = "word_combo", all.x = T)

  ### HERE YOU WILL TURN ON ###
  # subset out NAs at some point they will be practice trials
  ar_focus <- subset(ar_focus, !is.na(type))

  # calculate the total N versus timeout N
  ar_num_trials <- ar_focus %>%
    group_by(word_combo) %>%
  ...
  ### HERE YOU WILL TURN ON ###
  ar_focus <- subset(ar_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
  ar_Z <- subset(ar_focus, target_correct == TRUE)
  ar_Z <- subset(ar_Z, keep_trial == "keep")
...
# merge with complete stimuli list ----
  ar_merged <- merge(ar_words, ar_Z_summary,
                     by = "word_combo", all.x = T)

  ar_merged <- merge(ar_merged, ar_num_trials,
                     by = "word_combo", all.x = T)

# merge with old data ----
  # pull in other information from previous weeks
  list_ar_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "ar_summary_[0-9].*.csv", full.names = T),
                         import)
  ar_summaries <- bind_rows(list_ar_data, ar_merged)
  ar_merged <- ar_summaries %>%
    select(-any_of(c("done_both", "done_totalN", "done"))) %>%
    group_by(word_combo, ar_cue, ar_target, type, cue_type,
             target_type, ar_cosine) %>%
  ...
  # are we done? ----
  ar_merged$done_both <- (ar_merged$target_answeredN >= 50 & ar_merged$SE_Z <= .09) | ar_merged$target_answeredN >= 320
...
# generate summary chart for shiny ----
  write.csv(ar_merged, "./04_Procedure/summary_data/ar_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  p_lab <- ar_data_all[ar_data_all$observation %in% p_end, ]
  ...
  # pull in other information from previous weeks
  list_ar_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "ar_participants_[0-9].*.csv", full.names = T),
                         import)
  ...
  if (nrow(p_lab) > 0){
    ...
    p_lab <- unique(bind_rows(list_ar_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/ar_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(ar_use[ar_use$cue_type == "nonword" &
                    ar_use$target_type == "nonword", ]) >= 150){

      temp <- subset(ar_use,
                     ar_use$cue_type == "nonword" &
                       ar_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- ar_use[ar_use$cue_type == "nonword" &
                           ar_use$target_type == "nonword", ]
      temp <- subset(ar_sample,
                     ar_use$cue_type == "nonword" &
                       ar_use$target_type == "nonword")
    ...
    if (nrow(ar_use[(ar_use$cue_type == "nonword" &
                     ar_use$target_type == "word") |
                    (ar_use$cue_type == "word" &
                     ar_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- ar_use[(ar_use$cue_type == "nonword" &
                                ar_use$target_type == "word") |
                               (ar_use$cue_type == "word" &
                                  ar_use$target_type == "nonword"), ]
      temp <- subset(ar_sample,
    ...
    if (nrow(ar_use[ar_use$type == "related" , ]) >= 75){

      temp <- subset(ar_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- ar_use[ar_use$type == "related", ]
      temp <- subset(ar_sample, type == "related")
    ...
    if (nrow(ar_use[ar_use$type == "unrelated" , ]) >= 75){

      temp <- subset(ar_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- ar_use[ar_use$type == "unrelated", ]
      temp <- subset(ar_sample, type == "unrelated")
    ...
    practice <- '[
  {"word": "سنيي", "class": "nonword"},
  {"word": "شفه", "class": "word"},
  {"word": "قطران", "class": "word"},
  {"word": "جاب", "class": "nonword"},
  {"word": "نود", "class": "nonword"},
  {"word": "هدية مجانية", "class": "word"},
  {"word": "مخوط", "class": "nonword"},
  {"word": "طلقة", "class": "word"},
  {"word": "سنات", "class": "nonword"},
  {"word": "حذاء طويل", "class": "word"}]'

    writeLines(practice, con = paste0(
      "./04_Procedure/ar", folder_num,

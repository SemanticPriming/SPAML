...
# original word lists
el_words <- import("./04_Procedure/el/el_words.csv")

# collected data
el_data_all <-
  bind_rows(processData("./04_Procedure/el/data/data.sqlite") %>%
...
# Clean Up ----------------------------------------------------------------
  ...
  number_trials <- el_data_all %>% #data frame
...
# figure out trial type ----
  ...
  el_real_wide <- el_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                    "class.x", "correct.x", "trial_code.x",
                                    "duration.y", "word.y", "class.y", "correct.y",
                                    "Z_RT.y", "keep.y", "keep_participant.y",
                                    "ended_on.x", "ended_on.y")]
  # good names
  colnames(el_real_wide) <- c("unique_trial", "observation", "cue_word",
  ...
  el_focus <- subset(el_real_wide, target_type == "word" & cue_type == "word")
  el_focus$word_combo <- paste0(el_focus$cue_word, el_focus$target_word)

  # add if it's related or unrelated
  el_words$word_combo <- paste0(el_words$el_cue, el_words$el_target)
  el_focus <- merge(el_focus, el_words[ , c("type", "word_combo")],
                    by = "word_combo", all.x = T)

  ### HERE YOU WILL TURN ON ###
  # subset out NAs at some point they will be practice trials
  el_focus <- subset(el_focus, !is.na(type))

  # calculate the total N versus timeout N
  el_num_trials <- el_focus %>%
    group_by(word_combo) %>%
  ...
  ### HERE YOU WILL TURN ON ###
  el_focus <- subset(el_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
  el_Z <- subset(el_focus, target_correct == TRUE)
  el_Z <- subset(el_Z, keep_trial == "keep")
...
# merge with complete stimuli list ----
  el_merged <- merge(el_words, el_Z_summary,
                     by = "word_combo", all.x = T)

  el_merged <- merge(el_merged, el_num_trials,
                     by = "word_combo", all.x = T)

# merge with old data ----
  # pull in other information from previous weeks
  list_el_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "el_summary_[0-9].*.csv", full.names = T),
                         import)
  el_summaries <- bind_rows(list_el_data, el_merged)
  el_merged <- el_summaries %>%
    select(-any_of(c("done_both", "done_totalN", "done"))) %>%
    group_by(word_combo, el_cue, el_target, type, cue_type,
             target_type, el_cosine) %>%
  ...
  # are we done? ----
  el_merged$done_both <- (el_merged$target_answeredN >= 50 & el_merged$SE_Z <= .09) | el_merged$target_answeredN >= 320
  el_merged$done_totalN <- el_merged$target_answeredN >= 50
...
# generate summary chart for shiny ----
  write.csv(el_merged, "./04_Procedure/summary_data/el_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  p_lab <- el_data_all[el_data_all$observation %in% p_end, ]
  ...
  # pull in other information from previous weeks
  list_el_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "el_participants_[0-9].*.csv", full.names = T),
                         import)
  ...
  if (nrow(p_lab) > 0){
    ...
    p_lab <- unique(bind_rows(list_el_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/el_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(el_use[el_use$cue_type == "nonword" &
                    el_use$target_type == "nonword", ]) >= 150){

      temp <- subset(el_use,
                     el_use$cue_type == "nonword" &
                       el_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- el_use[el_use$cue_type == "nonword" &
                           el_use$target_type == "nonword", ]
      temp <- subset(el_sample,
                     el_use$cue_type == "nonword" &
                       el_use$target_type == "nonword")
    ...
    if (nrow(el_use[(el_use$cue_type == "nonword" &
                     el_use$target_type == "word") |
                    (el_use$cue_type == "word" &
                     el_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- el_use[(el_use$cue_type == "nonword" &
                                el_use$target_type == "word") |
                               (el_use$cue_type == "word" &
                                  el_use$target_type == "nonword"), ]
      temp <- subset(el_sample,
    ...
    if (nrow(el_use[el_use$type == "related" , ]) >= 75){

      temp <- subset(el_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- el_use[el_use$type == "related", ]
      temp <- subset(el_sample, type == "related")
    ...
    if (nrow(el_use[el_use$type == "unrelated" , ]) >= 75){

      temp <- subset(el_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- el_use[el_use$type == "unrelated", ]
      temp <- subset(el_sample, type == "unrelated")
    ...
    practice <- '[
  {"word": "χινι", "class": "nonword"},
  {"word": "χείλια", "class": "word"},
  {"word": "πίσσα", "class": "word"},
  {"word": "χριτνος", "class": "nonword"},
  {"word": "άτω", "class": "nonword"},
  {"word": "δώρο", "class": "word"},
  {"word": "κεγθε", "class": "nonword"},
  {"word": "βολή", "class": "word"},
  {"word": "βρόνια", "class": "nonword"},
  {"word": "μπότα", "class": "word"}]'

    writeLines(practice, con = paste0(
      "./04_Procedure/el", folder_num,

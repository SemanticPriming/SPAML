...
# original word lists
sk_words <- import("./04_Procedure/sk/sk_words.csv")

# collected data
sk_data_all <-
  bind_rows(processData("./04_Procedure/sk/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_sk_data <- lapply(list.files(path = "./04_Procedure/summary_data",
...
# generate summary chart for shiny ----
  write.csv(sk_merged, "./04_Procedure/summary_data/sk_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  write.csv(p_lab, "./04_Procedure/summary_data/sk_participants.csv", row.names = F)

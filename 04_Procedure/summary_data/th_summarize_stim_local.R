...
# original word lists
th_words <- import("./04_Procedure/th/th_words.csv")

# collected data
th_data_all <-
  list(processData("./04_Procedure/th/data/data.sqlite") %>%
              mutate_at(vars(one_of("url_lab")), as.character,
                          vars(one_of("url_special_code")), as.character))

  for (i in 1:length(th_data_all)){
    th_data_all[[i]] <- th_data_all[[i]] %>% mutate_at(vars(one_of("url_special_code")), as.character)
  }
...
# Clean Up ----------------------------------------------------------------
  ...
  number_trials <- th_data_all %>% #data frame
...
# grab only real trials ----
  real_trials <- th_data_all %>% #data frame
    filter(sender == "Stimulus Real") %>%  #filter out only the real stimuli
    select(observation, fix_sender, response, response_action, ended_on, duration,
           colnames(th_data_all)[grep("^time", colnames(th_data_all))],
...
# figure out trial type ----
  ...
  th_real_wide <- th_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                    "class.x", "correct.x", "trial_code.x",
                                    "duration.y", "word.y", "class.y", "correct.y",
                                    "Z_RT.y", "keep.y", "keep_participant.y",
                                    "ended_on.x", "ended_on.y")]
  # good names
  colnames(th_real_wide) <- c("unique_trial", "observation", "cue_word",
  ...
  th_focus <- subset(th_real_wide, target_type == "word" & cue_type == "word")
  th_focus$word_combo <- paste0(th_focus$cue_word, th_focus$target_word)

  # add if it's related or unrelated
  th_words$word_combo <- paste0(th_words$th_cue, th_words$th_target)
  th_focus <- merge(th_focus, th_words[ , c("type", "word_combo")],
                    by = "word_combo", all.x = T)

  ### HERE YOU WILL TURN ON ###
  # subset out NAs at some point they will be practice trials
  th_focus <- subset(th_focus, !is.na(type))

  # calculate the total N versus timeout N
  th_num_trials <- th_focus %>%
    group_by(word_combo) %>%
  ...
  ### HERE YOU WILL TURN ON ###
  th_focus <- subset(th_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
  th_Z <- subset(th_focus, target_correct == TRUE)
  th_Z <- subset(th_Z, keep_trial == "keep")
...
# merge with complete stimuli list ----
  th_merged <- merge(th_words, th_Z_summary,
                     by = "word_combo", all.x = T)

  th_merged <- merge(th_merged, th_num_trials,
                     by = "word_combo", all.x = T)

# merge with old data ----
  # pull in other information from previous weeks
  list_th_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "th_summary_[0-9].*.csv", full.names = T),
                         import)
  th_summaries <- bind_rows(list_th_data, th_merged)

  # a choice here to merge digusting with disgusting
  th_summaries$th_cue <- gsub("digusting", "disgusting", th_summaries$th_cue)
  ...
  # are we done? ----
  th_merged$done_both <- (th_merged$target_answeredN >= 50 & th_merged$SE_Z <= .09) | th_merged$target_answeredN >= 320
  th_merged$done_totalN <- th_merged$target_answeredN >= 50
  th_merged$done <- th_merged$sampleN >= 50

# use data ----
  th_use <- subset(th_merged, is.na(done_totalN) | done_totalN == FALSE)
  th_sample <- subset(th_merged, done_totalN == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
  write.csv(th_merged, "./04_Procedure/summary_data/th_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  p_lab <- th_data_all[th_data_all$observation %in% p_end, ]
  ...
  # pull in other information from previous weeks
  list_th_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "th_participants_[0-9].*.csv", full.names = T),
                         import)
  list_th_data <- lapply(list_th_data, function(df) dplyr::mutate_at(df, vars(matches("url_lab")), as.character))
  list_th_data <- lapply(list_th_data, function(df) dplyr::mutate_at(df, vars(matches("url_special_code")), as.character))
  list_th_data <- list_th_data[lapply(list_th_data, nrow) > 0]
  ...
  write.csv(p_lab, "./04_Procedure/summary_data/th_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(th_use[th_use$cue_type == "nonword" &
                    th_use$target_type == "nonword", ]) >= 150){

      temp <- subset(th_use,
                     th_use$cue_type == "nonword" &
                       th_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- th_use[th_use$cue_type == "nonword" &
                           th_use$target_type == "nonword", ]
      temp <- subset(th_sample,
                     th_use$cue_type == "nonword" &
                       th_use$target_type == "nonword")
    ...
    if (nrow(th_use[(th_use$cue_type == "nonword" &
                     th_use$target_type == "word") |
                    (th_use$cue_type == "word" &
                     th_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- th_use[(th_use$cue_type == "nonword" &
                                th_use$target_type == "word") |
                               (th_use$cue_type == "word" &
                                  th_use$target_type == "nonword"), ]
      temp <- subset(th_sample,
    ...
    if (nrow(th_use[th_use$type == "related" , ]) >= 75){

      temp <- subset(th_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- th_use[th_use$type == "related", ]
      temp <- subset(th_sample, type == "related")
    ...
    if (nrow(th_use[th_use$type == "unrelated" , ]) >= 75){

      temp <- subset(th_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- th_use[th_use$type == "unrelated", ]
      temp <- subset(th_sample, type == "unrelated")
    ...
    practice <- '[
  {"word": "แกมะ", "class": "nonword"},
  {"word": "เปิดใช้งาน", "class": "word"},
  {"word": "การแจ้งเตือน", "class": "word"},
  {"word": "ความแง่ดี", "class": "nonword"},
  {"word": "บาดงกาย", "class": "nonword"},
  {"word": "ฮอร์โมน", "class": "word"},
  {"word": "ภายเจน", "class": "nonword"},
  {"word": "บริษัท", "class": "word"},
  {"word": "วิสศพ", "class": "nonword"},
  {"word": "ข้อตกลง", "class": "word"}]'

    writeLines(practice, con = paste0(
      "./04_Procedure/th", folder_num,

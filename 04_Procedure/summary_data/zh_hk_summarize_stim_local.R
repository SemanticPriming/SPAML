...
# original word lists
zh_hk_words <- import("./04_Procedure/zh_hk/zh_hk_words.csv")

# collected data
zh_hk_data_all <-
  bind_rows(processData("./04_Procedure/zh_hk/data/data.sqlite") %>%
...
# Clean Up ----------------------------------------------------------------
  ...
  number_trials <- zh_hk_data_all %>% #data frame
...
# figure out trial type ----
  ...
  zh_hk_real_wide <- zh_hk_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                    "class.x", "correct.x", "trial_code.x",
                                    "duration.y", "word.y", "class.y", "correct.y",
                                    "Z_RT.y", "keep.y", "keep_participant.y",
                                    "ended_on.x", "ended_on.y")]
  # good names
  colnames(zh_hk_real_wide) <- c("unique_trial", "observation", "cue_word",
  ...
  zh_hk_focus <- subset(zh_hk_real_wide, target_type == "word" & cue_type == "word")
  zh_hk_focus$word_combo <- paste0(zh_hk_focus$cue_word, zh_hk_focus$target_word)

  # add if it's related or unrelated
  zh_hk_words$word_combo <- paste0(zh_hk_words$zh_hk_cue, zh_hk_words$zh_hk_target)
  zh_hk_focus <- merge(zh_hk_focus, zh_hk_words[ , c("type", "word_combo")],
                    by = "word_combo", all.x = T)

  ### HERE YOU WILL TURN ON ###
  # subset out NAs at some point they will be practice trials
  zh_hk_focus <- subset(zh_hk_focus, !is.na(type))

  # calculate the total N versus timeout N
  zh_hk_num_trials <- zh_hk_focus %>%
    group_by(word_combo) %>%
  ...
  ### HERE YOU WILL TURN ON ###
  zh_hk_focus <- subset(zh_hk_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
  zh_hk_Z <- subset(zh_hk_focus, target_correct == TRUE)
  zh_hk_Z <- subset(zh_hk_Z, keep_trial == "keep")

# Calculate Statistics ----------------------------------------------------

# calculates word, sample size, SE, "done" with <= .09 SE ----
  zh_hk_Z_summary <- zh_hk_Z %>%
    group_by(word_combo) %>%
...
# merge with complete stimuli list ----
  zh_hk_merged <- merge(zh_hk_words, zh_hk_Z_summary,
                     by = "word_combo", all.x = T)

  zh_hk_merged <- merge(zh_hk_merged, zh_hk_num_trials,
                     by = "word_combo", all.x = T)

# merge with old data ----
  # pull in other information from previous weeks
  list_zh_hk_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "zh_hk_summary_[0-9].*.csv", full.names = T),
                         import)
  zh_hk_summaries <- bind_rows(list_zh_hk_data, zh_hk_merged)
  zh_hk_merged <- zh_hk_summaries %>%
    select(-any_of(c("done_both", "done_totalN", "done"))) %>%
    group_by(word_combo, zh_hk_cue, zh_hk_target, type, cue_type,
             target_type, zh_hk_cosine) %>%
  ...
  # are we done? ----
  zh_hk_merged$done_both <- (zh_hk_merged$target_answeredN >= 50 & zh_hk_merged$SE_Z <= .09) | zh_hk_merged$target_answeredN >= 320
  zh_hk_merged$done_totalN <- zh_hk_merged$target_answeredN >= 50
...
# generate summary chart for shiny ----
  write.csv(zh_hk_merged, "./04_Procedure/summary_data/zh_hk_summary.csv", row.names = F)

# generate participant report for shiny ----
  p_end <- zh_hk_data_all %>%
  ...
  p_lab <- zh_hk_data_all[zh_hk_data_all$observation %in% p_end, ]
  ...
  # pull in other information from previous weeks
  list_zh_hk_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "zh_hk_participants_[0-9].*.csv", full.names = T),
                         import)
  list_zh_hk_data <- lapply(list_zh_hk_data, function(df) dplyr::mutate_at(df, vars(matches("url_lab")), as.character))
  list_zh_hk_data <- lapply(list_zh_hk_data, function(df) dplyr::mutate_at(df, vars(matches("url_special_code")), as.character))
  list_zh_hk_data <- list_zh_hk_data[lapply(list_zh_hk_data, nrow) > 0]

  if (nrow(p_lab) > 0){
    if (length(list_zh_hk_data) > 0){
      p_lab <- unique(bind_rows(bind_rows(list_zh_hk_data) %>%
    ...
    p_lab <- unique(bind_rows(list_zh_hk_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/zh_hk_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(zh_hk_use[zh_hk_use$cue_type == "nonword" &
                    zh_hk_use$target_type == "nonword", ]) >= 150){

      temp <- subset(zh_hk_use,
                     zh_hk_use$cue_type == "nonword" &
                       zh_hk_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- zh_hk_use[zh_hk_use$cue_type == "nonword" &
                           zh_hk_use$target_type == "nonword", ]
      temp <- subset(zh_hk_sample,
                     zh_hk_use$cue_type == "nonword" &
                       zh_hk_use$target_type == "nonword")
    ...
    if (nrow(zh_hk_use[(zh_hk_use$cue_type == "nonword" &
                     zh_hk_use$target_type == "word") |
                    (zh_hk_use$cue_type == "word" &
                     zh_hk_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- zh_hk_use[(zh_hk_use$cue_type == "nonword" &
                                zh_hk_use$target_type == "word") |
                               (zh_hk_use$cue_type == "word" &
                                  zh_hk_use$target_type == "nonword"), ]
      temp <- subset(zh_hk_sample,
    ...
    if (nrow(zh_hk_use[zh_hk_use$type == "related" , ]) >= 75){

      temp <- subset(zh_hk_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- zh_hk_use[zh_hk_use$type == "related", ]
      temp <- subset(zh_hk_sample, type == "related")
    ...
    if (nrow(zh_hk_use[zh_hk_use$type == "unrelated" , ]) >= 75){

      temp <- subset(zh_hk_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- zh_hk_use[zh_hk_use$type == "unrelated", ]
      temp <- subset(zh_hk_sample, type == "unrelated")
  ...
  {"word": "støvle", "class": "word"}]'
    ...
    all_trials$together <- paste('{"word": "',
                                 all_trials$zh_hk_cue,
                                 '", "class": "',
                                 all_trials$cue_type,
                                 '"}, ', #cue
                                 '{"word": "',
                                 all_trials$zh_hk_target,
                                 '", "class": "',

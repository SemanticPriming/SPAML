...
# original word lists
hi_words <- import("/var/www/html/hi/hi_words.csv")

# collected data
hi_data_all <-
  bind_rows(processData("/var/www/html/hi/data/data.sqlite") %>%
...
list_hi_data <- lapply(list.files(path = "/var/www/html/summary_data",
...
# use data ----
hi_use <- subset(hi_merged, is.na(done) | done == FALSE)
hi_sample <- subset(hi_merged, done_totalN == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
write.csv(hi_merged, "/var/www/html/summary_data/hi_summary.csv", row.names = F)
...
write.csv(p_lab, "/var/www/html/summary_data/hi_participants.csv", row.names = F)

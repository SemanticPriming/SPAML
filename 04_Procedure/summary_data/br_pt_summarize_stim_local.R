...
# original word lists
br_pt_words <- import("./04_Procedure/br_pt/br_pt_words.csv")

# collected data
br_pt_data_all <-
  bind_rows(processData("./04_Procedure/br_pt/data/data.sqlite") %>%
...
# Clean Up ----------------------------------------------------------------
  ...
  number_trials <- br_pt_data_all %>% #data frame
...
# figure out trial type ----
  ...
  br_pt_real_wide <- br_pt_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                    "class.x", "correct.x", "trial_code.x",
                                    "duration.y", "word.y", "class.y", "correct.y",
                                    "Z_RT.y", "keep.y", "keep_participant.y",
                                    "ended_on.x", "ended_on.y")]
  # good names
  colnames(br_pt_real_wide) <- c("unique_trial", "observation", "cue_word",
  ...
  br_pt_focus <- subset(br_pt_real_wide, target_type == "word" & cue_type == "word")
  br_pt_focus$word_combo <- paste0(br_pt_focus$cue_word, br_pt_focus$target_word)

  # add if it's related or unrelated
  br_pt_words$word_combo <- paste0(br_pt_words$pt_cue, br_pt_words$pt_target)
  br_pt_focus <- merge(br_pt_focus, br_pt_words[ , c("type", "word_combo")],
                    by = "word_combo", all.x = T)

  ### HERE YOU WILL TURN ON ###
  # subset out NAs at some point they will be practice trials
  br_pt_focus <- subset(br_pt_focus, !is.na(type))

  # calculate the total N versus timeout N
  br_pt_num_trials <- br_pt_focus %>%
    group_by(word_combo) %>%
  ...
  ### HERE YOU WILL TURN ON ###
  br_pt_focus <- subset(br_pt_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
  br_pt_Z <- subset(br_pt_focus, target_correct == TRUE)
  br_pt_Z <- subset(br_pt_Z, keep_trial == "keep")

# Calculate Statistics ----------------------------------------------------

# calculates word, sample size, SE, "done" with <= .09 SE ----
  br_pt_Z_summary <- br_pt_Z %>%
    group_by(word_combo) %>%
...
# merge with complete stimuli list ----
  br_pt_merged <- merge(br_pt_words, br_pt_Z_summary,
                     by = "word_combo", all.x = T)
...
# generate summary chart for shiny ----
  write.csv(br_pt_merged, "./04_Procedure/summary_data/br_pt_summary.csv", row.names = F)

# generate participant report for shiny ----
  p_end <- br_pt_data_all %>%
  ...
  p_lab <- br_pt_data_all[br_pt_data_all$observation %in% p_end, ]

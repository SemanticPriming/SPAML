...
# original word lists
fr_words <- import("/var/www/html/fr/fr_words.csv")

# collected data
fr_data_all <-
  bind_rows(processData("/var/www/html/fr/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab)),
            processData("/var/www/html/fr1/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab)),
            processData("/var/www/html/fr2/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab)),
            processData("/var/www/html/fr3/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab)),
            processData("/var/www/html/fr4/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab)),
            processData("/var/www/html/fr5/data/data.sqlite") %>%
...
number_trials <- fr_data_all %>% #data frame
...
fr_real_wide <- fr_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                  "class.x", "correct.x", "trial_code.x",
                                  "duration.y", "word.y", "class.y", "correct.y",
                                  "Z_RT.y", "keep.y", "keep_participant.y",
                                  "ended_on.x", "ended_on.y")]
# good names
colnames(fr_real_wide) <- c("unique_trial", "observation", "cue_word",
...
fr_focus <- subset(fr_real_wide, target_type == "word" & cue_type == "word")
fr_focus$word_combo <- paste0(fr_focus$cue_word, fr_focus$target_word)

# add if it's related or unrelated
fr_words$word_combo <- paste0(fr_words$fr_cue, fr_words$fr_target)
fr_focus <- merge(fr_focus, fr_words[ , c("type", "word_combo")],
                  by = "word_combo", all.x = T)

### HERE YOU WILL TURN ON ###
# subset out NAs at some point they will be practice trials
fr_focus <- subset(fr_focus, !is.na(type))

# calculate the total N versus timeout N
fr_num_trials <- fr_focus %>%
  group_by(word_combo) %>%
...
### HERE YOU WILL TURN ON ###
fr_focus <- subset(fr_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
fr_Z <- subset(fr_focus, target_correct == TRUE)
fr_Z <- subset(fr_Z, keep_trial == "keep")
...
fr_merged <- merge(fr_words, fr_Z_summary,
                   by = "word_combo", all.x = T)

fr_merged <- merge(fr_merged, fr_num_trials,
                   by = "word_combo", all.x = T)

# merge with old data ----
# pull in other information from previous weeks
list_fr_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "fr_summary_[0-9].*.csv", full.names = T),
                       import)
fr_summaries <- bind_rows(list_fr_data, fr_merged)
fr_merged <- fr_summaries %>%
  select(-any_of(c("done_both", "done_totalN", "done"))) %>%
  group_by(word_combo, fr_cue, fr_target, type, cue_type,
           target_type, fr_cosine) %>%
...
# are we done? ----
fr_merged$done_both <- (fr_merged$target_answeredN >= 50 & fr_merged$SE_Z <= .09) | fr_merged$target_answeredN >= 320
fr_merged$done_totalN <- fr_merged$target_answeredN >= 50
...
write.csv(fr_merged, "/var/www/html/summary_data/fr_summary.csv", row.names = F)
...
p_lab <- fr_data_all[fr_data_all$observation %in% p_end, ]
...
# pull in other information from previous weeks
list_fr_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "fr_participants_[0-9].*.csv", full.names = T),
                       import)
...
if (nrow(p_lab) > 0){
  ...
  p_lab <- unique(bind_rows(list_fr_data))
}

write.csv(p_lab, "/var/www/html/summary_data/fr_participants.csv", row.names = F)
...
for (i in 1:number_folders){
  ...
  if (nrow(fr_use[fr_use$cue_type == "nonword" &
                  fr_use$target_type == "nonword", ]) >= 150){

    temp <- subset(fr_use,
                   fr_use$cue_type == "nonword" &
                     fr_use$target_type == "nonword")
    nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

  }else{

    nonwords <- fr_use[fr_use$cue_type == "nonword" &
                         fr_use$target_type == "nonword", ]
    temp <- subset(fr_sample,
                   fr_use$cue_type == "nonword" &
                     fr_use$target_type == "nonword")
  ...
  if (nrow(fr_use[(fr_use$cue_type == "nonword" &
                   fr_use$target_type == "word") |
                  (fr_use$cue_type == "word" &
                   fr_use$target_type == "nonword"), ]) >= 100){
    ...
    nonwords_mix <- fr_use[(fr_use$cue_type == "nonword" &
                              fr_use$target_type == "word") |
                             (fr_use$cue_type == "word" &
                                fr_use$target_type == "nonword"), ]
    temp <- subset(fr_sample,
  ...
  if (nrow(fr_use[fr_use$type == "related" , ]) >= 75){

    temp <- subset(fr_use, type == "related")
    related <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    related <- fr_use[fr_use$type == "related", ]
    temp <- subset(fr_sample, type == "related")
  ...
  if (nrow(fr_use[fr_use$type == "unrelated" , ]) >= 75){

    temp <- subset(fr_use, type == "unrelated")
    unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    unrelated <- fr_use[fr_use$type == "unrelated", ]
    temp <- subset(fr_sample, type == "unrelated")

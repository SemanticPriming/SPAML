...
# original word lists
fa_words <- import("./04_Procedure/fa/fa_words.csv")

# collected data
fa_data_all <-
  bind_rows(processData("./04_Procedure/fa/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_fa_data <- lapply(list.files(path = "./04_Procedure/summary_data",
...
# generate summary chart for shiny ----
  write.csv(fa_merged, "./04_Procedure/summary_data/fa_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  write.csv(p_lab, "./04_Procedure/summary_data/fa_participants.csv", row.names = F)

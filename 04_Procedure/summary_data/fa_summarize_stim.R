...
# original word lists
fa_words <- import("/var/www/html/fa/fa_words.csv")

# collected data
fa_data_all <-
  bind_rows(processData("/var/www/html/fa/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab))) %>% unique()

# # delete stuff before we started
fa_data_all <- fa_data_all %>%
  filter(timestamp > as.POSIXct("2023-03-17"))
...
number_trials <- fa_data_all %>% #data frame
...
real_trials <- fa_data_all %>% #data frame
  filter(sender == "Stimulus Real") %>%  #filter out only the real stimuli
  select(observation, fix_sender, response, response_action, ended_on, duration,
         colnames(fa_data_all)[grep("^time", colnames(fa_data_all))],
...
fa_real_wide <- fa_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                  "class.x", "correct.x", "trial_code.x",
                                  "duration.y", "word.y", "class.y", "correct.y",
                                  "Z_RT.y", "keep.y", "keep_participant.y",
                                  "ended_on.x", "ended_on.y")]
# good names
colnames(fa_real_wide) <- c("unique_trial", "observation", "cue_word",
...
fa_focus <- subset(fa_real_wide, target_type == "word" & cue_type == "word")
fa_focus$word_combo <- paste0(fa_focus$cue_word, fa_focus$target_word)

# add if it's related or unrelated
fa_words$word_combo <- paste0(fa_words$fa_cue, fa_words$fa_target)
fa_focus <- merge(fa_focus, fa_words[ , c("type", "word_combo")],
                  by = "word_combo", all.x = T)

### HERE YOU WILL TURN ON ###
# subset out NAs at some point they will be practice trials
fa_focus <- subset(fa_focus, !is.na(type))

# calculate the total N versus timeout N
fa_num_trials <- fa_focus %>%
  group_by(word_combo) %>%
...
### HERE YOU WILL TURN ON ###
fa_focus <- subset(fa_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
fa_Z <- subset(fa_focus, target_correct == TRUE)
fa_Z <- subset(fa_Z, keep_trial == "keep")
...
fa_merged <- merge(fa_words, fa_Z_summary,
                   by = "word_combo", all.x = T)

fa_merged <- merge(fa_merged, fa_num_trials,
                   by = "word_combo", all.x = T)

# merge with old data ----
# pull in other information from previous weeks
list_fa_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "fa_summary_[0-9].*.csv", full.names = T),
                       import)
fa_summaries <- bind_rows(list_fa_data, fa_merged)
fa_merged <- fa_summaries %>%
  select(-any_of(c("done_both", "done_totalN", "done"))) %>%
  group_by(word_combo, fa_cue, fa_target, type, cue_type,
           target_type, fa_cosine) %>%
...
# are we done? ----
fa_merged$done_both <- (fa_merged$target_answeredN >= 50 & fa_merged$SE_Z <= .09) | fa_merged$target_answeredN >= 320
fa_merged$done_totalN <- fa_merged$target_answeredN >= 50
fa_merged$done <- fa_merged$sampleN >= 50

# use data ----
fa_use <- subset(fa_merged, is.na(done_totalN) | done_totalN == FALSE)
fa_sample <- subset(fa_merged, done_totalN == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
write.csv(fa_merged, "/var/www/html/summary_data/fa_summary.csv", row.names = F)
...
p_lab <- fa_data_all[fa_data_all$observation %in% p_end, ]
...
# pull in other information from previous weeks
list_fa_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "fa_participants_[0-9].*.csv", full.names = T),
                       import)
...
if (nrow(p_lab) > 0){
  if (length(list_fa_data) > 0){
    p_lab <- unique(bind_rows(bind_rows(list_fa_data) %>%
  ...
  p_lab <- unique(bind_rows(list_fa_data))
}

write.csv(p_lab, "/var/www/html/summary_data/fa_participants.csv", row.names = F)
...
for (i in 1:number_folders){
  ...
  if (nrow(fa_use[fa_use$cue_type == "nonword" &
                  fa_use$target_type == "nonword", ]) >= 150){

    temp <- subset(fa_use,
                   fa_use$cue_type == "nonword" &
                     fa_use$target_type == "nonword")
    nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

  }else{

    nonwords <- fa_use[fa_use$cue_type == "nonword" &
                         fa_use$target_type == "nonword", ]
    temp <- subset(fa_sample,
                   fa_use$cue_type == "nonword" &
                     fa_use$target_type == "nonword")
  ...
  if (nrow(fa_use[(fa_use$cue_type == "nonword" &
                   fa_use$target_type == "word") |
                  (fa_use$cue_type == "word" &
                   fa_use$target_type == "nonword"), ]) >= 100){
    ...
    nonwords_mix <- fa_use[(fa_use$cue_type == "nonword" &
                              fa_use$target_type == "word") |
                             (fa_use$cue_type == "word" &
                                fa_use$target_type == "nonword"), ]
    temp <- subset(fa_sample,
  ...
  if (nrow(fa_use[fa_use$type == "related" , ]) >= 75){

    temp <- subset(fa_use, type == "related")
    related <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    related <- fa_use[fa_use$type == "related", ]
    temp <- subset(fa_sample, type == "related")
  ...
  if (nrow(fa_use[fa_use$type == "unrelated" , ]) >= 75){

    temp <- subset(fa_use, type == "unrelated")
    unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    unrelated <- fa_use[fa_use$type == "unrelated", ]
    temp <- subset(fa_sample, type == "unrelated")
  ...
  practice <- '[
  {"word": "بر", "class": "nonword"},
  {"word": "لب", "class": "word"},
  {"word": "تار", "class": "word"},
  {"word": "جاب", "class": "nonword"},
  {"word": "پاین", "class": "nonword"},
  {"word": "هدیه", "class": "word"},
  {"word": "کهی", "class": "nonword"},

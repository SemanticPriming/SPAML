...
# original word lists
de_words <- import("/var/www/html/de/de_words.csv")
...
number_trials <- de_data_all %>% #data frame
...
de_real_wide <- de_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                  "class.x", "correct.x", "trial_code.x",
                                  "duration.y", "word.y", "class.y", "correct.y",
                                  "Z_RT.y", "keep.y", "keep_participant.y",
                                  "ended_on.x", "ended_on.y")]
# good names
colnames(de_real_wide) <- c("unique_trial", "observation", "cue_word",
...
de_focus <- subset(de_real_wide, target_type == "word" & cue_type == "word")
de_focus$word_combo <- paste0(de_focus$cue_word, de_focus$target_word)

# add if it's related or unrelated
de_words$word_combo <- paste0(de_words$de_cue, de_words$de_target)
de_focus <- merge(de_focus, de_words[ , c("type", "word_combo")],
                  by = "word_combo", all.x = T)

### HERE YOU WILL TURN ON ###
# subset out NAs at some point they will be practice trials
de_focus <- subset(de_focus, !is.na(type))

# calculate the total N versus timeout N
de_num_trials <- de_focus %>%
  group_by(word_combo) %>%
...
### HERE YOU WILL TURN ON ###
de_focus <- subset(de_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
de_Z <- subset(de_focus, target_correct == TRUE)
de_Z <- subset(de_Z, keep_trial == "keep")
...
de_merged <- merge(de_words, de_Z_summary,
                   by = "word_combo", all.x = T)
...
list_de_data <- lapply(list.files(path = "/var/www/html/summary_data",
...
write.csv(de_merged, "/var/www/html/summary_data/de_summary.csv", row.names = F)
...
p_lab <- de_data_all[de_data_all$observation %in% p_end, ]
...
write.csv(p_lab, "/var/www/html/summary_data/de_participants.csv", row.names = F)
...
for (i in 1:number_folders){
  ...
  if (nrow(de_use[de_use$cue_type == "nonword" &
                  de_use$target_type == "nonword", ]) >= 150){

    temp <- subset(de_use,
                   de_use$cue_type == "nonword" &
                     de_use$target_type == "nonword")
    nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

  }else{

    nonwords <- de_use[de_use$cue_type == "nonword" &
                         de_use$target_type == "nonword", ]
    temp <- subset(de_sample,
                   de_use$cue_type == "nonword" &
                     de_use$target_type == "nonword")
  ...
  if (nrow(de_use[(de_use$cue_type == "nonword" &
                   de_use$target_type == "word") |
                  (de_use$cue_type == "word" &
                   de_use$target_type == "nonword"), ]) >= 100){
    ...
    nonwords_mix <- de_use[(de_use$cue_type == "nonword" &
                              de_use$target_type == "word") |
                             (de_use$cue_type == "word" &
                                de_use$target_type == "nonword"), ]
    temp <- subset(de_sample,
  ...
  if (nrow(de_use[de_use$type == "related" , ]) >= 75){

    temp <- subset(de_use, type == "related")
    related <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    related <- de_use[de_use$type == "related", ]
    temp <- subset(de_sample, type == "related")
  ...
  if (nrow(de_use[de_use$type == "unrelated" , ]) >= 75){

    temp <- subset(de_use, type == "unrelated")
    unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    unrelated <- de_use[de_use$type == "unrelated", ]
    temp <- subset(de_sample, type == "unrelated")

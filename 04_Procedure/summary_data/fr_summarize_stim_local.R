...
# original word lists
fr_words <- import("./04_Procedure/fr/fr_words.csv")

# collected data
fr_data_all <-
  bind_rows(processData("./04_Procedure/fr/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_fr_data <- lapply(list.files(path = "./04_Procedure/summary_data",
...
# generate summary chart for shiny ----
  write.csv(fr_merged, "./04_Procedure/summary_data/fr_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  write.csv(p_lab, "./04_Procedure/summary_data/fr_participants.csv", row.names = F)

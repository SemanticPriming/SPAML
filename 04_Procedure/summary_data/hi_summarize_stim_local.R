...
# original word lists
hi_words <- import("./04_Procedure/hi/hi_words.csv")

# collected data
hi_data_all <-
  bind_rows(processData("./04_Procedure/hi/data/data.sqlite") %>%
...
# Clean Up ----------------------------------------------------------------
  ...
  number_trials <- hi_data_all %>% #data frame
...
# grab only real trials ----
  real_trials <- hi_data_all %>% #data frame
    filter(sender == "Stimulus Real") %>%  #filter out only the real stimuli
    select(observation, fix_sender, response, response_action, ended_on, duration,
           colnames(hi_data_all)[grep("^time", colnames(hi_data_all))],
...
# figure out trial type ----
  ...
  hi_real_wide <- hi_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                    "class.x", "correct.x", "trial_code.x",
                                    "duration.y", "word.y", "class.y", "correct.y",
                                    "Z_RT.y", "keep.y", "keep_participant.y",
                                    "ended_on.x", "ended_on.y")]
  # good names
  colnames(hi_real_wide) <- c("unique_trial", "observation", "cue_word",
  ...
  hi_focus <- subset(hi_real_wide, target_type == "word" & cue_type == "word")
  hi_focus$word_combo <- paste0(hi_focus$cue_word, hi_focus$target_word)

  # add if it's related or unrelated
  hi_words$word_combo <- paste0(hi_words$hi_cue, hi_words$hi_target)
  hi_focus <- merge(hi_focus, hi_words[ , c("type", "word_combo")],
                    by = "word_combo", all.x = T)

  ### HERE YOU WILL TURN ON ###
  # subset out NAs at some point they will be practice trials
  hi_focus <- subset(hi_focus, !is.na(type))

  # calculate the total N versus timeout N
  hi_num_trials <- hi_focus %>%
    group_by(word_combo) %>%
  ...
  ### HERE YOU WILL TURN ON ###
  hi_focus <- subset(hi_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
  hi_Z <- subset(hi_focus, target_correct == TRUE)
  hi_Z <- subset(hi_Z, keep_trial == "keep")
...
# merge with complete stimuli list ----
  hi_merged <- merge(hi_words, hi_Z_summary,
                     by = "word_combo", all.x = T)

  hi_merged <- merge(hi_merged, hi_num_trials,
                     by = "word_combo", all.x = T)

# merge with old data ----
  # pull in other information from previous weeks
  list_hi_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "hi_summary_[0-9].*.csv", full.names = T),
                         import)
  hi_summaries <- bind_rows(list_hi_data, hi_merged)
  hi_merged <- hi_summaries %>%
    select(-any_of(c("done_both", "done_totalN", "done"))) %>%
    group_by(word_combo, hi_cue, hi_target, type, cue_type,
             target_type, hi_cosine) %>%
  ...
  # are we done? ----
  hi_merged$done_both <- (hi_merged$target_answeredN >= 50 & hi_merged$SE_Z <= .09) | hi_merged$target_answeredN >= 320
  hi_merged$done_totalN <- hi_merged$target_answeredN >= 50
  hi_merged$done <- hi_merged$sampleN >= 50

# use data ----
  hi_use <- subset(hi_merged, is.na(done_totalN) | done_totalN == FALSE)
  hi_sample <- subset(hi_merged, done_totalN == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
  write.csv(hi_merged, "./04_Procedure/summary_data/hi_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  p_lab <- hi_data_all[hi_data_all$observation %in% p_end, ]
  ...
  # pull in other information from previous weeks
  list_hi_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "hi_participants_[0-9].*.csv", full.names = T),
                         import)
  list_hi_data <- lapply(list_hi_data, function(df) dplyr::mutate_at(df, vars(matches("url_lab")), as.character))
  list_hi_data <- lapply(list_hi_data, function(df) dplyr::mutate_at(df, vars(matches("url_special_code")), as.character))
  list_hi_data <- list_hi_data[lapply(list_hi_data, nrow) > 0]

  if (nrow(p_lab) > 0){
    if (length(list_hi_data) > 0){
      p_lab <- unique(bind_rows(bind_rows(list_hi_data) %>%
    ...
    p_lab <- unique(bind_rows(list_hi_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/hi_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(hi_use[hi_use$cue_type == "nonword" &
                    hi_use$target_type == "nonword", ]) >= 150){

      temp <- subset(hi_use,
                     hi_use$cue_type == "nonword" &
                       hi_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- hi_use[hi_use$cue_type == "nonword" &
                           hi_use$target_type == "nonword", ]
      temp <- subset(hi_sample,
                     hi_use$cue_type == "nonword" &
                       hi_use$target_type == "nonword")
    ...
    if (nrow(hi_use[(hi_use$cue_type == "nonword" &
                     hi_use$target_type == "word") |
                    (hi_use$cue_type == "word" &
                     hi_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- hi_use[(hi_use$cue_type == "nonword" &
                                hi_use$target_type == "word") |
                               (hi_use$cue_type == "word" &
                                  hi_use$target_type == "nonword"), ]
      temp <- subset(hi_sample,
    ...
    if (nrow(hi_use[hi_use$type == "related" , ]) >= 75){

      temp <- subset(hi_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- hi_use[hi_use$type == "related", ]
      temp <- subset(hi_sample, type == "related")
    ...
    if (nrow(hi_use[hi_use$type == "unrelated" , ]) >= 75){

      temp <- subset(hi_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- hi_use[hi_use$type == "unrelated", ]
      temp <- subset(hi_sample, type == "unrelated")

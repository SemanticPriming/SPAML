...
list_hu_data <- lapply(list.files(path = "/var/www/html/summary_data",
...
# use data ----
hu_use <- subset(hu_merged, is.na(done) | done == FALSE)
hu_sample <- subset(hu_merged, done == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
write.csv(hu_merged, "/var/www/html/summary_data/hu_summary.csv", row.names = F)
...
write.csv(p_lab, "/var/www/html/summary_data/hu_participants.csv", row.names = F)

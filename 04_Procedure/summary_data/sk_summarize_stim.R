...
# original word lists
sk_words <- import("/var/www/html/sk/sk_words.csv")

# collected data
sk_data_all <-
  bind_rows(processData("/var/www/html/sk/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab))) %>% unique()

# # remove test data before official start 
# sk_data_all <- sk_data_all %>% 
...
number_trials <- sk_data_all %>% #data frame
...
real_trials <- sk_data_all %>% #data frame
  filter(sender == "Stimulus Real") %>%  #filter out only the real stimuli
  select(observation, fix_sender, response, response_action, ended_on, duration,
         colnames(sk_data_all)[grep("^time", colnames(sk_data_all))],
...
sk_real_wide <- sk_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                  "class.x", "correct.x", "trial_code.x",
                                  "duration.y", "word.y", "class.y", "correct.y",
                                  "Z_RT.y", "keep.y", "keep_participant.y",
                                  "ended_on.x", "ended_on.y")]
# good names
colnames(sk_real_wide) <- c("unique_trial", "observation", "cue_word",
...
sk_focus <- subset(sk_real_wide, target_type == "word" & cue_type == "word")
sk_focus$word_combo <- paste0(sk_focus$cue_word, sk_focus$target_word)

# add if it's related or unrelated
sk_words$word_combo <- paste0(sk_words$sk_cue, sk_words$sk_target)
sk_focus <- merge(sk_focus, sk_words[ , c("type", "word_combo")],
                  by = "word_combo", all.x = T)

### HERE YOU WILL TURN ON ###
# subset out NAs at some point they will be practice trials
sk_focus <- subset(sk_focus, !is.na(type))

# calculate the total N versus timeout N
sk_num_trials <- sk_focus %>%
  group_by(word_combo) %>%
...
### HERE YOU WILL TURN ON ###
sk_focus <- subset(sk_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
sk_Z <- subset(sk_focus, target_correct == TRUE)
sk_Z <- subset(sk_Z, keep_trial == "keep")
...
sk_merged <- merge(sk_words, sk_Z_summary,
                   by = "word_combo", all.x = T)

sk_merged <- merge(sk_merged, sk_num_trials,
                   by = "word_combo", all.x = T)

# merge with old data ----
# pull in other information from previous weeks
list_sk_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "sk_summary_[0-9].*.csv", full.names = T),
                       import)
sk_summaries <- bind_rows(list_sk_data, sk_merged)
sk_merged <- sk_summaries %>%
  select(-any_of(c("done_both", "done_totalN", "done"))) %>%
  group_by(word_combo, sk_cue, sk_target, type, cue_type,
           target_type, sk_cosine) %>%
...
# are we done? ----
sk_merged$done_both <- (sk_merged$target_answeredN >= 50 & sk_merged$SE_Z <= .09) | sk_merged$target_answeredN >= 320
sk_merged$done_totalN <- sk_merged$target_answeredN >= 50
sk_merged$done <- sk_merged$sampleN >= 50

# use data ----
sk_use <- subset(sk_merged, is.na(done_totalN) | done_totalN == FALSE)
sk_sample <- subset(sk_merged, done_totalN == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
write.csv(sk_merged, "/var/www/html/summary_data/sk_summary.csv", row.names = F)
...
p_lab <- sk_data_all[sk_data_all$observation %in% p_end, ]
...
# pull in other information from previous weeks
list_sk_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "sk_participants_[0-9].*.csv", full.names = T),
                       import)
list_sk_data <- lapply(list_sk_data, function(df) dplyr::mutate_at(df, vars(matches("url_lab")), as.character))
list_sk_data <- lapply(list_sk_data, function(df) dplyr::mutate_at(df, vars(matches("url_special_code")), as.character))
list_sk_data <- list_sk_data[lapply(list_sk_data, nrow) > 0]

if (nrow(p_lab) > 0){
  if (length(list_sk_data) > 0){
    p_lab <- unique(bind_rows(bind_rows(list_sk_data) %>%
  ...
  p_lab <- unique(bind_rows(list_sk_data))
}

write.csv(p_lab, "/var/www/html/summary_data/sk_participants.csv", row.names = F)
...
for (i in 1:number_folders){
  ...
  if (nrow(sk_use[sk_use$cue_type == "nonword" &
                  sk_use$target_type == "nonword", ]) >= 150){

    temp <- subset(sk_use,
                   sk_use$cue_type == "nonword" &
                     sk_use$target_type == "nonword")
    nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

  }else{

    nonwords <- sk_use[sk_use$cue_type == "nonword" &
                         sk_use$target_type == "nonword", ]
    temp <- subset(sk_sample,
                   sk_use$cue_type == "nonword" &
                     sk_use$target_type == "nonword")
  ...
  if (nrow(sk_use[(sk_use$cue_type == "nonword" &
                   sk_use$target_type == "word") |
                  (sk_use$cue_type == "word" &
                   sk_use$target_type == "nonword"), ]) >= 100){
    ...
    nonwords_mix <- sk_use[(sk_use$cue_type == "nonword" &
                              sk_use$target_type == "word") |
                             (sk_use$cue_type == "word" &
                                sk_use$target_type == "nonword"), ]
    temp <- subset(sk_sample,
  ...
  if (nrow(sk_use[sk_use$type == "related" , ]) >= 75){

    temp <- subset(sk_use, type == "related")
    related <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    related <- sk_use[sk_use$type == "related", ]
    temp <- subset(sk_sample, type == "related")
  ...
  if (nrow(sk_use[sk_use$type == "unrelated" , ]) >= 75){

    temp <- subset(sk_use, type == "unrelated")
    unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    unrelated <- sk_use[sk_use$type == "unrelated", ]
    temp <- subset(sk_sample, type == "unrelated")
  ...
  practice <- '[
  {"word": "aset", "class": "nonword"},
  {"word": "pery", "class": "word"},
  {"word": "decht", "class": "word"},
  {"word": "hodip", "class": "nonword"},
  {"word": "cyborgký", "class": "nonword"},
  {"word": "darček", "class": "word"},
  {"word": "hnes", "class": "nonword"},
  {"word": "výstup", "class": "word"},
  {"word": "znakdy", "class": "nonword"},
  {"word": "zahodiť", "class": "word"}]'

  writeLines(practice, con = paste0(
    "/var/www/html/sk", folder_num,

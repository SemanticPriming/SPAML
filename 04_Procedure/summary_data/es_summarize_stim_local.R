...
# original word lists
es_words <- import("./04_Procedure/es/es_words.csv")
...
# merge with old data ----
  ...
  list_es_data <- lapply(list.files(path = "./04_Procedure/summary_data",
...
# generate summary chart for shiny ----
  write.csv(es_merged, "./04_Procedure/summary_data/es_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  write.csv(p_lab, "./04_Procedure/summary_data/es_participants.csv", row.names = F)

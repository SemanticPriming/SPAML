...
# original word lists
it_words <- import("./04_Procedure/it/it_words.csv")

# collected data
it_data_all <-
  bind_rows(processData("./04_Procedure/it/data/data.sqlite") %>%
...
# Clean Up ----------------------------------------------------------------
  ...
  number_trials <- it_data_all %>% #data frame
...
# figure out trial type ----
  ...
  it_real_wide <- it_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                    "class.x", "correct.x", "trial_code.x",
                                    "duration.y", "word.y", "class.y", "correct.y",
                                    "Z_RT.y", "keep.y", "keep_participant.y",
                                    "ended_on.x", "ended_on.y")]
  # good names
  colnames(it_real_wide) <- c("unique_trial", "observation", "cue_word",
  ...
  it_focus <- subset(it_real_wide, target_type == "word" & cue_type == "word")
  it_focus$word_combo <- paste0(it_focus$cue_word, it_focus$target_word)

  # add if it's related or unrelated
  it_words$word_combo <- paste0(it_words$it_cue, it_words$it_target)
  it_focus <- merge(it_focus, it_words[ , c("type", "word_combo")],
                    by = "word_combo", all.x = T)

  ### HERE YOU WILL TURN ON ###
  # subset out NAs at some point they will be practice trials
  it_focus <- subset(it_focus, !is.na(type))

  # calculate the total N versus timeout N
  it_num_trials <- it_focus %>%
    group_by(word_combo) %>%
  ...
  ### HERE YOU WILL TURN ON ###
  it_focus <- subset(it_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
  it_Z <- subset(it_focus, target_correct == TRUE)
  it_Z <- subset(it_Z, keep_trial == "keep")
...
# merge with complete stimuli list ----
  it_merged <- merge(it_words, it_Z_summary,
                     by = "word_combo", all.x = T)

  it_merged <- merge(it_merged, it_num_trials,
                     by = "word_combo", all.x = T)

# merge with old data ----
  # pull in other information from previous weeks
  list_it_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "it_summary_[0-9].*.csv", full.names = T),
                         import)
  it_summaries <- bind_rows(list_it_data, it_merged)
  it_merged <- it_summaries %>%
    select(-any_of(c("done_both", "done_totalN", "done"))) %>%
    group_by(word_combo, it_cue, it_target, type, cue_type,
             target_type, it_cosine) %>%
  ...
  # are we done? ----
  it_merged$done_both <- (it_merged$target_answeredN >= 50 & it_merged$SE_Z <= .09) | it_merged$target_answeredN >= 320
  it_merged$done_totalN <- it_merged$target_answeredN >= 50
...
# generate summary chart for shiny ----
  write.csv(it_merged, "./04_Procedure/summary_data/it_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  p_lab <- it_data_all[it_data_all$observation %in% p_end, ]
  ...
  # pull in other information from previous weeks
  list_it_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "it_participants_[0-9].*.csv", full.names = T),
                         import)
  ...
  if (nrow(p_lab) > 0){
    ...
    p_lab <- unique(bind_rows(list_it_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/it_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(it_use[it_use$cue_type == "nonword" &
                    it_use$target_type == "nonword", ]) >= 150){

      temp <- subset(it_use,
                     it_use$cue_type == "nonword" &
                       it_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- it_use[it_use$cue_type == "nonword" &
                           it_use$target_type == "nonword", ]
      temp <- subset(it_sample,
                     it_use$cue_type == "nonword" &
                       it_use$target_type == "nonword")
    ...
    if (nrow(it_use[(it_use$cue_type == "nonword" &
                     it_use$target_type == "word") |
                    (it_use$cue_type == "word" &
                     it_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- it_use[(it_use$cue_type == "nonword" &
                                it_use$target_type == "word") |
                               (it_use$cue_type == "word" &
                                  it_use$target_type == "nonword"), ]
      temp <- subset(it_sample,
    ...
    if (nrow(it_use[it_use$type == "related" , ]) >= 75){

      temp <- subset(it_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- it_use[it_use$type == "related", ]
      temp <- subset(it_sample, type == "related")
    ...
    if (nrow(it_use[it_use$type == "unrelated" , ]) >= 75){

      temp <- subset(it_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- it_use[it_use$type == "unrelated", ]
      temp <- subset(it_sample, type == "unrelated")

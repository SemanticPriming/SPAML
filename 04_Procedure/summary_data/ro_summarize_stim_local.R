...
# original word lists
ro_words <- import("./04_Procedure/ro/ro_words.csv")

# collected data
ro_data_all <-
  bind_rows(processData("./04_Procedure/ro/data/data.sqlite") %>%
...
# Clean Up ----------------------------------------------------------------
  ...
  number_trials <- ro_data_all %>% #data frame
...
# grab only real trials ----
  real_trials <- ro_data_all %>% #data frame
    filter(sender == "Stimulus Real") %>%  #filter out only the real stimuli
    select(observation, fix_sender, response, response_action, ended_on, duration,
           colnames(ro_data_all)[grep("^time", colnames(ro_data_all))],
...
# figure out trial type ----
  ...
  ro_real_wide <- ro_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                    "class.x", "correct.x", "trial_code.x",
                                    "duration.y", "word.y", "class.y", "correct.y",
                                    "Z_RT.y", "keep.y", "keep_participant.y",
                                    "ended_on.x", "ended_on.y")]
  # good names
  colnames(ro_real_wide) <- c("unique_trial", "observation", "cue_word",
  ...
  ro_focus <- subset(ro_real_wide, target_type == "word" & cue_type == "word")
  ro_focus$word_combo <- paste0(ro_focus$cue_word, ro_focus$target_word)

  # add if it's related or unrelated
  ro_words$word_combo <- paste0(ro_words$ro_cue, ro_words$ro_target)
  ro_focus <- merge(ro_focus, ro_words[ , c("type", "word_combo")],
                    by = "word_combo", all.x = T)

  ### HERE YOU WILL TURN ON ###
  # subset out NAs at some point they will be practice trials
  ro_focus <- subset(ro_focus, !is.na(type))

  # calculate the total N versus timeout N
  ro_num_trials <- ro_focus %>%
    group_by(word_combo) %>%
  ...
  ### HERE YOU WILL TURN ON ###
  ro_focus <- subset(ro_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
  ro_Z <- subset(ro_focus, target_correct == TRUE)
  ro_Z <- subset(ro_Z, keep_trial == "keep")
...
# merge with complete stimuli list ----
  ro_merged <- merge(ro_words, ro_Z_summary,
                     by = "word_combo", all.x = T)

  ro_merged <- merge(ro_merged, ro_num_trials,
                     by = "word_combo", all.x = T)

# merge with old data ----
  # pull in other information from previous weeks
  list_ro_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "ro_summary_[0-9].*.csv", full.names = T),
                         import)
  ro_summaries <- bind_rows(list_ro_data, ro_merged)
  ro_merged <- ro_summaries %>%
    select(-any_of(c("done_both", "done_totalN", "done"))) %>%
    group_by(word_combo, ro_cue, ro_target, type, cue_type,
             target_type, ro_cosine) %>%
  ...
  # are we done? ----
  ro_merged$done_both <- (ro_merged$target_answeredN >= 50 & ro_merged$SE_Z <= .09) | ro_merged$target_answeredN >= 320
  ro_merged$done_totalN <- ro_merged$target_answeredN >= 50
  ro_merged$done <- ro_merged$sampleN >= 50

# use data ----
  ro_use <- subset(ro_merged, is.na(done_totalN) | done_totalN == FALSE)
  ro_sample <- subset(ro_merged, done_totalN == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
  write.csv(ro_merged, "./04_Procedure/summary_data/ro_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  p_lab <- ro_data_all[ro_data_all$observation %in% p_end, ]
  ...
  # pull in other information from previous weeks
  list_ro_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "ro_participants_[0-9].*.csv", full.names = T),
                         import)
  list_ro_data <- lapply(list_ro_data, function(df) dplyr::mutate_at(df, vars(matches("url_lab")), as.character))
  list_ro_data <- lapply(list_ro_data, function(df) dplyr::mutate_at(df, vars(matches("url_special_code")), as.character))
  list_ro_data <- list_ro_data[lapply(list_ro_data, nrow) > 0]

  if (nrow(p_lab) > 0){
    if (length(list_ro_data) > 0){
      p_lab <- unique(bind_rows(bind_rows(list_ro_data) %>%
    ...
    p_lab <- unique(bind_rows(list_ro_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/ro_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(ro_use[ro_use$cue_type == "nonword" &
                    ro_use$target_type == "nonword", ]) >= 150){

      temp <- subset(ro_use,
                     ro_use$cue_type == "nonword" &
                       ro_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- ro_use[ro_use$cue_type == "nonword" &
                           ro_use$target_type == "nonword", ]
      temp <- subset(ro_sample,
                     ro_use$cue_type == "nonword" &
                       ro_use$target_type == "nonword")
    ...
    if (nrow(ro_use[(ro_use$cue_type == "nonword" &
                     ro_use$target_type == "word") |
                    (ro_use$cue_type == "word" &
                     ro_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- ro_use[(ro_use$cue_type == "nonword" &
                                ro_use$target_type == "word") |
                               (ro_use$cue_type == "word" &
                                  ro_use$target_type == "nonword"), ]
      temp <- subset(ro_sample,
    ...
    if (nrow(ro_use[ro_use$type == "related" , ]) >= 75){

      temp <- subset(ro_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- ro_use[ro_use$type == "related", ]
      temp <- subset(ro_sample, type == "related")
    ...
    if (nrow(ro_use[ro_use$type == "unrelated" , ]) >= 75){

      temp <- subset(ro_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- ro_use[ro_use$type == "unrelated", ]
      temp <- subset(ro_sample, type == "unrelated")

...
# original word lists
ja_words <- import("./04_Procedure_real/ja/ja_words.csv")

# collected data
ja_data_all <-
        list(processData("./04_Procedure/ja/data/data.sqlite") %>%
...
number_trials <- ja_data_all %>% #data frame
...
real_trials <- ja_data_all %>% #data frame
  ...
  select(observation, fix_sender, response, response_action, ended_on, duration,
         colnames(ja_data_all)[grep("^time", colnames(ja_data_all))],
...
ja_focus$word_combo <- paste0(ja_focus$cue_word, ja_focus$target_word)

# add if it's related or unrelated
ja_words$word_combo <- paste0(ja_words$ja_cue, ja_words$ja_target)
ja_focus <- merge(ja_focus, ja_words[ , c("type", "word_combo")],
...
ja_Z <- subset(ja_focus, target_correct == TRUE)
ja_Z <- subset(ja_Z, keep_trial == "keep")
...
list_ja_data <- lapply(list.files(path = "./04_Procedure_real/summary_data",
                                  pattern = "ja_summary_[0-9].*.csv", full.names = T),
                       import)
...
write.csv(ja_merged, "./04_Procedure_real/summary_data/ja_summary.csv", row.names = F)
...
list_ja_data <- list_ja_data[lapply(list_ja_data, nrow) > 0]

if (nrow(p_lab) > 0){
  if (length(list_ja_data) > 0){
...
write.csv(p_lab, "./04_Procedure_real/summary_data/ja_participants.csv", row.names = F)
...
# loop over the number of random folders you need
  ...
  temp <- subset(ja_merged,
                 ja_merged$cue_type == "nonword" &
                   ja_merged$target_type == "nonword")
  # shuffle the words
  ...
  practice <- '[
  {"word": "枢機卿", "class": "nonword"},
  {"word": "ゴルフ", "class": "word"},
  {"word": "ローン", "class": "word"},
  {"word": "じょ", "class": "nonword"},
  {"word": "ツナップ", "class": "nonword"},
  {"word": "匿名", "class": "word"},
  {"word": "ボネ", "class": "nonword"},
  {"word": "三", "class": "word"},
  {"word": "ンー", "class": "nonword"},
  {"word": "最後に", "class": "word"}]'
  ...
  writeLines(practice, con = paste0(
    "./04_Procedure_real/ja", folder_num,
...
if (adaptive == TRUE){
  ...
  for (i in 1:number_folders){
    ...
    # eight blocks of 100 trials = 800 trials = 400 pairs or 8 blocks of 50
    # 150 non word non word = 300 trials
    if (nrow(ja_use[ja_use$cue_type == "nonword" &
                    ja_use$target_type == "nonword", ]) >= 150){

      temp <- subset(ja_use,
                     ja_use$cue_type == "nonword" &
                       ja_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- ja_use[ja_use$cue_type == "nonword" &
                           ja_use$target_type == "nonword", ]
      temp <- subset(ja_sample,
                     ja_use$cue_type == "nonword" &
                       ja_use$target_type == "nonword")
    ...
    # 100 non word non word = 200 trials
    if (nrow(ja_use[(ja_use$cue_type == "nonword" &
                     ja_use$target_type == "word") |
                    (ja_use$cue_type == "word" &
                     ja_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- ja_use[(ja_use$cue_type == "nonword" &
                                ja_use$target_type == "word") |
                               (ja_use$cue_type == "word" &
                                  ja_use$target_type == "nonword"), ]
      temp <- subset(ja_sample,
    ...
    # 75 related pairs = 150 trials
    if (nrow(ja_use[ja_use$type == "related" , ]) >= 75){

      temp <- subset(ja_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- ja_use[ja_use$type == "related", ]
      temp <- subset(ja_sample, type == "related")
    ...
    # 75 unrelated pairs = 150 trials
    if (nrow(ja_use[ja_use$type == "unrelated" , ]) >= 75){

      temp <- subset(ja_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- ja_use[ja_use$type == "unrelated", ]
      temp <- subset(ja_sample, type == "unrelated")

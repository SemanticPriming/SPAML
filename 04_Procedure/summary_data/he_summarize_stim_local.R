...
# original word lists
he_words <- import("./04_Procedure/he/he_words.csv")

# collected data
he_data_all <-
  bind_rows(processData("./04_Procedure/he/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_he_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "he_summary_[0-9].*.csv", full.names = T),
                         import)
  he_summaries <- bind_rows(list_he_data, he_merged)
  he_merged <- he_summaries %>%
    select(-any_of(c("done_both", "done_totalN", "done"))) %>%
    group_by(word_combo, he_cue, he_target, type, cue_type,
             target_type, he_cosine) %>%
...
# use data ----
  he_use <- subset(he_merged, is.na(done) | done == FALSE)
  he_sample <- subset(he_merged, done == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
  write.csv(he_merged, "./04_Procedure/summary_data/he_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  # pull in other information from previous weeks
  list_he_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "he_participants_[0-9].*.csv", full.names = T),
                         import)
  list_he_data <- lapply(list_he_data, function(df) dplyr::mutate_at(df, vars(matches("url_lab")), as.character))
  list_he_data <- lapply(list_he_data, function(df) dplyr::mutate_at(df, vars(matches("url_special_code")), as.character))
  list_he_data <- list_he_data[lapply(list_he_data, nrow) > 0]
  
  if (nrow(p_lab) > 0){
    if (length(list_he_data) > 0){
      p_lab <- unique(bind_rows(bind_rows(list_he_data) %>%
    ...
    p_lab <- unique(bind_rows(list_he_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/he_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(he_use[he_use$cue_type == "nonword" &
                    he_use$target_type == "nonword", ]) >= 150){

      temp <- subset(he_use,
                     he_use$cue_type == "nonword" &
                       he_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- he_use[he_use$cue_type == "nonword" &
                           he_use$target_type == "nonword", ]
      temp <- subset(he_sample,
                     he_use$cue_type == "nonword" &
                       he_use$target_type == "nonword")
    ...
    if (nrow(he_use[(he_use$cue_type == "nonword" &
                     he_use$target_type == "word") |
                    (he_use$cue_type == "word" &
                     he_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- he_use[(he_use$cue_type == "nonword" &
                                he_use$target_type == "word") |
                               (he_use$cue_type == "word" &
                                  he_use$target_type == "nonword"), ]
      temp <- subset(he_sample,
    ...
    if (nrow(he_use[he_use$type == "related" , ]) >= 75){

      temp <- subset(he_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- he_use[he_use$type == "related", ]
      temp <- subset(he_sample, type == "related")
    ...
    if (nrow(he_use[he_use$type == "unrelated" , ]) >= 75){

      temp <- subset(he_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- he_use[he_use$type == "unrelated", ]
      temp <- subset(he_sample, type == "unrelated")

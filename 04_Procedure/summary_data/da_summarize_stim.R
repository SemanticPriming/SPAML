...
# original word lists
da_words <- import("/var/www/html/da/da_words.csv")
...
number_trials <- da_data_all %>% #data frame
...
real_trials <- da_data_all %>% #data frame
  filter(sender == "Stimulus Real") %>%  #filter out only the real stimuli
  select(observation, sender_id, response, response_action, ended_on, duration,
         colnames(da_data_all)[grep("^time", colnames(da_data_all))], 
...
da_real_wide <- da_real_wide[ , c("unique_trial", "observation.x", "word.x", 
                                  "class.x", "correct.x", "trial_code.x", 
                                  "duration.y", "word.y", "class.y", "correct.y", 
                                  "Z_RT.y", "keep.y", "keep_participant.y", 
                                  "ended_on.x", "ended_on.y")]
# good names
colnames(da_real_wide) <- c("unique_trial", "observation", "cue_word", 
...
da_focus <- subset(da_real_wide, target_type == "word" & cue_type == "word")
da_focus$word_combo <- paste0(da_focus$cue_word, da_focus$target_word)

# add if it's related or unrelated
da_words$word_combo <- paste0(da_words$da_cue, da_words$da_target)
da_focus <- merge(da_focus, da_words[ , c("type", "word_combo")], 
                  by = "word_combo", all.x = T)

### HERE YOU WILL TURN ON ###
# subset out NAs at some point they will be practice trials
da_focus <- subset(da_focus, !is.na(type))

# calculate the total N versus timeout N
da_num_trials <- da_focus %>% 
  group_by(word_combo) %>% 
...
### HERE YOU WILL TURN ON ###
da_focus <- subset(da_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
da_Z <- subset(da_focus, target_correct == TRUE)
da_Z <- subset(da_Z, keep_trial == "keep")
...
da_merged <- merge(da_words, da_Z_summary, 
                   by = "word_combo", all.x = T)

da_merged <- merge(da_merged, da_num_trials, 
                   by = "word_combo", all.x = T)

# merge with old data ----
# pull in other information from previous weeks
list_da_data <- lapply(list.files(path = "/var/www/html/summary_data", 
                                  pattern = "da_summary_[0-9].*.csv", full.names = T),
                       import)
da_summaries <- bind_rows(list_da_data, da_merged)
da_merged <- da_summaries %>%
  select(-any_of(c("done_both", "done_totalN", "done"))) %>% 
  group_by(word_combo, da_cue, da_target, type, cue_type, 
           target_type, da_cosine) %>% 
...
# are we done? ----
da_merged$done_both <- (da_merged$target_answeredN >= 50 & da_merged$SE_Z <= .09) | da_merged$target_answeredN >= 320
da_merged$done_totalN <- da_merged$target_answeredN >= 50
da_merged$done <- da_merged$sampleN >= 50

# use data ----
da_use <- subset(da_merged, is.na(done) | done == FALSE)
da_sample <- subset(da_merged, done == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
write.csv(da_merged, "/var/www/html/summary_data/da_summary.csv", row.names = F)
...
p_lab <- da_data_all[da_data_all$observation %in% p_end, ]
...
write.csv(p_lab, "/var/www/html/summary_data/da_participants.csv", row.names = F)
...
for (i in 1:number_folders){
  ...
  if (nrow(da_use[da_use$cue_type == "nonword" &
                  da_use$target_type == "nonword", ]) >= 150){
    
    temp <- subset(da_use, 
                   da_use$cue_type == "nonword" &
                     da_use$target_type == "nonword")
    nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]
    
  }else{
    
    nonwords <- da_use[da_use$cue_type == "nonword" &
                         da_use$target_type == "nonword", ]
    temp <- subset(da_sample, 
                   da_use$cue_type == "nonword" &
                     da_use$target_type == "nonword")
  ...
  if (nrow(da_use[(da_use$cue_type == "nonword" &
                   da_use$target_type == "word") | 
                  (da_use$cue_type == "word" & 
                   da_use$target_type == "nonword"), ]) >= 100){
    ...
    nonwords_mix <- da_use[(da_use$cue_type == "nonword" &
                              da_use$target_type == "word") | 
                             (da_use$cue_type == "word" & 
                                da_use$target_type == "nonword"), ]
    temp <- subset(da_sample, 
  ...
  if (nrow(da_use[da_use$type == "related" , ]) >= 75){
    
    temp <- subset(da_use, type == "related")
    related <- temp[sample(1:nrow(temp), 75, replace = F), ]
    
  }else{
    
    related <- da_use[da_use$type == "related", ]
    temp <- subset(da_sample, type == "related")
  ...
  if (nrow(da_use[da_use$type == "unrelated" , ]) >= 75){
    
    temp <- subset(da_use, type == "unrelated")
    unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]
    
  }else{
    
    unrelated <- da_use[da_use$type == "unrelated", ]
    temp <- subset(da_sample, type == "unrelated")

...
pl_data_all <-
  bind_rows(processData("/var/www/html/pl/data/data.sqlite") %>%
            ...
            processData("/var/www/html/pl1/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab)),
            processData("/var/www/html/pl3/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab))) %>% unique()

# delete stuff before we started
pl_data_all <- pl_data_all %>%
  filter(timestamp > as.POSIXct("2022-12-16"))
...
list_pl_data <- lapply(list.files(path = "/var/www/html/summary_data",
...
write.csv(pl_merged, "/var/www/html/summary_data/pl_summary.csv", row.names = F)
...
write.csv(p_lab, "/var/www/html/summary_data/pl_participants.csv", row.names = F)

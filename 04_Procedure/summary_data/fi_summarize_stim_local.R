...
# original word lists
fi_words <- import("./04_Procedure/fi/fi_words.csv")

# collected data
fi_data_all <-
  bind_rows(processData("./04_Procedure/fi/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_fi_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "fi_summary_[0-9].*.csv", full.names = T),
                         import)
  fi_summaries <- bind_rows(list_fi_data, fi_merged)
  fi_merged <- fi_summaries %>%
    select(-any_of(c("done_both", "done_totalN", "done"))) %>%
    group_by(word_combo, fi_cue, fi_target, type, cue_type,
             target_type, fi_cosine) %>%
...
# generate summary chart for shiny ----
  write.csv(fi_merged, "./04_Procedure/summary_data/fi_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  # pull in other information from previous weeks
  list_fi_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "fi_participants_[0-9].*.csv", full.names = T),
                         import)
  ...
  if (nrow(p_lab) > 0){
    if (length(list_fi_data) > 0){
      p_lab <- unique(bind_rows(bind_rows(list_fi_data) %>%
    ...
    p_lab <- unique(bind_rows(list_fi_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/fi_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(fi_use[fi_use$cue_type == "nonword" &
                    fi_use$target_type == "nonword", ]) >= 150){

      temp <- subset(fi_use,
                     fi_use$cue_type == "nonword" &
                       fi_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- fi_use[fi_use$cue_type == "nonword" &
                           fi_use$target_type == "nonword", ]
      temp <- subset(fi_sample,
                     fi_use$cue_type == "nonword" &
                       fi_use$target_type == "nonword")
    ...
    if (nrow(fi_use[(fi_use$cue_type == "nonword" &
                     fi_use$target_type == "word") |
                    (fi_use$cue_type == "word" &
                     fi_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- fi_use[(fi_use$cue_type == "nonword" &
                                fi_use$target_type == "word") |
                               (fi_use$cue_type == "word" &
                                  fi_use$target_type == "nonword"), ]
      temp <- subset(fi_sample,
    ...
    if (nrow(fi_use[fi_use$type == "related" , ]) >= 75){

      temp <- subset(fi_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- fi_use[fi_use$type == "related", ]
      temp <- subset(fi_sample, type == "related")
    ...
    if (nrow(fi_use[fi_use$type == "unrelated" , ]) >= 75){

      temp <- subset(fi_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- fi_use[fi_use$type == "unrelated", ]
      temp <- subset(fi_sample, type == "unrelated")
    ...
    practice <- '[
  {"word": "makuuhuone", "class": "nonword"},
  {"word": "huulet", "class": "word"},
  {"word": "terva", "class": "word"},
  {"word": "loistava", "class": "nonword"},
  {"word": "enemsyys", "class": "nonword"},
  {"word": "lahja", "class": "word"},
  {"word": "vyyte", "class": "nonword"},
  {"word": "ammuttu", "class": "word"},
  {"word": "emapa", "class": "nonword"},
  {"word": "saapas", "class": "word"}]'

    writeLines(practice, con = paste0(
      "./04_Procedure/fi", folder_num,

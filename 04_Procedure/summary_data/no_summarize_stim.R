...
# original word lists
no_words <- import("/var/www/html/no/no_words.csv")

# collected data
no_data_all <-
  bind_rows(processData("/var/www/html/no/data/data.sqlite") %>%
              mutate(url_lab = as.character(url_lab))) %>% unique()

# delete stuff before we started
no_data_all <- no_data_all %>%
  filter(timestamp > as.POSIXct("2023-01-20"))
...
number_trials <- no_data_all %>% #data frame
...
real_trials <- no_data_all %>% #data frame
  filter(sender == "Stimulus Real") %>%  #filter out only the real stimuli
  select(observation, fix_sender, response, response_action, ended_on, duration,
         colnames(no_data_all)[grep("^time", colnames(no_data_all))],
...
no_real_wide <- no_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                  "class.x", "correct.x", "trial_code.x",
                                  "duration.y", "word.y", "class.y", "correct.y",
                                  "Z_RT.y", "keep.y", "keep_participant.y",
                                  "ended_on.x", "ended_on.y")]
# good names
colnames(no_real_wide) <- c("unique_trial", "observation", "cue_word",
...
no_focus <- subset(no_real_wide, target_type == "word" & cue_type == "word")
no_focus$word_combo <- paste0(no_focus$cue_word, no_focus$target_word)

# add if it's related or unrelated
no_words$word_combo <- paste0(no_words$no_cue, no_words$no_target)
no_focus <- merge(no_focus, no_words[ , c("type", "word_combo")],
                  by = "word_combo", all.x = T)

### HERE YOU WILL TURN ON ###
# subset out NAs at some point they will be practice trials
no_focus <- subset(no_focus, !is.na(type))

# calculate the total N versus timeout N
no_num_trials <- no_focus %>%
  group_by(word_combo) %>%
...
### HERE YOU WILL TURN ON ###
no_focus <- subset(no_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
no_Z <- subset(no_focus, target_correct == TRUE)
no_Z <- subset(no_Z, keep_trial == "keep")
...
no_merged <- merge(no_words, no_Z_summary,
                   by = "word_combo", all.x = T)

no_merged <- merge(no_merged, no_num_trials,
                   by = "word_combo", all.x = T)

# merge with old data ----
# pull in other information from previous weeks
list_no_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "no_summary_[0-9].*.csv", full.names = T),
                       import)
no_summaries <- bind_rows(list_no_data, no_merged)
no_merged <- no_summaries %>%
  select(-any_of(c("done_both", "done_totalN", "done"))) %>%
  group_by(word_combo, no_cue, no_target, type, cue_type,
           target_type, no_cosine) %>%
...
# are we done? ----
no_merged$done_both <- (no_merged$target_answeredN >= 50 & no_merged$SE_Z <= .09) | no_merged$target_answeredN >= 320
no_merged$done_totalN <- no_merged$target_answeredN >= 50
no_merged$done <- no_merged$sampleN >= 50

# use data ----
no_use <- subset(no_merged, is.na(done_totalN) | done_totalN == FALSE)
no_sample <- subset(no_merged, done_totalN == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
write.csv(no_merged, "/var/www/html/summary_data/no_summary.csv", row.names = F)
...
p_lab <- no_data_all[no_data_all$observation %in% p_end, ]
...
# pull in other information from previous weeks
list_no_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "no_participants_[0-9].*.csv", full.names = T),
                       import)
...
if (nrow(p_lab) > 0){
  ...
  p_lab <- unique(bind_rows(list_no_data))
}

write.csv(p_lab, "/var/www/html/summary_data/no_participants.csv", row.names = F)
...
for (i in 1:number_folders){
  ...
  if (nrow(no_use[no_use$cue_type == "nonword" &
                  no_use$target_type == "nonword", ]) >= 150){

    temp <- subset(no_use,
                   no_use$cue_type == "nonword" &
                     no_use$target_type == "nonword")
    nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

  }else{

    nonwords <- no_use[no_use$cue_type == "nonword" &
                         no_use$target_type == "nonword", ]
    temp <- subset(no_sample,
                   no_use$cue_type == "nonword" &
                     no_use$target_type == "nonword")
  ...
  if (nrow(no_use[(no_use$cue_type == "nonword" &
                   no_use$target_type == "word") |
                  (no_use$cue_type == "word" &
                   no_use$target_type == "nonword"), ]) >= 100){
    ...
    nonwords_mix <- no_use[(no_use$cue_type == "nonword" &
                              no_use$target_type == "word") |
                             (no_use$cue_type == "word" &
                                no_use$target_type == "nonword"), ]
    temp <- subset(no_sample,
  ...
  if (nrow(no_use[no_use$type == "related" , ]) >= 75){

    temp <- subset(no_use, type == "related")
    related <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    related <- no_use[no_use$type == "related", ]
    temp <- subset(no_sample, type == "related")
  ...
  if (nrow(no_use[no_use$type == "unrelated" , ]) >= 75){

    temp <- subset(no_use, type == "unrelated")
    unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    unrelated <- no_use[no_use$type == "unrelated", ]
    temp <- subset(no_sample, type == "unrelated")

...
# original word lists
es_words <- import("/var/www/html/es/es_words.csv")
...
es_data_all <-
  list(processData("/var/www/html/es/data/data.sqlite") %>%
            ...
            processData("/var/www/html/es1/data/data.sqlite") %>%
              mutate_at(vars(one_of("url_lab")), as.character,
                          vars(one_of("url_special_code")), as.character),
       processData("/var/www/html/es2/data/data.sqlite") %>%
         mutate_at(vars(one_of("url_lab")), as.character,
                   vars(one_of("url_special_code")), as.character),
       processData("/var/www/html/es3/data/data.sqlite") %>%
         mutate_at(vars(one_of("url_lab")), as.character,
                   vars(one_of("url_special_code")), as.character))
...
number_trials <- es_data_all %>% #data frame
...
es_real_wide <- es_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                  "class.x", "correct.x", "trial_code.x",
                                  "duration.y", "word.y", "class.y", "correct.y",
                                  "Z_RT.y", "keep.y", "keep_participant.y",
                                  "ended_on.x", "ended_on.y")]
# good names
colnames(es_real_wide) <- c("unique_trial", "observation", "cue_word",
...
es_focus <- subset(es_real_wide, target_type == "word" & cue_type == "word")
es_focus$word_combo <- paste0(es_focus$cue_word, es_focus$target_word)

# add if it's related or unrelated
es_words$word_combo <- paste0(es_words$es_cue, es_words$es_target)
es_focus <- merge(es_focus, es_words[ , c("type", "word_combo")],
                  by = "word_combo", all.x = T)

### HERE YOU WILL TURN ON ###
# subset out NAs at some point they will be practice trials
es_focus <- subset(es_focus, !is.na(type))

# calculate the total N versus timeout N
es_num_trials <- es_focus %>%
  group_by(word_combo) %>%
...
### HERE YOU WILL TURN ON ###
es_focus <- subset(es_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
es_Z <- subset(es_focus, target_correct == TRUE)
es_Z <- subset(es_Z, keep_trial == "keep")
...
es_merged <- merge(es_words, es_Z_summary,
                   by = "word_combo", all.x = T)

es_merged <- merge(es_merged, es_num_trials,
                   by = "word_combo", all.x = T)

# merge with old data ----
# pull in other information from previous weeks
list_es_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "es_summary_[0-9].*.csv", full.names = T),
                       import)
es_summaries <- bind_rows(list_es_data, es_merged)
es_merged <- es_summaries %>%
  select(-any_of(c("done_both", "done_totalN", "done"))) %>%
  group_by(word_combo, es_cue, es_target, type, cue_type,
           target_type, es_cosine) %>%
...
# are we done? ----
es_merged$done_both <- (es_merged$target_answeredN >= 50 & es_merged$SE_Z <= .09) | es_merged$target_answeredN >= 320
es_merged$done_totalN <- es_merged$target_answeredN >= 50
...
write.csv(es_merged, "/var/www/html/summary_data/es_summary.csv", row.names = F)
...
p_lab <- es_data_all[es_data_all$observation %in% p_end, ]
...
# pull in other information from previous weeks
list_es_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "es_participants_[0-9].*.csv", full.names = T),
                       import)
...
if (nrow(p_lab) > 0){
  if (length(list_es_data) > 0){
    p_lab <- unique(bind_rows(bind_rows(list_es_data) %>%
  ...
  p_lab <- unique(bind_rows(list_es_data))
}

write.csv(p_lab, "/var/www/html/summary_data/es_participants.csv", row.names = F)
...
for (i in 1:number_folders){
  ...
  if (nrow(es_use[es_use$cue_type == "nonword" &
                  es_use$target_type == "nonword", ]) >= 150){

    temp <- subset(es_use,
                   es_use$cue_type == "nonword" &
                     es_use$target_type == "nonword")
    nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

  }else{

    nonwords <- es_use[es_use$cue_type == "nonword" &
                         es_use$target_type == "nonword", ]
    temp <- subset(es_sample,
                   es_use$cue_type == "nonword" &
                     es_use$target_type == "nonword")
  ...
  if (nrow(es_use[(es_use$cue_type == "nonword" &
                   es_use$target_type == "word") |
                  (es_use$cue_type == "word" &
                   es_use$target_type == "nonword"), ]) >= 100){
    ...
    nonwords_mix <- es_use[(es_use$cue_type == "nonword" &
                              es_use$target_type == "word") |
                             (es_use$cue_type == "word" &
                                es_use$target_type == "nonword"), ]
    temp <- subset(es_sample,
  ...
  if (nrow(es_use[es_use$type == "related" , ]) >= 75){

    temp <- subset(es_use, type == "related")
    related <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    related <- es_use[es_use$type == "related", ]
    temp <- subset(es_sample, type == "related")
  ...
  if (nrow(es_use[es_use$type == "unrelated" , ]) >= 75){

    temp <- subset(es_use, type == "unrelated")
    unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    unrelated <- es_use[es_use$type == "unrelated", ]
    temp <- subset(es_sample, type == "unrelated")
  ...
  {"word": "labios", "class": "word"},
  {"word": "alquitrán", "class": "word"},
  {"word": "slodo", "class": "nonword"},
  {"word": "nwede", "class": "nonword"},
  {"word": "regalo", "class": "word"},
  {"word": "kegthe", "class": "nonword"},
  {"word": "disparo", "class": "word"},
  {"word": "ethorar", "class": "nonword"},
  {"word": "bota", "class": "word"}]'

  writeLines(practice, con = paste0(
    "/var/www/html/es", folder_num,

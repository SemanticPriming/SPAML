...
# original word lists
zh_words <- import("./04_Procedure/zh/zh_words.csv")

# collected data
zh_data_all <-
  bind_rows(processData("./04_Procedure/zh/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_zh_data <- lapply(list.files(path = "./04_Procedure/summary_data",
...
# generate summary chart for shiny ----
  write.csv(zh_merged, "./04_Procedure/summary_data/zh_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  write.csv(p_lab, "./04_Procedure/summary_data/zh_participants.csv", row.names = F)

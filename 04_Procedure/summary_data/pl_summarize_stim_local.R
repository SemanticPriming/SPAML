...
# original word lists
pl_words <- import("./04_Procedure/pl/pl_words.csv")

# collected data
pl_data_all <-
  bind_rows(processData("./04_Procedure/pl/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_pl_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "pl_summary_[0-9].*.csv", full.names = T),
                         import)
  pl_summaries <- bind_rows(list_pl_data, pl_merged)
  pl_merged <- pl_summaries %>%
    select(-any_of(c("done_both", "done_totalN", "done"))) %>%
    group_by(word_combo, pl_cue, pl_target, type, cue_type,
             target_type, pl_cosine) %>%
...
# generate summary chart for shiny ----
  write.csv(pl_merged, "./04_Procedure/summary_data/pl_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  # pull in other information from previous weeks
  list_pl_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "pl_participants_[0-9].*.csv", full.names = T),
                         import)
  ...
  if (nrow(p_lab) > 0){
    ...
    p_lab <- unique(bind_rows(list_pl_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/pl_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(pl_use[pl_use$cue_type == "nonword" &
                    pl_use$target_type == "nonword", ]) >= 150){

      temp <- subset(pl_use,
                     pl_use$cue_type == "nonword" &
                       pl_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- pl_use[pl_use$cue_type == "nonword" &
                           pl_use$target_type == "nonword", ]
      temp <- subset(pl_sample,
                     pl_use$cue_type == "nonword" &
                       pl_use$target_type == "nonword")
    ...
    if (nrow(pl_use[(pl_use$cue_type == "nonword" &
                     pl_use$target_type == "word") |
                    (pl_use$cue_type == "word" &
                     pl_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- pl_use[(pl_use$cue_type == "nonword" &
                                pl_use$target_type == "word") |
                               (pl_use$cue_type == "word" &
                                  pl_use$target_type == "nonword"), ]
      temp <- subset(pl_sample,
    ...
    if (nrow(pl_use[pl_use$type == "related" , ]) >= 75){

      temp <- subset(pl_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- pl_use[pl_use$type == "related", ]
      temp <- subset(pl_sample, type == "related")
    ...
    if (nrow(pl_use[pl_use$type == "unrelated" , ]) >= 75){

      temp <- subset(pl_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- pl_use[pl_use$type == "unrelated", ]
      temp <- subset(pl_sample, type == "unrelated")
    ...
    practice <- '[
  {"word": "spurzw", "class": "nonword"},
  {"word": "usta", "class": "word"},
  {"word": "smoła", "class": "word"},
  {"word": "badajesz", "class": "nonword"},
  {"word": "stroczony", "class": "nonword"},
  {"word": "dał", "class": "word"},
  {"word": "polter", "class": "nonword"},
  {"word": "strzał", "class": "word"},
  {"word": "swostatnie", "class": "nonword"},
  {"word": "uruchomić", "class": "word"}]'

    writeLines(practice, con = paste0(
      "./04_Procedure/pl", folder_num,

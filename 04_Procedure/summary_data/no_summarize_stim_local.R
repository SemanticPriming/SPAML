...
# original word lists
no_words <- import("./04_Procedure/no/no_words.csv")

# collected data
no_data_all <-
  bind_rows(processData("./04_Procedure/no/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_no_data <- lapply(list.files(path = "./04_Procedure/summary_data",
...
# generate summary chart for shiny ----
  write.csv(no_merged, "./04_Procedure/summary_data/no_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  write.csv(p_lab, "./04_Procedure/summary_data/no_participants.csv", row.names = F)

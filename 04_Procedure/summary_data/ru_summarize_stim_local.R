...
# collected data
ru_data_all <-
  list(processData("./04_Procedure/ru/data/data.sqlite") %>%
...
list_ru_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                  pattern = "ru_summary_[0-9].*.csv", full.names = T),
                       import)
...
# pull in other information from previous weeks
list_ru_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                  pattern = "ru_participants_[0-9].*.csv", full.names = T),
                       import)

list_ru_data <- lapply(list_ru_data, function(df) dplyr::mutate_at(df, vars(matches("url_lab")), as.character))
list_ru_data <- lapply(list_ru_data, function(df) dplyr::mutate_at(df, vars(matches("url_special_code")), as.character))
list_ru_data <- list_ru_data[lapply(list_ru_data, nrow) > 0]

if (nrow(p_lab) > 0){
  if (length(list_ru_data) > 0){
  p_lab <- unique(bind_rows(bind_rows(list_ru_data) %>%

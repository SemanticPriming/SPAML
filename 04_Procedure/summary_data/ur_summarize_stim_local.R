...
# original word lists
ur_words <- import("./04_Procedure/ur/ur_words.csv")

# collected data
ur_data_all <-
  bind_rows(processData("./04_Procedure/ur/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_ur_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "ur_summary_[0-9].*.csv", full.names = T),
                         import)
  ur_summaries <- bind_rows(list_ur_data, ur_merged)
  ur_merged <- ur_summaries %>%
    select(-any_of(c("done_both", "done_totalN", "done"))) %>%
    group_by(word_combo, ur_cue, ur_target, type, cue_type,
             target_type, ur_cosine) %>%
...
# use data ----
  ur_use <- subset(ur_merged, is.na(done_totalN) | done_totalN == FALSE)
  ur_sample <- subset(ur_merged, done_totalN == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
  write.csv(ur_merged, "./04_Procedure/summary_data/ur_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  # pull in other information from previous weeks
  list_ur_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "ur_participants_[0-9].*.csv", full.names = T),
                         import)
  ...
  if (nrow(p_lab) > 0){
    if (length(list_ur_data) > 0){
      p_lab <- unique(bind_rows(bind_rows(list_ur_data) %>%
    ...
    p_lab <- unique(bind_rows(list_ur_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/ur_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(ur_use[ur_use$cue_type == "nonword" &
                    ur_use$target_type == "nonword", ]) >= 150){

      temp <- subset(ur_use,
                     ur_use$cue_type == "nonword" &
                       ur_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- ur_use[ur_use$cue_type == "nonword" &
                           ur_use$target_type == "nonword", ]
      temp <- subset(ur_sample,
                     ur_use$cue_type == "nonword" &
                       ur_use$target_type == "nonword")
    ...
    if (nrow(ur_use[(ur_use$cue_type == "nonword" &
                     ur_use$target_type == "word") |
                    (ur_use$cue_type == "word" &
                     ur_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- ur_use[(ur_use$cue_type == "nonword" &
                                ur_use$target_type == "word") |
                               (ur_use$cue_type == "word" &
                                  ur_use$target_type == "nonword"), ]
      temp <- subset(ur_sample,
    ...
    if (nrow(ur_use[ur_use$type == "related" , ]) >= 75){

      temp <- subset(ur_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- ur_use[ur_use$type == "related", ]
      temp <- subset(ur_sample, type == "related")
    ...
    if (nrow(ur_use[ur_use$type == "unrelated" , ]) >= 75){

      temp <- subset(ur_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- ur_use[ur_use$type == "unrelated", ]
      temp <- subset(ur_sample, type == "unrelated")
    ...
    practice <- '[
  {"word": "قتل", "class": "nonword"},
  {"word": "ہونٹ", "class": "word"},
  {"word": "ٹار", "class": "word"},
  {"word": "ح,ت", "class": "nonword"},
  {"word": "خنس", "class": "nonword"},
  {"word": "تحفہ", "class": "word"},
  {"word": "پپائپ", "class": "nonword"},
  {"word": "گولی مار دی", "class": "word"},
  {"word": "و,", "class": "nonword"},
  {"word": "بوٹ", "class": "word"}]'

    writeLines(practice, con = paste0(
      "./04_Procedure/ur", folder_num,

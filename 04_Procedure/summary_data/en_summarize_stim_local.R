...
# collected data
en_data_all <-
  list(processData("./04_Procedure/en/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_en_data <- lapply(list.files(path = "./04_Procedure/summary_data",
...
# generate summary chart for shiny ----
  write.csv(en_merged, "./04_Procedure/summary_data/en_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  list_en_data <- list_en_data[lapply(list_en_data, nrow) > 0]

  if (nrow(p_lab) > 0){
    if (length(list_en_data) > 0){
    ...
    p_lab <- unique(bind_rows(list_en_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/en_participants.csv", row.names = F)

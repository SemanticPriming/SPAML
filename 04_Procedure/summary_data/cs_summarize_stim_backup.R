...
# are we done? ---- 
cs_Z_summary$done_both <- (cs_Z_summary$sampleN >= 50 & cs_Z_summary$SE_Z <= .09) | cs_Z_summary$sampleN >= 320
cs_Z_summary$done <- cs_Z_summary$sampleN >= 50
...
write.csv(cs_merged, 
          paste("/var/www/html/summary_data/cs_summary_",
                Sys.time(), ".csv", sep = ""),
          row.names = F)
...
write.csv(p_lab, 
          paste("/var/www/html/summary_data/cs_participants_",
                Sys.time(), ".csv", sep = ""),
          row.names = F)
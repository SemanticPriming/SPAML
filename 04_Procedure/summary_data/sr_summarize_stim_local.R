...
# original word lists
sr_words <- import("./04_Procedure/sr/sr_words.csv")

# collected data
sr_data_all <-
  bind_rows(processData("./04_Procedure/sr/data/data.sqlite") %>%
...
# merge with old data ----
  ...
  list_sr_data <- lapply(list.files(path = "./04_Procedure/summary_data",
...
# use data ----
  sr_use <- subset(sr_merged, is.na(done) | done == FALSE)
  sr_sample <- subset(sr_merged, done == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
  write.csv(sr_merged, "./04_Procedure/summary_data/sr_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  write.csv(p_lab, "./04_Procedure/summary_data/sr_participants.csv", row.names = F)

...
# original word lists
zh_words <- import("/var/www/html/zh/zh_words.csv")
...
# add in url_lab
zh_data_all$url_lab[zh_data_all$observation %in% dup_rows$observation &
                      zh_data_all$sender_id == 1] <- 2772
...
number_trials <- zh_data_all %>% #data frame
...
zh_real_wide <- zh_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                  "class.x", "correct.x", "trial_code.x",
                                  "duration.y", "word.y", "class.y", "correct.y",
                                  "Z_RT.y", "keep.y", "keep_participant.y",
                                  "ended_on.x", "ended_on.y")]
# good names
colnames(zh_real_wide) <- c("unique_trial", "observation", "cue_word",
...
zh_focus <- subset(zh_real_wide, target_type == "word" & cue_type == "word")
zh_focus$word_combo <- paste0(zh_focus$cue_word, zh_focus$target_word)

# add if it's related or unrelated
zh_words$word_combo <- paste0(zh_words$zh_cue, zh_words$zh_target)
zh_focus <- merge(zh_focus, zh_words[ , c("type", "word_combo")],
                  by = "word_combo", all.x = T)

### HERE YOU WILL TURN ON ###
# subset out NAs at some point they will be practice trials
zh_focus <- subset(zh_focus, !is.na(type))

# calculate the total N versus timeout N
zh_num_trials <- zh_focus %>%
  group_by(word_combo) %>%
...
### HERE YOU WILL TURN ON ###
zh_focus <- subset(zh_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
zh_Z <- subset(zh_focus, target_correct == TRUE)
zh_Z <- subset(zh_Z, keep_trial == "keep")
...
list_zh_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "zh_summary_[0-9].*.csv", full.names = T),
                       import)
zh_summaries <- bind_rows(list_zh_data, zh_merged)
zh_merged <- zh_summaries %>%
  select(-any_of(c("done_both", "done_totalN", "done"))) %>%
  group_by(word_combo, zh_cue, zh_target, type, cue_type,
           target_type, zh_cosine) %>%
...
zh_merged$done_totalN <- zh_merged$target_answeredN >= 50
...
write.csv(zh_merged, "/var/www/html/summary_data/zh_summary.csv", row.names = F)
...
p_lab <- zh_data_all[zh_data_all$observation %in% p_end, ]
...
# pull in other information from previous weeks
list_zh_data <- lapply(list.files(path = "/var/www/html/summary_data",
                                  pattern = "zh_participants_[0-9].*.csv", full.names = T),
                       import)
...
if (nrow(p_lab) > 0){
  ...
  p_lab <- unique(bind_rows(list_zh_data))
}

write.csv(p_lab, "/var/www/html/summary_data/zh_participants.csv", row.names = F)
...
for (i in 1:number_folders){
  ...
  if (nrow(zh_use[zh_use$cue_type == "nonword" &
                  zh_use$target_type == "nonword", ]) >= 150){

    temp <- subset(zh_use,
                   zh_use$cue_type == "nonword" &
                     zh_use$target_type == "nonword")
    nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

  }else{

    nonwords <- zh_use[zh_use$cue_type == "nonword" &
                         zh_use$target_type == "nonword", ]
    temp <- subset(zh_sample,
                   zh_use$cue_type == "nonword" &
                     zh_use$target_type == "nonword")
  ...
  if (nrow(zh_use[(zh_use$cue_type == "nonword" &
                   zh_use$target_type == "word") |
                  (zh_use$cue_type == "word" &
                   zh_use$target_type == "nonword"), ]) >= 100){
    ...
    nonwords_mix <- zh_use[(zh_use$cue_type == "nonword" &
                              zh_use$target_type == "word") |
                             (zh_use$cue_type == "word" &
                                zh_use$target_type == "nonword"), ]
    temp <- subset(zh_sample,
  ...
  if (nrow(zh_use[zh_use$type == "related" , ]) >= 75){

    temp <- subset(zh_use, type == "related")
    related <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    related <- zh_use[zh_use$type == "related", ]
    temp <- subset(zh_sample, type == "related")
  ...
  if (nrow(zh_use[zh_use$type == "unrelated" , ]) >= 75){

    temp <- subset(zh_use, type == "unrelated")
    unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

  }else{

    unrelated <- zh_use[zh_use$type == "unrelated", ]
    temp <- subset(zh_sample, type == "unrelated")

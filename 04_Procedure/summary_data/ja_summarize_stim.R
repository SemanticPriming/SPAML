...
# original word lists
ja_words <- import("/var/www/html/ja/ja_words.csv")
...
write.csv(participant_DF %>% select(please_tell_us_your_gender, keep, url_special_code), 
          "/var/www/html/summary_data/ja_totals.csv", row.names = F)
...
# are we done? ---- 
ja_Z_summary$done <- (ja_Z_summary$sampleN >= 50 & ja_Z_summary$SE_Z <= .09) | ja_Z_summary$sampleN >= 320
...
write.csv(ja_merged, "/var/www/html/summary_data/ja_summary.csv", row.names = F)

...
# original word lists
sl_words <- import("./04_Procedure/sl/sl_words.csv")

# collected data
sl_data_all <-
  list(processData("./04_Procedure/sl/data/data.sqlite") %>%
              mutate_at(vars(one_of("url_lab")), as.character,
                          vars(one_of("url_special_code")), as.character))

  for (i in 1:length(sl_data_all)){
    sl_data_all[[i]] <- sl_data_all[[i]] %>% mutate_at(vars(one_of("url_special_code")), as.character)
  }

  sl_data_all <- bind_rows(sl_data_all) %>% unique()

# delete stuff before we started
# sl_data_all <- sl_data_all %>%
...
# Clean Up ----------------------------------------------------------------
  ...
  number_trials <- sl_data_all %>% #data frame
...
# grab only real trials ----
  real_trials <- sl_data_all %>% #data frame
    filter(sender == "Stimulus Real") %>%  #filter out only the real stimuli
    select(observation, fix_sender, response, response_action, ended_on, duration,
           colnames(sl_data_all)[grep("^time", colnames(sl_data_all))],
...
# figure out trial type ----
  ...
  sl_real_wide <- sl_real_wide[ , c("unique_trial", "observation.x", "word.x",
                                    "class.x", "correct.x", "trial_code.x",
                                    "duration.y", "word.y", "class.y", "correct.y",
                                    "Z_RT.y", "keep.y", "keep_participant.y",
                                    "ended_on.x", "ended_on.y")]
  # good names
  colnames(sl_real_wide) <- c("unique_trial", "observation", "cue_word",
  ...
  sl_focus <- subset(sl_real_wide, target_type == "word" & cue_type == "word")
  sl_focus$word_combo <- paste0(sl_focus$cue_word, sl_focus$target_word)

  # add if it's related or unrelated
  sl_words$word_combo <- paste0(sl_words$sl_cue, sl_words$sl_target)
  sl_focus <- merge(sl_focus, sl_words[ , c("type", "word_combo")],
                    by = "word_combo", all.x = T)

  ### HERE YOU WILL TURN ON ###
  # subset out NAs at some point they will be practice trials
  sl_focus <- subset(sl_focus, !is.na(type))

  # calculate the total N versus timeout N
  sl_num_trials <- sl_focus %>%
    group_by(word_combo) %>%
  ...
  ### HERE YOU WILL TURN ON ###
  sl_focus <- subset(sl_focus, keep_participant == "keep")

# only correct answers for checking stimuli counts ----
  sl_Z <- subset(sl_focus, target_correct == TRUE)
  sl_Z <- subset(sl_Z, keep_trial == "keep")
...
# merge with complete stimuli list ----
  sl_merged <- merge(sl_words, sl_Z_summary,
                     by = "word_combo", all.x = T)

  sl_merged <- merge(sl_merged, sl_num_trials,
                     by = "word_combo", all.x = T)

# merge with old data ----
  # pull in other information from previous weeks
  list_sl_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "sl_summary_[0-9].*.csv", full.names = T),
                         import)
  sl_summaries <- bind_rows(list_sl_data, sl_merged)

  # a choice here to merge digusting with disgusting
  sl_summaries$sl_cue <- gsub("digusting", "disgusting", sl_summaries$sl_cue)
  sl_summaries$word_combo <- gsub("digusting", "disgusting", sl_summaries$word_combo)
  ...
  # are we done? ----
  sl_merged$done_both <- (sl_merged$target_answeredN >= 50 & sl_merged$SE_Z <= .09) | sl_merged$target_answeredN >= 320
  sl_merged$done_totalN <- sl_merged$target_answeredN >= 50
  sl_merged$done <- sl_merged$sampleN >= 50

# use data ----
  sl_use <- subset(sl_merged, is.na(done_totalN) | done_totalN == FALSE)
  sl_sample <- subset(sl_merged, done_totalN == TRUE)

# Generate ----------------------------------------------------------------

# generate summary chart for shiny ----
  write.csv(sl_merged, "./04_Procedure/summary_data/sl_summary.csv", row.names = F)
...
# generate participant report for shiny ----
  ...
  p_lab <- sl_data_all[sl_data_all$observation %in% p_end, ]
  ...
  # pull in other information from previous weeks
  list_sl_data <- lapply(list.files(path = "./04_Procedure/summary_data",
                                    pattern = "sl_participants_[0-9].*.csv", full.names = T),
                         import)
  list_sl_data <- lapply(list_sl_data, function(df) dplyr::mutate_at(df, vars(matches("url_lab")), as.character))
  list_sl_data <- lapply(list_sl_data, function(df) dplyr::mutate_at(df, vars(matches("url_special_code")), as.character))
  list_sl_data <- list_sl_data[lapply(list_sl_data, nrow) > 0]

  if (nrow(p_lab) > 0){
    if (length(list_sl_data) > 0){
      p_lab <- unique(bind_rows(bind_rows(list_sl_data) %>%
    ...
    p_lab <- unique(bind_rows(list_sl_data))
  }

  write.csv(p_lab, "./04_Procedure/summary_data/sl_participants.csv", row.names = F)
...
# generate new stimuli ----
  ...
  for (i in 1:number_folders){
    ...
    if (nrow(sl_use[sl_use$cue_type == "nonword" &
                    sl_use$target_type == "nonword", ]) >= 150){

      temp <- subset(sl_use,
                     sl_use$cue_type == "nonword" &
                       sl_use$target_type == "nonword")
      nonwords <- temp[sample(1:nrow(temp), 150, replace = F), ]

    }else{

      nonwords <- sl_use[sl_use$cue_type == "nonword" &
                           sl_use$target_type == "nonword", ]
      temp <- subset(sl_sample,
                     sl_use$cue_type == "nonword" &
                       sl_use$target_type == "nonword")
    ...
    if (nrow(sl_use[(sl_use$cue_type == "nonword" &
                     sl_use$target_type == "word") |
                    (sl_use$cue_type == "word" &
                     sl_use$target_type == "nonword"), ]) >= 100){
      ...
      nonwords_mix <- sl_use[(sl_use$cue_type == "nonword" &
                                sl_use$target_type == "word") |
                               (sl_use$cue_type == "word" &
                                  sl_use$target_type == "nonword"), ]
      temp <- subset(sl_sample,
    ...
    if (nrow(sl_use[sl_use$type == "related" , ]) >= 75){

      temp <- subset(sl_use, type == "related")
      related <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      related <- sl_use[sl_use$type == "related", ]
      temp <- subset(sl_sample, type == "related")
    ...
    if (nrow(sl_use[sl_use$type == "unrelated" , ]) >= 75){

      temp <- subset(sl_use, type == "unrelated")
      unrelated <- temp[sample(1:nrow(temp), 75, replace = F), ]

    }else{

      unrelated <- sl_use[sl_use$type == "unrelated", ]
      temp <- subset(sl_sample, type == "unrelated")
    ...
    practice <- '[
  {"word": "vlekopter", "class": "nonword"},
  {"word": "podjetje", "class": "word"},
  {"word": "afrika", "class": "word"},
  {"word": "porožiti", "class": "nonword"},
  {"word": "sfasno", "class": "nonword"},
  {"word": "hormon", "class": "word"},
  {"word": "čag", "class": "nonword"},
  {"word": "inženir", "class": "word"},
  {"word": "virič", "class": "nonword"},
  {"word": "alibi", "class": "word"}]'

    writeLines(practice, con = paste0(
      "./04_Procedure/sl", folder_num,

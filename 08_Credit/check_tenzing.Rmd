...
- add email to the code version of the sheet
...
final_tenzing_sheet <- final_tenzing %>% 
  ...
  # fix the people who are on the sheet but not in canvas
  mutate(LastName = ifelse(is.na(LastName), Surname, LastName),
         FirstName = ifelse(is.na(FirstName), Firstname, FirstName),
         user.name = ifelse(is.na(user.name), 
                            paste(FirstName, `Middle name`, LastName, sep = " "),
                            user.name)) %>% 
  # fix credit contributions
  mutate(
  ...
  mutate(
    investigation = ifelse(is.na(investigation), FALSE, investigation),
    resources = ifelse(is.na(resources), FALSE, resources),
    software = ifelse(is.na(software), FALSE, software),
    collaboration = ifelse(is.na(collaboration), FALSE, collaboration)
  ) %>% 
  # placeholder columns
  mutate(
    completed_author = FALSE,
    `Conflict of interest` = NA
  ) %>% 
  # select final columns
  select(Tier, PSA_ID.x, FirstName, `Middle name`, LastName, user.name, 
         Conceptualization, `Data curation`, `Formal analysis`, 
         `Funding acquisition`, investigation, Methodology, `Project administration`, 
         resources, software, Supervision, Validation, Visualization, 
         `Writing - original draft`, `Writing - review & editing`, 
         collaboration, completed_author, `Email address`, `Primary affiliation`, 
         `Secondary affiliation`, Funding, `ORCID iD`, `Corresponding author?`, 
         `Conflict of interest`, user.login_id
         ) %>% 
  dplyr::rename(
    Firstname = FirstName,
    Surname = LastName,
    Investation = investigation, 
    Resourcese = resources, 
    Software = software,
    PSA_ID = PSA_ID.x
  ) %>% 
  mutate(
    Tier = ifelse(Tier == 2 | Tier == 2.1 | Tier == "new", 2, Tier),
    across(c(Conceptualization:`Writing - review & editing`), as.logical))

final_tenzing_sheet$number_credit <- apply(
  final_tenzing_sheet %>% select(Conceptualization:`Writing - review & editing`),
  1,
  sum
)
...
export(final_tenzing_sheet, "final_tenzing.xlsx", row.names = F)

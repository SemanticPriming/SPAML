...
smallstim <- finalstimuli[1:10000, 1:6]

# figure out duplicates
smallstim$dup1 <- duplicated(smallstim$en_cue)
smallstim$dup2 <- duplicated(smallstim$en_target)
...
selected <- import("finalstimuli_list.xlsx")

# use unique to grab the full set
selected_final <- subset(finalstimuli, unique %in% selected$unique)

# fill in missing english words
selected_final_NA <- subset(selected_final, is.na(en_cue))
selected_final_not <- subset(selected_final, !is.na(en_cue))

selected_final_NA <- merge(selected_final_NA, selected, by = "unique")
selected_final_NA <- selected_final_NA[ , c(223:224, 1, 4:222) ]
colnames(selected_final_NA)[1:2] <- c("en_cue", "en_target")

# put them back
selected_final <- rbind(selected_final_NA, selected_final_not)

# write this out for RR
write.csv(selected_final, "final_selected_words.csv", row.names = F)
```

## Get Missing Information

We will fill in the missing holes of the data when the procedure for selecting the words has been approved. Each language will be checked by a translator for that language before putting online, and then each of the final words will be analyzed for cosine from their vector space model. Please note that the fake words will also be assessed to ensure they are appropriate psuedowords. 

...
temp <- matchDF2_final %>%
       select(en_cue, en_target, zh_target_word_unique, zh_cue, zh_target) %>%
  mutate(trial_copy = paste0(zh_cue, zh_target, ",", zh_target),
         word_combo = paste0(zh_cue, zh_target)) %>% 
  full_join(
    import("zh_matched.csv") %>% 
      mutate(across(everything(), tolower)) %>% 
      mutate(en_cue = gsub("naïve", "naive", en_cue)), 
...
temp_nchar <- temp %>% 
      filter(is.na(zh_target_word_unique) | is.na(en_cue) | nchar(word_combo_related) < 1 | nchar(word_combo_unrelated) < 1) %>% pull(zh_target_word_unique)

temp2 <- import("../../04_Procedure/zh/zh_words.csv") %>% 
  filter(type == "unrelated") %>% 
  #filter(zh_target %in% temp_nchar) %>% 
  mutate(word_combo_unrelated = paste0(zh_cue, zh_target),
         cue_word_unrelated = zh_cue)

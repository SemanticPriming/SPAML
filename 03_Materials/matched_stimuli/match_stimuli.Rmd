...
temp <- matchDF2 %>%
       select(en_cue, en_target, br_pt_target_word_unique, br_pt_cue, br_pt_target) %>%
  mutate(trial_copy = paste0(br_pt_cue, br_pt_target, ",", br_pt_target),
         word_combo = paste0(br_pt_cue, br_pt_target)) %>% 
  full_join(
    import("br_pt_matched.csv") %>% 
      mutate(across(everything(), tolower)) %>% 
      mutate(en_cue = gsub("naïve", "naive", en_cue)), 
  ...
  full_join(
    import("br_pt_trial_unique.csv") %>% 
                        mutate(across(everything(), tolower),
                               lang = "br_pt"),
    by = c("br_pt_target_word_unique" = "target_word_unique")
  )

View(temp %>% 
      filter(is.na(br_pt_target_word_unique)))

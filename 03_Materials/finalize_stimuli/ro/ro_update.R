...
ro_patterns <- read.hyph.pat("03_materials/finalize_stimuli/hyphen/hyph-ro.tex", "ro")

# create fake words based on cue
ro_fake_cue_update <- get_fake(wordlist = c(DF$ro_cue_final, DF$ro_target_final), #possibles
                          language_hyp = ro_patterns, #hyphen rules
                          replacewords = DF$ro_cue_final) #your words

DF <- DF %>% 
  left_join(ro_fake_cue_update %>% select(original_word, replacement_word), 
            by = c("ro_cue_final" = "original_word")) %>% rename("ro_fake_cue_update" = "replacement_word")

ro_fake_target_update <- get_fake(wordlist = c(DF$ro_cue_final, DF$ro_target_final), #possibles
                                  language_hyp = ro_patterns, #hyphen rules
                                  replacewords = DF$ro_target_final) #your words

DF <- DF %>% 
  left_join(ro_fake_target_update %>% select(original_word, replacement_word), 

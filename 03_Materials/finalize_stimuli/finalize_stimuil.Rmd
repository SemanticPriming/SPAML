...
bad_trials$cs_cosine[bad_trials$cs_target == "땡땡"] <- 0
bad_trials$cs_cosine[bad_trials$cs_target == "버리다"] <- 0
...
# creates too many so sample 
cs_final <- bind_rows(good_trials_small, new_good_trials[sample(1:nrow(new_good_trials), 5000-nrow(good_trials_small), replace = F), ])
...
```{r eval = F}
# original words
# don't use the originlal trials because there are lot of weird typos
# that were clearly fixed in the translated sheet
ko <- import("ko/ko_translated.xlsx")
ko_temp <- import("ko/ko_translate.csv")
ko <- ko %>% select(colnames(ko_temp))

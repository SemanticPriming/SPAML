...
for (i in 1:nrow(DF)){
  if (!is.na(DF$ja_cue_trans[i])) {DF$ja_cue_final[i] <- DF$ja_cue_trans[i]}
  if (!is.na(DF$ja_target_trans[i])) {DF$ja_target_final[i] <- DF$ja_target_trans[i]}
}

DF$ja_target_final[DF$ja_target_final == "FALSE"] <- "間違い"
...
ja_patterns <- read.hyph.pat("03_materials/finalize_stimuli/hyphen/hyph-en-us.tex", "en")

# create fake words based on cue
ja_fake_cue_update <- get_fake(wordlist = c(DF$ja_cue_final, DF$ja_target_final), #possibles
                          language_hyp = ja_patterns, #hyphen rules
                          replacewords = DF$ja_cue_final) #your words

DF <- DF %>% 
  left_join(ja_fake_cue_update %>% select(original_word, replacement_word), 
            by = c("ja_cue_final" = "original_word")) %>% rename("ja_fake_cue_update" = "replacement_word")

ja_fake_target_update <- get_fake(wordlist = c(DF$ja_cue_final, DF$ja_target_final), #possibles
                                  language_hyp = ja_patterns, #hyphen rules
                                  replacewords = DF$ja_target_final) #your words

DF <- DF %>% 
  left_join(ja_fake_target_update %>% select(original_word, replacement_word), 
            by = c("ja_target_final" = "original_word")) %>% 
  rename("ja_fake_target_update" = "replacement_word")

export(DF, "03_materials/finalize_stimuli/ja/ja_update.xlsx")

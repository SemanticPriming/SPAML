...
DF <- import("03_materials/finalize_stimuli/hi/hi_translated.xlsx")
...
hi_patterns <- "hi"
hi_patterns <- read.hyph.pat("03_materials/finalize_stimuli/hyphen/hyph-hi.tex", "hi")

# create fake words based on cue
hi_fake_cue_update <- get_fake(wordlist = c(DF$hi_cue, DF$hi_target), #possibles
                          language_hyp = hi_patterns, #hyphen rules
                          replacewords = DF$hi_cue) #your words

DF <- DF %>%
  left_join(hi_fake_cue_update %>% select(original_word, replacement_word),
            by = c("hi_cue" = "original_word")) %>% rename("hi_fake_cue_update" = "replacement_word")

hi_fake_target_update <- get_fake(wordlist = c(DF$hi_cue, DF$hi_target), #possibles
                                  language_hyp = hi_patterns, #hyphen rules
                                  replacewords = DF$hi_target) #your words

DF <- DF %>% 
  left_join(hi_fake_target_update %>% select(original_word, replacement_word), 
            by = c("hi_target" = "original_word")) %>% 
  rename("hi_fake_target_update" = "replacement_word")

export(DF, "03_materials/finalize_stimuli/hi/hi_translated_update.xlsx")

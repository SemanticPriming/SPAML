...
autofit(flextable(subset(combined_sheet, 
                 `SPAML Registered Report (48)` > 0 && is.na(Tier), 
...
```{r}
autofit(flextable(subset(combined_sheet, (is.na(`SPAML Registered Report (48)`) | `SPAML Registered Report (48)` == 0) & !is.na(Tier) & !(PSA_ID %in% admin_team), 
                 select = c(PSA_ID, `Full Name`, `Email address`))))
```
...
```{r}
autofit(flextable(subset(combined_sheet, (is.na(`Collaborator Agreement (22)`) | `Collaborator Agreement (22)` == 0) & !is.na(Tier) & !(PSA_ID %in% admin_team), 
                 select = c(PSA_ID, `Full Name`, `Email address`))))
```

...
colnames(canvas_contributions) <- c("Student", "Canvas_ID", "PSA_ID", "Login_Email", 
                                    ...
                                    "submit_irb", "team_list", "SPAML_RR", "fall2021_pres", 
                                    "SPAML_comments", "cc_points", "cf_points", "cuc_score", 
                                    "cf_score", "cuf_score", "current_points", "final_points",
...
```{r}
autofit(flextable(subset(combined_sheet, 
                         (is.na(SPAML_RR) | SPAML_RR == 0) & 
                           !is.na(Tier) & !(PSA_ID %in% admin_team), 
                 select = c(PSA_ID, `Full Name`, `Email address`))))
```
...
```{r}
autofit(flextable(subset(combined_sheet, 
                         (is.na(collab_agree) | collab_agree == 0) & 
                           !is.na(Tier) & !(PSA_ID %in% admin_team), 
                 select = c(PSA_ID, `Full Name`, `Email address`))))
```

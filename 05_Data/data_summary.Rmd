...
```{r}
# get all the files
participant_files <- list.files("data_processing/output_data/participant_data",
...
for (i in 1:length(participant_files)){
  ...
  participant_data[[i]] <- participant_data[[i]] %>% 
    ...
    participant_data[[i]]$lang <- 
      gsub("data_processing/output_data/participant_data/|_participant_data.csv|_combo", 
           "", 
           participant_files[i])
  }
}
...
```{r}
# get all the files
prime_files <- list.files("data_processing/output_data/priming_data",
...
prime_data$lang <- gsub("data_processing/output_data/priming_data/", "", prime_data$lang)
...
participant_data %>% 
  filter(keep == "keep") %>% 
  summarize(mean_trials = mean(n_trials),
            mean_answered = mean(n_answered), 
            mean_correct = mean(p_correct), 
            mean_timing = mean(study_length/n_trials*800), 
            med_timing = median(study_length/n_trials*800))
```

## Report by Language

```{r}
participant_data$age <- as.numeric(substr(participant_data$timestamp_consent, 1, 4)) - participant_data$which_year_were_you_bor
...
lang_DF <- 
  ...
  filter(keep == "keep") %>% 
    group_by(lang) %>% 
  summarize(mean_trials = round(mean(n_trials), 2),
            mean_answered = round(mean(n_answered), 2), 
            mean_correct = round(mean(p_correct), 2), 
            mean_timing = round(mean(study_length/n_trials*800), 2), 
            med_timing = round(median(study_length/n_trials*800), 2))),
  by = "lang"
  )
...
prime_DF <- prime_data %>% 
  group_by(lang) %>% 
  summarize(total_done = sum(target_answeredN_unrelated > 49 & 
              target_answeredN_related > 49, na.rm = T),
            related_done = sum(target_answeredN_related > 49, na.rm = T), 
            unrelated_done = sum(target_answeredN_unrelated > 49, na.rm = T)) %>% 
...
lang_DF <- lang_DF %>% select(
  "Language", "Exclude", "Keep", "Exclude_Rescored",
  ...
  "M_Age_Keep", "SD_Age_Keep", "Min_Age_Keep", "Max_Age_Keep",
  "M_Trials", "M_Answered", "M_Correct", "M_Time", "Med_Time", 
  "total_done", "related_done", "unrelated_done"
)
...
```{r}
participant_data$url_lab[is.na(participant_data$url_lab) | 
                           participant_data$url_lab == "" |
                         participant_data$url_lab == "1234"] <- "No Lab ID"

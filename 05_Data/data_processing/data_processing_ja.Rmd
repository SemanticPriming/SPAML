---
title: "Data Processing - Japanese"
...
# get all the files
ja_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
ja_files <- ja_files[grepl("ja", ja_files)]
ja_info <- file.info(ja_files)
ja_files <- ja_files[ ja_info$size > 1531904]

ja_data <- list()

# open all files
for (i in 1:length(ja_files)){
  ja_data[[i]] <- processData(ja_files[i])
  ja_data[[i]]$url_lab <- as.character(ja_data[[i]]$url_lab)
}

# convert to data frame and make unique
ja_data_all <- bind_rows(ja_data) %>% unique()
...
# remove test data before official start japanese
ja_data_all <- ja_data_all %>% 
...
ja_data_all <- fill(ja_data_all, uuid, .direction = "down")
...
ja_data_all$uuid[ja_data_all$observation == "2ec82"] <- "69d2c1ec-7b96-4118-b2ca-e721d930d0aa"
ja_data_all$uuid[ja_data_all$observation == "f83b4"] <- "397f978c-3153-4593-a2a0-7f88e5295b5e"
ja_data_all$uuid[ja_data_all$observation == "32248"] <- "c98e732d-83d7-4abd-9569-4b57760ecea1"
ja_data_all$uuid[ja_data_all$observation == "7f860"] <- "05f101c5-6af7-4223-96db-7de5253ec016"
ja_data_all$uuid[ja_data_all$observation == "ce6ea"] <- "cbebd227-69c7-48fb-b214-ad704990ea98"
ja_data_all$uuid[ja_data_all$observation == "692d0"] <- "e4753908-2f3e-4f70-8e68-f81c657818a0"
ja_data_all$uuid[ja_data_all$observation == "8086b"] <- "7d53c867-d8aa-43f8-8c21-7e2990be1161"

# long timeout merge for consent form 
# beginning
# 365b8-38f010df-57ff-477b-8f18-178b75d271b7
# end 
# 6dd8c-fdd0f152-faa6-4492-89b9-1e7434c21c2b (this one duped)
ja_data_all$uuid[ja_data_all$uuid == "fdd0f152-faa6-4492-89b9-1e7434c21c2b" & ja_data_all$observation == "6dd8c"] <- "38f010df-57ff-477b-8f18-178b75d271b7"
ja_data_all$observation[ja_data_all$uuid == "38f010df-57ff-477b-8f18-178b75d271b7" & ja_data_all$observation == "6dd8c"] <- "365b8"

ja_data_all$observation <- paste0(ja_data_all$observation, substr(ja_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(ja_data_all$observation))
...
```{r write-full, eval = F}
write.csv(ja_data_all, 
       gzfile("output_data/full_data/ja_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
ja_data_all <- read.csv("output_data/full_data/ja_full_data.csv.gz")
```
...
export(participant_DF, "output_data/participant_data/ja_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(ja_data_all)[ c(6, 8:37, 41:42) ]
...
ja_data_all[ ja_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
```{r z_score}
# hang on to original time
real_trials$original_duration <- real_trials$duration 
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/ja_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/ja/ja_words.csv")
...
stimuli_data %>% 
  select(target_type, ja_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, ja_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$ja_cue, stimuli_data$ja_target)))
...
length(unique(c(stimuli_data$ja_cue, stimuli_data$ja_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$ja_cue, stimuli_data$ja_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/ja_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/ja_answered_item_data.csv", row.names = F)
```

## Priming Data

- 5/20: fixed the following issues that were misaligned

```{r fix-line-up}
# line up trials for some mismatches
real_trials <- real_trials %>% 
  filter(!(observation == "692d08e68-f81c" & fix_sender == "05_00_044_1")) %>% 
  filter(!(observation == "7f86096db-7de5" & fix_sender == "15_00_093_1")) %>% 
  filter(!(observation == "8086b8c21-7e29" & fix_sender == "17_00_075_1"))
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$ja_cue, stimuli_data$ja_target)
  ...
  export(priming_focus, "output_data/priming_data/ja_prime_trials.csv", row.names = F)
  ...
  ja_num_trials <- ja_num_trials %>% 
    full_join(ja_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/ja_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(ja_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/ja_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/ja_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/ja_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/ja_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/ja_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/ja_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/ja_answered_prime_summary_no3.0.csv", row.names = F)
```

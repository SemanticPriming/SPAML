---
title: "Data Processing - Norwegian"
...
# get all the files
no_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
no_files <- no_files[grepl("no/", no_files)]

no_data <- list()

# open all files
for (i in 1:length(no_files)){
  no_data[[i]] <- processData(no_files[i])
  no_data[[i]]$url_lab <- as.character(no_data[[i]]$url_lab)
}

# convert to data frame and make unique
no_data_all <- bind_rows(no_data) %>% unique()
...
no_data_all <- fill(no_data_all, uuid, .direction = "down")
...
no_data_all$uuid[no_data_all$observation == "7affe"] <- "1bfcd568-4256-4f22-b3ce-e3a60f6196e6"

no_data_all$observation <- paste0(no_data_all$observation, substr(no_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(no_data_all$observation))
...
```{r write-full, eval = F}
write.csv(no_data_all, 
       gzfile("output_data/full_data/no_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
no_data_all <- read.csv("output_data/full_data/no_full_data.csv.gz")
```
...
no_data_all$url_lab[is.na(no_data_all$url_lab)] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/no_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(no_data_all)[ c(6, 8:37, 41:42) ]
...
no_data_all[ no_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/no_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/no/no_words.csv")
...
stimuli_data %>% 
  select(target_type, no_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, no_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$no_cue, stimuli_data$no_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$no_cue, stimuli_data$no_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$no_cue, stimuli_data$no_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/no_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/no_answered_item_data.csv", row.names = F)
```

## Priming Data

- 5/21: no misaligned trials found 
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$no_cue, stimuli_data$no_target)
  ...
  export(priming_focus, "output_data/priming_data/no_prime_trials.csv", row.names = F)
  ...
  no_num_trials <- no_num_trials %>% 
    full_join(no_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/no_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(no_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/no_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/no_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/no_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/no_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/no_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/no_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/no_answered_prime_summary_no3.0.csv", row.names = F)
```

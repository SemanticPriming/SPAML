...
# get all the files
en_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)

en_files <- en_files[grepl("en", en_files)]
en_info <- file.info(en_files)
en_files <- en_files[ en_info$size > 1531904]

# exclude the testing data
en_files <- en_files[!grepl("nonrun", en_files)]

en_data <- list()

# open all files
for (i in 1:length(en_files)){
  en_data[[i]] <- processData(en_files[i])
  en_data[[i]]$url_lab <- as.character(en_data[[i]]$url_lab)
  en_data[[i]]$url_special_code <- as.character(en_data[[i]]$url_special_code)
}

# convert to data frame and make unique
en_data_all <- bind_rows(en_data) %>% unique()
...
remove_zpid <- en_data_all %>% 
  filter(url_lab == "ZPID") %>% 
  pull(observation)

en_data_all <- en_data_all %>% 
  filter(!(observation %in% remove_zpid)) %>% 
...
# fix lab 829 before removing column
temp_obs <- en_data_all %>% 
  filter(sender == "Consent Form") %>% 
  filter(!is.na(url_lab829) | !is.na(url_lab_829))

temp_obs # none so ok 

# fill in uuid for each row
en_data_all <- fill(en_data_all, uuid, .direction = "down")
...
# work out the issues
temp_2 <- temp %>% 
  filter(observation %in% temp$observation[temp$dups_o])

# a large number of these are the duplicate consent form issue 
dup_consent <- en_data_all %>% 
  filter(sender == "Consent Form") %>% 
  filter(url_lab == "2772") %>% 
  pull(observation)

temp_2 <- temp_2 %>% 
  filter(!(observation %in% dup_consent))

# fixes
en_data_all$uuid[en_data_all$observation == "d89dd"] <- "f8e4088b-05ca-4e45-b24b-cb4e21f50195"
en_data_all$uuid[en_data_all$observation == "791cc"] <- "5ef1edac-ffe8-4123-a246-eb1be8286877"
en_data_all$uuid[en_data_all$observation == "32843"] <- "8563cff8-3cee-4f2a-bda6-b82e047cc79c"
en_data_all$uuid[en_data_all$observation == "58e7b"] <- "f329facf-a466-42ad-af68-f2eea2578d56"
en_data_all$uuid[en_data_all$observation == "66ec5"] <- "3a5954ee-1904-42c5-a5df-a949f229efac"
en_data_all$uuid[en_data_all$observation == "8168a"] <- "6c84cd1f-540a-46cd-a134-f93408f744c7"
en_data_all$uuid[en_data_all$observation == "893f7"] <- "b7c2a7b4-d896-4409-9a49-d1d85acc73fd"
en_data_all$uuid[en_data_all$observation == "8b90d"] <- "c9f13981-1bf0-4867-a1e8-a305446e0ee5"
en_data_all$uuid[en_data_all$observation == "92faa"] <- "aa38168b-c647-4dfa-a50e-a4941fc013f9"
en_data_all$uuid[en_data_all$observation == "a22f7"] <- "57846cec-4e76-4d50-97ce-e47cb661c580"
en_data_all$uuid[en_data_all$observation == "d71ab"] <- "156bc7da-a99b-4f26-bb5c-79bf791642ea"
en_data_all$uuid[en_data_all$observation == "d76fd"] <- "8c7b36b3-7f53-4910-9363-94c2bd83b6d9"
en_data_all$uuid[en_data_all$observation == "fa987"] <- "36c7395b-26a6-4fc6-9495-46a491ef432a"

# all the rest checked either they are 1 row or
# two pairs of 2469 (total number of exp rows)
# or one with full rows and one with partial rows

en_data_all$observation <- paste0(en_data_all$observation, substr(en_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(en_data_all$observation))

# remove ZPID, mturk, uuid 
en_data_all <- en_data_all %>% 
  select(-url_special_code, -uuid, -meta_location, -x, -url,
         -url_fbclid, -url_lab_829, -url_lab829, -url_special)

# deal with issues of words that were incorrect 
temp <- en_data_all %>% 
  group_by(word, class) %>% 
  summarize(accuracy = sum(correct, na.rm = T) / n())

en_data_all$word <- gsub("digusting", "disgusting", en_data_all$word)

# deal with real words miscoded
incorrect_words <- c("midnight", "strawberry", "dvd", "fell", "lapsing", "gps")
find_trials <- en_data_all$word %in% incorrect_words & 
                            en_data_all$class == "nonword"

table(en_data_all$correct[find_trials])

en_data_all$correct[find_trials] <- ifelse(
  en_data_all$correct[find_trials] == FALSE, TRUE, ifelse(
    en_data_all$correct[find_trials] == TRUE, FALSE, en_data_all$correct[find_trials]
  )
)

table(en_data_all$correct[find_trials])

en_data_all$class[find_trials] <- "word"
...
```{r write-full, eval = F}
write.csv(en_data_all, 
       gzfile("output_data/full_data/en_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
en_data_all <- read.csv("output_data/full_data/en_full_data.csv.gz")
```
...
# make timestamp a time
en_data_all$timestamp <- as.POSIXct(substr(en_data_all$timestamp, 1, 19), 
                                    format = "%Y-%m-%d %H:%M:%OS")
```
...
en_data_all$url_lab <- gsub("[[:punct:]]", "", en_data_all$url_lab)
en_data_all$url_lab <- gsub("^2793.*", "279", en_data_all$url_lab)
en_data_all$url_lab <- gsub("^2791.*", "279", en_data_all$url_lab)
en_data_all$url_lab <- gsub("^2792.*", "279", en_data_all$url_lab)
en_data_all$url_lab <- gsub("^2970$", "2790", en_data_all$url_lab)
en_data_all$url_lab[en_data_all$url_lab == "37"] <- "371"
en_data_all$url_lab[is.na(en_data_all$url_lab) | 
                      en_data_all$url_lab == "" | 
                      en_data_all$url_lab == "999" | 
                      en_data_all$url_lab == "9999" |
                      en_data_all$url_lab == "2" | 
                      en_data_all$url_lab == "1234"
                      ] <- "No Lab ID"
en_data_all$url_lab[en_data_all$url_lab == "828"] <- "829"

# Merge 373 and 1485
en_data_all$url_lab[en_data_all$url_lab == "373" | 
                           en_data_all$url_lab == "1485" ] <- "373-1485"

# deal with double consent form lab code below 
# sum(lab_codes$count)
```

```{r participant_data}
# temp create temp_obs to deal with consent form issue
en_data_all$temp_obs <- substr(en_data_all$observation, 1, 5)
...
# there are two rows different nrow
combo_temp <- bind_rows(second_one, dup_rows) %>% 
  arrange(temp_obs, sender_id) %>% 
  left_join(
    en_data_all %>% 
      group_by(observation) %>% 
      summarize(n_trial = n()), 
  ...
  select(observation, sender_id, fix_sender, n_trial, temp_obs) %>% 
  left_join(
    bind_rows(second_one, dup_rows) %>% 
  arrange(temp_obs, sender_id) %>% 
  left_join(
    en_data_all %>% 
      group_by(observation) %>% 
...
# this next line removes the ones with a sender_id == 0 for consent
# not the ones - so remove all the ones that need to stay that have
# sender_id == 0 but actual rows of data, not just the one duplicated
# line
nrow(dup_rows)

dup_rows <- dup_rows %>% 
  anti_join(combo_temp, by = "observation")
...
# add in url_lab for this specific lab
en_data_all$url_lab[en_data_all$temp_obs %in% dup_rows$temp_obs &
                      en_data_all$sender_id == 1] <- 2772

# updated lab code count
lab_codes <- en_data_all %>% 
  filter(sender == "Consent Form") %>% 
  group_by(url_lab) %>% 
  summarize(count = n())
...
# back to making participant data
exp <- en_data_all %>%
  filter(sender == "Consent Form")
...
sum(is.na(participant_DF$time_commit_consent))
...
participant_DF$keep_answered[(as.numeric(substr(participant_DF$timestamp_consent, 1,4)) - participant_DF$which_year_were_you_born) < 18 | is.na(participant_DF$which_year_were_you_born)] <- "exclude"
  ...
  # special issue with lab 91 as they made us 
  # make the age question optional
  # but we verified they were 18 in a qualtrics form
  # get the ids
  ids_91 <- en_data_all %>% 
    filter(url_lab == "91") %>% 
    pull(observation)
...
export(participant_DF, "output_data/participant_data/en_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(en_data_all)[ c(6, 8:37, 41:42) ]
...
en_data_all[ en_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/en_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/en/en_words.csv")
...
stimuli_data %>% 
  select(target_type, en_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, en_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
length(unique(c(stimuli_data$en_cue, stimuli_data$en_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$en_cue, stimuli_data$en_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/en_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/en_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  bad_scramble <- temp %>% 
               filter(cue_word == "doctor2") %>% 
               pull(observation)
  
  temp <- temp %>% 
    filter(!(observation %in% bad_scramble))
  nrow(temp)
  
  # subset out NAs at some point they will be practice trials
  priming_focus <- subset(priming_focus, !is.na(type))
  
  # exclude nonword trials that got matched because of 
  # bad nonwords for english
  priming_focus <- subset(priming_focus, type != "nonword")
  
  nrow(priming_focus)
  table(priming_focus$type, useNA = "ifany")
  
  export(priming_focus, "output_data/priming_data/en_prime_trials.csv", row.names = F)
  ...
  en_num_trials <- en_num_trials %>% 
    full_join(en_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/en_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(en_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/en_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/en_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/en_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/en_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/en_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/en_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/en_answered_prime_summary_no3.0.csv", row.names = F)
```

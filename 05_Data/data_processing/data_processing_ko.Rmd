...
# get all the files
ko_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
ko_files <- ko_files[grepl("ko", ko_files)]

ko_data <- list()

# open all files
for (i in 1:length(ko_files)){
  ko_data[[i]] <- processData(ko_files[i])
  ko_data[[i]]$url_lab <- as.character(ko_data[[i]]$url_lab)
  ko_data[[i]]$url_special_code <- as.character(ko_data[[i]]$url_special_code)
}

# convert to data frame and make unique
ko_data_all <- bind_rows(ko_data) %>% unique()
...
ko_data_all <- fill(ko_data_all, uuid, .direction = "down")
...
ko_data_all$observation <- paste0(ko_data_all$observation, substr(ko_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(ko_data_all$observation))
...
```{r write-full, eval = F}
write.csv(ko_data_all, 
       gzfile("output_data/full_data/ko_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
ko_data_all <- read.csv("output_data/full_data/ko_full_data.csv.gz")
```
...
export(participant_DF, "output_data/participant_data/ko_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(ko_data_all)[ c(6, 8:37, 41:42) ]
...
ko_data_all[ ko_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
# fix that 밍 is sorry and a real word
# 위원 is commissioner 
real_trials$class[real_trials$word == "밍"] <- "word"
real_trials$correct_response[real_trials$word == "밍"] <- "word"
real_trials$correct[real_trials$word == "밍"] <- 
  real_trials$response[real_trials$word == "밍"] == "word"

real_trials$class[real_trials$word == "위원"] <- "word"
real_trials$correct_response[real_trials$word == "위원"] <- "word"
real_trials$correct[real_trials$word == "위원"] <- 
  real_trials$response[real_trials$word == "위원"] == "word"
...
real_trials_NA <- real_trials %>% #data frame
  filter(is.na(correct) | #grab time out trials OR
           is.na(duration) | #deal with weird non-time records
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/ko_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/ko/ko_words.csv")
...
stimuli_data %>% 
  select(target_type, ko_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, ko_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$ko_cue, stimuli_data$ko_target)))
...
length(unique(c(stimuli_data$ko_cue, stimuli_data$ko_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$ko_cue, stimuli_data$ko_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/ko_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/ko_answered_item_data.csv", row.names = F)
```

## Priming Data

- 5/20: 262 trials are because of a misspelling of a word early in the experiment 

```{r}
real_trials <- real_trials %>% 
  filter(!(observation == "86a239709-d964" & fix_sender == "17_00_052_1"))
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$ko_cue, stimuli_data$ko_target)
  ...
  export(priming_focus, "output_data/priming_data/ko_prime_trials.csv", row.names = F)
  ...
  ko_num_trials <- ko_num_trials %>% 
    full_join(ko_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/ko_trial_unique.csv")
...
# need to fix the one related trials that can be paired with the other unrelated trials
# there are two related and one unrelated so best to copy the unrelated line again into the data    
    prime_duplicate <- priming_Z_summary %>% 
      filter(type == "unrelated") %>% 
      filter(target_word == "배")
    
  priming_Z_summary <- priming_Z_summary %>% 
    bind_rows(prime_duplicate) %>% 
    # merge with the number of trials data
    left_join(ko_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
## this process will be repeated for 2.5 and 3.0 z score outliers excluded
  ...
  prime_duplicate <- priming_Z_summary_no2.5 %>% 
      filter(type == "unrelated") %>% 
      filter(target_word == "배")
    
  priming_Z_summary_no2.5 <- priming_Z_summary_no2.5 %>% 
    bind_rows(prime_duplicate) %>% 
    # deal with duplicates because of translation
    arrange(target_word, type) %>% 
      group_by(type) %>% 
  ...
  prime_duplicate <- priming_Z_summary_no3.0 %>% 
      filter(type == "unrelated") %>% 
      filter(target_word == "배")
    
  priming_Z_summary_no3.0 <- priming_Z_summary_no3.0 %>% 
    bind_rows(prime_duplicate) %>% 
    # merge with the number of trials data
    left_join(ko_num_trials, by = c("word_combo" = "word_combo")) %>% 
    # deal with duplicates because of translation
...
write.csv(priming_Z_summary, "output_data/priming_data/ko_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/ko_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/ko_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/ko_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/ko_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/ko_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/ko_answered_prime_summary_no3.0.csv", row.names = F)
```

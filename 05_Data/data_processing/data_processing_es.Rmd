...
# get all the files
es_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
es_files <- es_files[grepl("es", es_files)]

es_data <- list()

# open all files
for (i in 1:length(es_files)){
  es_data[[i]] <- processData(es_files[i])
  es_data[[i]]$url_lab <- as.character(es_data[[i]]$url_lab)
}

# convert to data frame and make unique
es_data_all <- bind_rows(es_data) %>% unique()
...
es_data_all <- fill(es_data_all, uuid, .direction = "down")
...
es_data_all$uuid[es_data_all$observation == "7134e"] <- "6459f0cc-17d2-4ff7-a3c5-0fb0ac090b4a"
es_data_all$uuid[es_data_all$observation == "a593d"] <- "c9bc99ed-24b9-4992-a963-04241c30f3fb"
es_data_all$uuid[es_data_all$observation == "d901c"] <- "cbcf0bb1-4e29-452b-b2c2-12f1a4559c07"

# other one is two separate people 

es_data_all$observation <- paste0(es_data_all$observation, substr(es_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(es_data_all$observation))

# remove ZPID, mturk, uuid 
es_data_all <- es_data_all %>% 
  select(-url_special_code, -uuid, -meta_location, -x, -url,
         -url_s, -url_prolific_pid, -url_session_id, -url_fbclid,
         -url_study_id)
...
```{r write-full, eval = F}
write.csv(es_data_all, 
       gzfile("output_data/full_data/es_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
es_data_all <- read.csv("output_data/full_data/es_full_data.csv.gz")
```
...
es_data_all$url_lab <- gsub("[[:punct:]]|Lleva", "", es_data_all$url_lab)
es_data_all$url_lab[
  es_data_all$url_lab == "9999" | 
    is.na(es_data_all$url_lab)
] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/es_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(es_data_all)[ c(6, 8:37, 41:42) ]
...
es_data_all[ es_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials,
          gzfile("output_data/trial_data/es_trial_data.csv.gz"),
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/es/es_words.csv")
...
stimuli_data %>% 
  select(target_type, es_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, es_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$es_cue, stimuli_data$es_target)))
...
length(unique(c(stimuli_data$es_cue, stimuli_data$es_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$es_cue, stimuli_data$es_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/es_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/es_answered_item_data.csv", row.names = F)
```

## Priming Data

- 5/17: no misaligned trials
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$es_cue, stimuli_data$es_target)
  ...
  export(priming_focus, "output_data/priming_data/es_prime_trials.csv", row.names = F)
  ...
  es_num_trials <- es_num_trials %>% 
    full_join(es_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/es_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(es_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/es_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/es_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/es_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/es_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/es_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/es_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/es_answered_prime_summary_no3.0.csv", row.names = F)
```

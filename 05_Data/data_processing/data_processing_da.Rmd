...
knitr::opts_chunk$set(echo = TRUE, size="scriptsize", message = FALSE)
...
# get all the files
da_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
da_files <- da_files[grepl("/da/", da_files)]

da_data <- list()

# open all files
for (i in 1:length(da_files)){
  da_data[[i]] <- processData(da_files[i])
  da_data[[i]]$url_lab <- as.character(da_data[[i]]$url_lab)
  da_data[[i]]$url_special_code <- as.character(da_data[[i]]$url_special_code)
}

# convert to data frame and make unique
da_data_all <- bind_rows(da_data) %>% unique()
...
da_data_all <- fill(da_data_all, uuid, .direction = "down")
...
# long break probably computer went to sleep
# long pause on consent form
da_data_all$uuid[da_data_all$observation == "01bbb"] <- "14824890-af7e-4fe4-8dca-bcb1a9e93ad4"
da_data_all$uuid[da_data_all$observation == "32b6f"] <- "e6586bdc-221d-4fd4-a790-61dc7ea5ad68"

da_data_all$observation <- paste0(da_data_all$observation, substr(da_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(da_data_all$observation))

# remove ZPID, mturk, uuid 
da_data_all <- da_data_all %>% 
  select(-url_special_code, -uuid, -x, -url, -meta_location,
         -url_study_id, -url_session_id, -url_fbclid
         )
...
```{r write-full, eval = F}
write.csv(da_data_all, 
       gzfile("output_data/full_data/da_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
da_data_all <- read.csv("output_data/full_data/da_full_data.csv.gz")
```
...
# 9999 appears to be just consent
# NA appears ok - given the timing it's 2530
da_data_all$url_lab[
  is.na(da_data_all$url_lab) & da_data_all$sender == "Consent Form"
] <- 2530

da_data_all$url_lab[da_data_all$url_lab == "9999"] <- "No Lab ID"
...
# people who didn't answer anything 
...
export(participant_DF, "output_data/participant_data/da_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(da_data_all)[ c(6, 8:37, 41:42) ]
...
da_data_all[ da_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials,
          gzfile("output_data/trial_data/da_trial_data.csv.gz"),
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/da/da_words.csv")
...
stimuli_data %>% 
  select(target_type, da_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, da_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
item_data <- real_trials %>% #data frame
  ...
  #but probably need to exclude time outs for accuracy (see redefined accuracy)
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$da_cue, stimuli_data$da_target)))
...
length(unique(c(stimuli_data$da_cue, stimuli_data$da_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$da_cue, stimuli_data$da_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/da_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/da_answered_item_data.csv", row.names = F)
```

## Priming Data

- Checked 5/3, no issues matching alignment. 
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$da_cue, stimuli_data$da_target)
  ...
  export(priming_focus, "output_data/priming_data/da_prime_trials.csv", row.names = F)
  ...
  da_num_trials <- priming_focus %>% 
    group_by(word_combo) %>% 
    summarize(
              # take out information due to exclusion of participants
  ...
  da_num_trials <- da_num_trials %>% 
    full_join(da_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/da_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(da_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/da_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/da_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/da_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/da_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/da_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/da_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/da_answered_prime_summary_no3.0.csv", row.names = F)
```

---
title: "Data Processing - Traditional Chinese"
...
# get all the files
zh_hk_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
zh_hk_files <- zh_hk_files[grepl("zh_hk", zh_hk_files)]

zh_hk_data <- list()

# open all files
for (i in 1:length(zh_hk_files)){
  zh_hk_data[[i]] <- processData(zh_hk_files[i])
  zh_hk_data[[i]]$url_lab <- as.character(zh_hk_data[[i]]$url_lab)
}

# convert to data frame and make unique
zh_hk_data_all <- bind_rows(zh_hk_data) %>% unique()
...
names(zh_hk_data_all)

# fill in uuid for each row
zh_hk_data_all <- fill(zh_hk_data_all, uuid, .direction = "down")
...
nrow(temp_2)
# all issues with duplicate consent

zh_hk_data_all$observation <- paste0(zh_hk_data_all$observation, substr(zh_hk_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(zh_hk_data_all$observation))
...
```{r write-full, eval = F}
write.csv(zh_hk_data_all, 
       gzfile("output_data/full_data/zh_hk_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
zh_hk_data_all <- read.csv("output_data/full_data/zh_hk_full_data.csv.gz")
```
...
zh_hk_data_all$url_lab[is.na(zh_hk_data_all$url_lab)] <- "2772"

# includes 64 duplicate consent forms 
sum(lab_codes$count)
```

```{r participant_data}
# temp create temp_obs to deal with consent form issue
zh_hk_data_all$temp_obs <- substr(zh_hk_data_all$observation, 1, 5)
...
zh_hk_data_all$url_lab[zh_hk_data_all$temp_obs %in% dup_rows$temp_obs &
...
export(participant_DF, "output_data/participant_data/zh_hk_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(zh_hk_data_all)[ c(6, 8:37, 41:42) ]
...
zh_hk_data_all[ zh_hk_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/zh_hk_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/zh_hk/zh_hk_words.csv")
...
stimuli_data %>% 
  select(target_type, zh_hk_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, zh_hk_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$zh_hk_cue, stimuli_data$zh_hk_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$zh_hk_cue, stimuli_data$zh_hk_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$zh_hk_cue, stimuli_data$zh_hk_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/zh_hk_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/zh_hk_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$zh_hk_cue, stimuli_data$zh_hk_target)
  ...
  export(priming_focus, "output_data/priming_data/zh_hk_prime_trials.csv", row.names = F)
  ...
  zh_hk_num_trials <- priming_focus %>% 
    group_by(word_combo) %>% 
    summarize(
  ...
  zh_hk_num_trials <- zh_hk_num_trials %>% 
    full_join(zh_hk_num_trials2, 
              by = "word_combo")
...
# Calculate Statistics ----------------------------------------------------
  priming_trial_paired <- priming_Z %>% 
    ...
    export(., file = "../../03_Materials/matched_stimuli/zh_hk_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(zh_hk_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/zh_hk_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/zh_hk_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/zh_hk_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  table(priming_focus$type, useNA = "ifany")
    ...
    export(priming_focus, "output_data/priming_data/zh_hk_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/zh_hk_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/zh_hk_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/zh_hk_answered_prime_summary_no3.0.csv", row.names = F)
```

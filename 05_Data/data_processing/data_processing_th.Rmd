...
# get all the files
th_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
th_files <- th_files[grepl("th", th_files)]

th_data <- list()

# open all files
for (i in 1:length(th_files)){
  th_data[[i]] <- processData(th_files[i])
  th_data[[i]]$url_lab <- as.character(th_data[[i]]$url_lab)
}

# convert to data frame and make unique
th_data_all <- bind_rows(th_data) %>% unique()
...
th_data_all <- fill(th_data_all, uuid, .direction = "down")
...
th_data_all$observation <- paste0(th_data_all$observation, substr(th_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(th_data_all$observation))
...
```{r write-full, eval = F}
write.csv(th_data_all, 
       gzfile("output_data/full_data/th_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
th_data_all <- read.csv("output_data/full_data/th_full_data.csv.gz")
```
...
# only one lab
th_data_all$url_lab[is.na(th_data_all$url_lab)] <- "2779"
...
export(participant_DF, "output_data/participant_data/th_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(th_data_all)[ c(6, 8:37, 41:42) ]
...
th_data_all[ th_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/th_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/th/th_words.csv")
...
stimuli_data %>% 
  select(target_type, th_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, th_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$th_cue, stimuli_data$th_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$th_cue, stimuli_data$th_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$th_cue, stimuli_data$th_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/th_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/th_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$th_cue, stimuli_data$th_target)
  ...
  export(priming_focus, "output_data/priming_data/th_prime_trials.csv", row.names = F)
  ...
  th_num_trials <- th_num_trials %>% 
    full_join(th_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
 ...
 priming_trial_paired <- priming_Z %>% 
  ...
  export(., file = "../../03_Materials/matched_stimuli/th_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(th_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/th_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/th_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/th_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/th_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/th_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/th_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/th_answered_prime_summary_no3.0.csv", row.names = F)
```

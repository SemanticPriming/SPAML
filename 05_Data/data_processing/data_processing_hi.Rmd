...
# get all the files
hi_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
hi_files <- hi_files[grepl("hi", hi_files)]

hi_data <- list()

# open all files
for (i in 1:length(hi_files)){
  hi_data[[i]] <- processData(hi_files[i])
  hi_data[[i]]$url_lab <- as.character(hi_data[[i]]$url_lab)
}

# convert to data frame and make unique
hi_data_all <- bind_rows(hi_data) %>% unique()
...
hi_data_all <- hi_data_all %>%
  filter(timestamp > as.POSIXct("2023-11-05"))
...
hi_data_all <- fill(hi_data_all, uuid, .direction = "down")
...
hi_data_all$observation <- paste0(hi_data_all$observation, substr(hi_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(hi_data_all$observation))
...
```{r write-full, eval = F}
write.csv(hi_data_all, 
       gzfile("output_data/full_data/hi_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
hi_data_all <- read.csv("output_data/full_data/hi_full_data.csv.gz")
```
...
hi_data_all$url_lab[hi_data_all$url_lab == "9999" | 
                      is.na(hi_data_all$url_lab)] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/hi_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(hi_data_all)[ c(6, 8:37, 41:42) ]
...
hi_data_all[ hi_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/hi_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/hi/hi_words.csv")
...
stimuli_data %>% 
  select(target_type, hi_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, hi_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$hi_cue, stimuli_data$hi_target)))
...
length(unique(c(stimuli_data$hi_cue, stimuli_data$hi_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$hi_cue, stimuli_data$hi_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/hi_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/hi_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$hi_cue, stimuli_data$hi_target)
  ...
  export(priming_focus, "output_data/priming_data/hi_prime_trials.csv", row.names = F)
  ...
  hi_num_trials <- hi_num_trials %>% 
    full_join(hi_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/hi_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(hi_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/hi_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/hi_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/hi_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/hi_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/hi_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/hi_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/hi_answered_prime_summary_no3.0.csv", row.names = F)
```

...
# get all the files
it_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
it_files <- it_files[grepl("/it.*/", it_files)]

it_data <- list()

# open all files
for (i in 1:length(it_files)){
  it_data[[i]] <- processData(it_files[i])
  it_data[[i]]$url_lab <- as.character(it_data[[i]]$url_lab)
  it_data[[i]]$url_special_code <- as.character(it_data[[i]]$url_special_code)
}

# convert to data frame and make unique
it_data_all <- bind_rows(it_data) %>% unique()
...
it_data_all <- fill(it_data_all, uuid, .direction = "down")
...
it_data_all$observation <- paste0(it_data_all$observation, substr(it_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(it_data_all$observation))
...
```{r write-full, eval = F}
write.csv(it_data_all, 
       gzfile("output_data/full_data/it_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
it_data_all <- read.csv("output_data/full_data/it_full_data.csv.gz")
```
...
it_data_all$url_lab[it_data_all$url_lab == "9999" | 
                      is.na(it_data_all$url_lab)] <- "No Lab ID"
it_data_all$url_lab[it_data_all$url_lab == "2537"] <- "2539"
...
export(participant_DF, "output_data/participant_data/it_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(it_data_all)[ c(6, 8:37, 41:42) ]
...
it_data_all[ it_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
# fix that DVD is a word, re means king 
real_trials$class[real_trials$word == "dvd"] <- "word"
real_trials$correct_response[real_trials$word == "dvd"] <- "word"
real_trials$correct[real_trials$word == "dvd"] <- 
  real_trials$response[real_trials$word == "dvd"] == "word"
...
# fix that DVD is a word
real_trials$class[real_trials$word == "re"] <- "word"
real_trials$correct_response[real_trials$word == "re"] <- "word"
real_trials$correct[real_trials$word == "re"] <- 
  real_trials$response[real_trials$word == "re"] == "word"
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/it_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/it/it_words.csv")
...
stimuli_data %>% 
  select(target_type, it_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, it_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$it_cue, stimuli_data$it_target)))
...
length(unique(c(stimuli_data$it_cue, stimuli_data$it_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$it_cue, stimuli_data$it_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/it_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/it_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$it_cue, stimuli_data$it_target)
  ...
  export(priming_focus, "output_data/priming_data/it_prime_trials.csv", row.names = F)
  ...
  it_num_trials <- it_num_trials %>% 
    full_join(it_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/it_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(it_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/it_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/it_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/it_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/it_answered_prime_trials.csv", row.names = F)
  ...
  it_num_trials2 <-
    ...
    group_by(word_combo) %>% 
    summarize(
              target_correct_keep = sum(target_correct, na.rm = T),
              target_answeredN_keep = sum(target_end_of_trial == "response", na.rm = T), 
              target_timeoutN_keep = sum(target_end_of_trial == "timeout", na.rm = T),
              target_prop_correct_keep = target_correct_keep/target_answeredN_keep,
              cue_correct_keep = sum(cue_correct, na.rm = T),
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/it_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/it_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/it_answered_prime_summary_no3.0.csv", row.names = F)
```

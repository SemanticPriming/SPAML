...
```{r import_data, eval = F, message = FALSE}
# get all the files
ar_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
ar_files <- ar_files[grepl("ar", ar_files)]

ar_data <- list()

# open all files
for (i in 1:length(ar_files)){
  ar_data[[i]] <- processData(ar_files[i])
  ar_data[[i]]$url_lab <- as.character(ar_data[[i]]$url_lab)
}

# convert to data frame and make unique
ar_data_all <- bind_rows(ar_data) %>% unique()
...
ar_data_all <- fill(ar_data_all, uuid, .direction = "down")
...
ar_data_all$uuid[ar_data_all$observation == "c30b3"] <- "312e7b8a-186d-4a2c-b22b-2af4626b1027"

ar_data_all$observation <- paste0(ar_data_all$observation, substr(ar_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(ar_data_all$observation))

# remove ZPID, mturk, uuid 
ar_data_all <- ar_data_all %>% 
  select(-url_special_code, -uuid, -x, -url,
         -meta_location)
...
```{r write-full, eval = F}
write.csv(ar_data_all, 
       gzfile("output_data/full_data/ar_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
ar_data_all <- read.csv("output_data/full_data/ar_full_data.csv.gz")
```
...
# 58 is a turkish lab, so unclear how they are here 
...
export(participant_DF, "output_data/participant_data/ar_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(ar_data_all)[ c(6, 8:37, 41:42) ]
...
ar_data_all[ ar_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
- Response latencies longer than 3000 ms (note we determined these do happen even with a timeout set on the software).
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/ar_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/ar/ar_words.csv")
...
stimuli_data %>% 
  select(target_type, ar_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, ar_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$ar_cue, stimuli_data$ar_target)))
...
length(unique(c(stimuli_data$ar_cue, stimuli_data$ar_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$ar_cue, stimuli_data$ar_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/ar_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/ar_answered_item_data.csv", row.names = F)
```

## Priming Data

- Checked 5/3/23 only two trials were not matched, early in the study, we fixed a typo in the stimuli. 
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$ar_cue, stimuli_data$ar_target)
  ...
  export(priming_focus, "output_data/priming_data/ar_prime_trials.csv", row.names = F)
  ...
  ar_num_trials <- ar_num_trials %>% 
    full_join(ar_num_trials2, by = "word_combo")
...
# Calculate Statistics ----------------------------------------------------
  priming_trial_paired <- priming_Z %>% 
    ...
    export(., file = "../../03_Materials/matched_stimuli/ar_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(ar_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/ar_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/ar_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/ar_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/ar_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/ar_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/ar_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/ar_answered_prime_summary_no3.0.csv", row.names = F)
```

---
title: "Data Processing - Simplified Chinese"
...
# get all the files
zh_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
zh_files <- zh_files[grepl("/zh.?/", zh_files)]

zh_data <- list()

# open all files
for (i in 1:length(zh_files)){
  zh_data[[i]] <- processData(zh_files[i])
  zh_data[[i]]$url_lab <- as.character(zh_data[[i]]$url_lab)
}

# convert to data frame and make unique
zh_data_all <- bind_rows(zh_data) %>% unique()
...
zh_data_all <- fill(zh_data_all, uuid, .direction = "down")
...
zh_data_all$uuid[zh_data_all$observation == "1e810"] <- "77bb90f6-afd2-4d99-81a6-8b79d8155abc"
zh_data_all$uuid[zh_data_all$observation == "c13b7"] <- "b4cb4822-2af9-4762-b138-2a050314c238"
zh_data_all$uuid[zh_data_all$observation == "e28be"] <- "d6b807ca-349b-43cd-b489-691c63458292"

zh_data_all$observation <- paste0(zh_data_all$observation, substr(zh_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(zh_data_all$observation))
...
```{r write-full, eval = F}
write.csv(zh_data_all, 
       gzfile("output_data/full_data/zh_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
zh_data_all <- read.csv("output_data/full_data/zh_full_data.csv.gz")
```
...
zh_data_all$url_lab <- gsub("[[:punct:]]", "", zh_data_all$url_lab)
zh_data_all$url_lab[zh_data_all$url_lab == "2976httpspsa007psysciaccorgzhlab2976"] <- "2976"
zh_data_all$url_lab[zh_data_all$url_lab == "299"] <- "2998"
zh_data_all$url_lab[zh_data_all$url_lab == "9999"] <- "No Lab ID"
...
zh_data_all$temp_obs <- substr(zh_data_all$observation, 1, 5)
...
zh_data_all$url_lab[zh_data_all$temp_obs %in% dup_rows$temp_obs &
...
export(participant_DF, "output_data/participant_data/zh_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(zh_data_all)[ c(6, 8:37, 41:42) ]
...
zh_data_all[ zh_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/zh_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/zh/zh_words.csv")
...
stimuli_data %>% 
  select(target_type, zh_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, zh_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$zh_cue, stimuli_data$zh_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$zh_cue, stimuli_data$zh_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$zh_cue, stimuli_data$zh_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/zh_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/zh_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$zh_cue, stimuli_data$zh_target)
  ...
  export(priming_focus, "output_data/priming_data/zh_prime_trials.csv", row.names = F)
  ...
  zh_num_trials <- zh_num_trials %>% 
    full_join(zh_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
    ...
    export(., file = "../../03_Materials/matched_stimuli/zh_trial_unique.csv")
  
  priming_find_four <- priming_Z %>% 
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
            ...
            .groups = "keep") %>% 
    filter(type == "related") %>% 
    filter(target_word == "写作" | 
             target_word == "危险" | 
             target_word == "大概")
  
  priming_find_four <- bind_rows(
    priming_find_four,
    priming_find_four %>% filter(target_word == "大概")
  )

  
  priming_Z_summary <- bind_rows(
    priming_Z %>% 
  ##group them by target word and condition related/unrelated
  group_by(word_combo, type, target_word) %>% 
  ##create average scores by condition 
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
            ...
            .groups = "keep"),
  priming_find_four) %>% 
    # add in four trials that repeat

    # merge with the number of trials data
    left_join(zh_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
## this process will be repeated for 2.5 and 3.0 z score outliers excluded
  ...
  priming_find_four2.5 <- priming_Z %>% 
  ...
  priming_find_four2.5 <- bind_rows(
    priming_find_four2.5,
    priming_find_four2.5 %>% filter(target_word == "大概")
  )
  
  priming_Z_summary_no2.5 <- bind_rows(
    priming_Z %>% 
  ...
  priming_find_four3.0 <- priming_Z %>% 
  ...
  priming_find_four3.0 <- bind_rows(
    priming_find_four3.0,
    priming_find_four3.0 %>% filter(target_word == "大概")
  )
  
  priming_Z_summary_no3.0 <- bind_rows(
    priming_Z %>% 
  ##filter out z score outliers
  filter(target_Z_RT < 3.0) %>% 
  ##group them by target word and condition related/unrelated
...
write.csv(priming_Z_summary, "output_data/priming_data/zh_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/zh_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/zh_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/zh_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/zh_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/zh_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/zh_answered_prime_summary_no3.0.csv", row.names = F)
```

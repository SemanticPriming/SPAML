---
title: "Data Processing - Slovenian"
...
# get all the files
sl_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
sl_files <- sl_files[grepl("sl", sl_files)]

sl_data <- list()

# open all files
for (i in 1:length(sl_files)){
  sl_data[[i]] <- processData(sl_files[i])
  sl_data[[i]]$url_lab <- as.character(sl_data[[i]]$url_lab)
}

# convert to data frame and make unique
sl_data_all <- bind_rows(sl_data) %>% unique()
...
sl_data_all <- sl_data_all %>%
  filter(timestamp > as.POSIXct("2023-11-18"))
...
sl_data_all <- fill(sl_data_all, uuid, .direction = "down")
...
sl_data_all$observation <- paste0(sl_data_all$observation, substr(sl_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(sl_data_all$observation))
...
```{r write-full, eval = F}
write.csv(sl_data_all, 
       gzfile("output_data/full_data/sl_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
sl_data_all <- read.csv("output_data/full_data/sl_full_data.csv.gz")
```
...
export(participant_DF, "output_data/participant_data/sl_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(sl_data_all)[ c(6, 8:37, 41:42) ]
...
sl_data_all[ sl_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/sl_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/sl/sl_words.csv")
...
stimuli_data %>% 
  select(target_type, sl_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, sl_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$sl_cue, stimuli_data$sl_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$sl_cue, stimuli_data$sl_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$sl_cue, stimuli_data$sl_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/sl_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/sl_answered_item_data.csv", row.names = F)
```

## Priming Data

- Unclear why the trials are the old ones for one of these batches - the files online were clearly updated, but some old combinations of trials remain. 
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$sl_cue, stimuli_data$sl_target)
  ...
  export(priming_focus, "output_data/priming_data/sl_prime_trials.csv", row.names = F)
  ...
  sl_num_trials <- sl_num_trials %>% 
    full_join(sl_num_trials2)
...
priming_trial_paired <- priming_Z %>% 
  ...
  export(., file = "../../03_Materials/matched_stimuli/sl_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(sl_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/sl_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/sl_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/sl_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/sl_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/sl_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/sl_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/sl_answered_prime_summary_no3.0.csv", row.names = F)
```

---
title: "Data Processing - Hungarian"
...
# get all the files
hu_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
hu_files <- hu_files[grepl("hu", hu_files)]

hu_data <- list()

# open all files
for (i in 1:length(hu_files)){
  hu_data[[i]] <- processData(hu_files[i])
  hu_data[[i]]$url_lab <- as.character(hu_data[[i]]$url_lab)
}

# convert to data frame and make unique
hu_data_all <- bind_rows(hu_data) %>% unique()
...
hu_data_all <- fill(hu_data_all, uuid, .direction = "down")
...
hu_data_all$uuid[hu_data_all$observation == "df459"] <- "3b61e4c8-1083-496b-b2bd-6dd651022d08"

hu_data_all$observation <- paste0(hu_data_all$observation, substr(hu_data_all$uuid, 20, 30))

# make sure every row has an id number 
sum(is.na(hu_data_all$observation))
...
```{r write-full, eval = F}
write.csv(hu_data_all, 
       gzfile("output_data/full_data/hu_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
hu_data_all <- read.csv("output_data/full_data/hu_full_data.csv.gz")
```
...
# consent forms are duplicated by seeing two of them
# mark them all as 60 lab to ensure they stay
# only one lab
hu_data_all$url_lab[is.na(hu_data_all$url_lab)] <- "60"
...
export(participant_DF, "output_data/participant_data/hu_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(hu_data_all)[ c(6, 8:37, 41:42) ]
...
hu_data_all[ hu_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/hu_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/hu/hu_words.csv")
...
stimuli_data %>% 
  select(target_type, hu_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, hu_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$hu_cue, stimuli_data$hu_target)))
...
length(unique(c(stimuli_data$hu_cue, stimuli_data$hu_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$hu_cue, stimuli_data$hu_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/hu_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/hu_answered_item_data.csv", row.names = F)
```

## Priming Data

- 5/19, no misaligned trials. 
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$hu_cue, stimuli_data$hu_target)
  ...
  export(priming_focus, "output_data/priming_data/hu_prime_trials.csv", row.names = F)
  ...
  hu_num_trials <- hu_num_trials %>% 
    full_join(hu_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/hu_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    full_join(hu_num_trials, by = c("word_combo" = "word_combo")) %>% 
      # deal with mingling
      mutate(type = ifelse(word_combo == "mingling", "related", type),
             target_word = ifelse(word_combo == "mingling", "ling", target_word)) %>% 
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    # deal with mingling
      mutate(type = ifelse(word_combo == "mingling", "related", type),
             target_word = ifelse(word_combo == "mingling", "ling", target_word)) %>% 
    # merge with the number of trials data
    left_join(hu_num_trials, by = c("word_combo" = "word_combo")) %>% 
    # deal with duplicates because of translation
    arrange(target_word, type) %>% 
      group_by(type) %>% 
      mutate(target_word_unique = make.unique(target_word)) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/hu_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/hu_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/hu_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/hu_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/hu_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/hu_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/hu_answered_prime_summary_no3.0.csv", row.names = F)
```

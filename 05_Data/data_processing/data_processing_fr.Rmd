...
# get all the files
fr_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
fr_files <- fr_files[grepl("fr", fr_files)]

fr_data <- list()

# open all files
for (i in 1:length(fr_files)){
  fr_data[[i]] <- processData(fr_files[i])
  fr_data[[i]]$url_lab <- as.character(fr_data[[i]]$url_lab)
}

# convert to data frame and make unique
fr_data_all <- bind_rows(fr_data) %>% unique()
...
fr_data_all <- fill(fr_data_all, uuid, .direction = "down")
...
fr_data_all$observation <- paste0(fr_data_all$observation, substr(fr_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(fr_data_all$observation))
...
```{r write-full, eval = F}
write.csv(fr_data_all, 
       gzfile("output_data/full_data/fr_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
fr_data_all <- read.csv("output_data/full_data/fr_full_data.csv.gz")
```
...
fr_data_all$url_lab[fr_data_all$url_lab == "9999" | 
                      is.na(fr_data_all$url_lab)] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/fr_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(fr_data_all)[ c(6, 8:37, 41:42) ]
...
fr_data_all[ fr_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials,
          gzfile("output_data/trial_data/fr_trial_data.csv.gz"),
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/fr/fr_words.csv")
...
stimuli_data %>% 
  select(target_type, fr_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, fr_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$fr_cue, stimuli_data$fr_target)))
...
length(unique(c(stimuli_data$fr_cue, stimuli_data$fr_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$fr_cue, stimuli_data$fr_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/fr_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/fr_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$fr_cue, stimuli_data$fr_target)
  priming_focus <- merge(priming_focus, stimuli_data[ , c("type", "word_combo")], 
                    by = "word_combo", all.x = T)

  # this was an issue with non-randomization scrambling 
  temp <- priming_focus %>% 
    filter(is.na(type))
  nrow(temp)
  ...
  export(priming_focus, "output_data/priming_data/fr_prime_trials.csv", row.names = F)
  ...
  fr_num_trials <- fr_num_trials %>% 
    full_join(fr_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/fr_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(fr_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/fr_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/fr_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/fr_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/fr_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/fr_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/fr_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/fr_answered_prime_summary_no3.0.csv", row.names = F)
```

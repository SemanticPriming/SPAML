...
# get all the files
tr_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
tr_files <- tr_files[grepl("tr", tr_files)]

tr_info <- file.info(tr_files)
tr_files <- tr_files[ tr_info$size > 1531904]

tr_data <- list()

# open all files
for (i in 1:length(tr_files)){
  tr_data[[i]] <- processData(tr_files[i])
  tr_data[[i]]$url_lab <- as.character(tr_data[[i]]$url_lab)
}

# convert to data frame and make unique
tr_data_all <- bind_rows(tr_data) %>% unique()
...
# remove test data before official start turkish
tr_data_all <- tr_data_all %>% 
...
tr_data_all <- fill(tr_data_all, uuid, .direction = "down")
...
tr_data_all$uuid[tr_data_all$observation == "b5503"] <- "b859c701-b404-4499-8f7a-324c20c4645b"

tr_data_all$observation <- paste0(tr_data_all$observation, substr(tr_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(tr_data_all$observation))

# remove ZPID, mturk, uuid 
tr_data_all <- tr_data_all %>% 
  select(-url_special_code, -uuid, 
         -meta_location, -x, -url, 
         -url_special_code222526045147377, 
         -url_mbstx, -url_fbclid)
...
```{r write-full, eval = F}
write.csv(tr_data_all, 
       gzfile("output_data/full_data/tr_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
tr_data_all <- read.csv("output_data/full_data/tr_full_data.csv.gz")
```
...
tr_data_all$url_lab <- gsub("[[:punct:]]", "", tr_data_all$url_lab)
tr_data_all$url_lab <- gsub("387293", "387", tr_data_all$url_lab)
tr_data_all$url_lab[is.na(tr_data_all$url_lab)] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/tr_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(tr_data_all)[ c(6, 8:37, 41:42) ]
...
tr_data_all[ tr_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/tr_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/tr/tr_words.csv")
...
stimuli_data %>% 
  select(target_type, tr_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, tr_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$tr_cue, stimuli_data$tr_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$tr_cue, stimuli_data$tr_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$tr_cue, stimuli_data$tr_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/tr_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/tr_answered_item_data.csv", row.names = F)
```

## Priming Data

- 5/31: 57 rows due to typos and one person who did not answer anything
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$tr_cue, stimuli_data$tr_target)
  ...
  export(priming_focus, "output_data/priming_data/tr_prime_trials.csv", row.names = F)
  ...
  tr_num_trials <- tr_num_trials %>% 
    full_join(tr_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
 ...
 priming_trial_paired <- priming_Z %>% 
  ...
  export(., file = "../../03_Materials/matched_stimuli/tr_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(tr_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/tr_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/tr_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/tr_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/tr_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/tr_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/tr_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/tr_answered_prime_summary_no3.0.csv", row.names = F)
```

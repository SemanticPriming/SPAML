...
# get all the files
nl_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
nl_files <- nl_files[grepl("nl", nl_files)]

nl_data <- list()

# open all files
for (i in 1:length(nl_files)){
  nl_data[[i]] <- processData(nl_files[i])
  nl_data[[i]]$url_lab <- as.character(nl_data[[i]]$url_lab)
}

# convert to data frame and make unique
nl_data_all <- bind_rows(nl_data) %>% unique()
...
nl_data_all <- fill(nl_data_all, uuid, .direction = "down")
...
nl_data_all$observation <- paste0(nl_data_all$observation, substr(nl_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(nl_data_all$observation))
...
```{r write-full, eval = F}
write.csv(nl_data_all, 
       gzfile("output_data/full_data/nl_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
nl_data_all <- read.csv("output_data/full_data/nl_full_data.csv.gz")
```
...
nl_data_all$url_lab[is.na(nl_data_all$url_lab)] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/nl_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(nl_data_all)[ c(6, 8:37, 41:42) ]
...
nl_data_all[ nl_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/nl_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/nl/nl_words.csv")
...
stimuli_data %>% 
  select(target_type, nl_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, nl_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$nl_cue, stimuli_data$nl_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$nl_cue, stimuli_data$nl_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$nl_cue, stimuli_data$nl_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/nl_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/nl_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$nl_cue, stimuli_data$nl_target)
  ...
  export(priming_focus, "output_data/priming_data/nl_prime_trials.csv", row.names = F)
  ...
  nl_num_trials <- nl_num_trials %>% 
    full_join(nl_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/nl_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(nl_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/nl_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/nl_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/nl_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/nl_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/nl_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/nl_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/nl_answered_prime_summary_no3.0.csv", row.names = F)
```

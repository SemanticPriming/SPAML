---
title: "Data Processing - Combined Portuguese"
...
# get all the files
pt_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
...
for (i in 1:length(pt_files)){
  ...
  pt_data[[i]]$url_lab <- as.character(pt_data[[i]]$url_lab)
  pt_data[[i]] <- pt_data[[i]] %>% 
    mutate(across(matches(c("portuguese_sona", "url_portuguese_sona", "url_special_code")), as.character))
}
...
pt_data_all <- fill(pt_data_all, uuid, .direction = "down")
...
pt_data_all$observation <- paste0(pt_data_all$observation, substr(pt_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(pt_data_all$observation))

# remove ZPID, mturk, uuid 
pt_data_all <- pt_data_all %>% 
  select(-url_special_code, -uuid, -meta_location, -x, -url,
         -url_authuser, -clickworker, -url_user, 
         -url_user_id, -portuguese_sona, -url_portuguese_sona, 
         -url_prolific_pid, -url_study_id, -url_session_id,
         -url_fbclid)
...
```{r write-full, eval = F}
write.csv(pt_data_all, 
       gzfile("output_data/full_data/pt_combo_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
pt_data_all <- read.csv("output_data/full_data/pt_combo_full_data.csv.gz")
```
...
pt_data_all$url_lab <- gsub("[[:punct:]]", "", pt_data_all$url_lab)
pt_data_all$url_lab[pt_data_all$url_lab == "152httpspsa007psysciaccorgbrptlab152"] <- "152"
pt_data_all$url_lab[pt_data_all$url_lab == "1697httpspsa007psysciaccorgbrptlab1697"] <- "1697"
pt_data_all$url_lab[pt_data_all$url_lab == "1679"] <- "1697"
pt_data_all$url_lab[pt_data_all$url_lab == "1967"] <- "1697"
pt_data_all$url_lab[pt_data_all$url_lab == "52"] <- "152"
...
export(participant_DF, "output_data/participant_data/pt_combo_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(pt_data_all)[ c(6, 8:37, 41:42) ]
...
pt_data_all[ pt_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/pt_combo_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data_pt <- import("../../04_Procedure/pt/pt_words.csv") %>% 
  mutate(pt_cue = gsub("seqüestrar", "sequestrar", pt_cue),
         word_combo = paste0(pt_cue, pt_target))
stimuli_data_br_pt <- import("../../04_Procedure/br_pt/br_pt_words.csv") %>% 
  mutate(pt_cue = gsub("seqüestrar", "sequestrar", pt_cue),
         word_combo = paste0(pt_cue, pt_target))

# overall numbers of stimuli 
nrow(stimuli_data_pt)
stimuli_data_pt %>% 
  group_by(type, cue_type, target_type) %>% 
  summarize(n = n(), .groups = "keep")

stimuli_data_pt %>% 
  select(target_type, pt_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data_pt %>% 
  select(cue_type, pt_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())

nrow(stimuli_data_br_pt)
stimuli_data_br_pt %>% 
  group_by(type, cue_type, target_type) %>% 
  summarize(n = n(), .groups = "keep")
...
# examine words that will get excluded for issues -- overall
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data_pt$pt_cue,
                       stimuli_data_pt$pt_target,
                       stimuli_data_br_pt$pt_cue, 
                       stimuli_data_br_pt$pt_target)))

temp

##remove words that aren't part of the real data (testers)
item_data_pt <- item_data %>% 
  filter(word %in% c(stimuli_data_pt$pt_cue, stimuli_data_pt$pt_target))

nrow(item_data)
nrow(item_data_pt)
length(unique(c(stimuli_data_pt$pt_cue, stimuli_data_pt$pt_target)))
setdiff(unique(c(stimuli_data_pt$pt_cue, stimuli_data_pt$pt_target)), item_data_pt$word)

##remove words that aren't part of the real data (testers)
item_data_br_pt <- item_data %>% 
  filter(word %in% c(stimuli_data_br_pt$pt_cue, stimuli_data_br_pt$pt_target))

nrow(item_data_br_pt)
length(unique(c(stimuli_data_br_pt$pt_cue, stimuli_data_br_pt$pt_target)))
setdiff(unique(c(stimuli_data_br_pt$pt_cue, stimuli_data_br_pt$pt_target)), item_data_br_pt$word)

# pt words
pt_words <- stimuli_data_pt %>% 
           filter(type != "nonword") %>% 
           pull(word_combo)

# overlap trials
stimuli_data_br_pt %>%
  filter(word_combo %in% pt_words) %>% 
  nrow()
...
#merge together two z score calculations
item_data_combo_pt <- item_data_pt %>% 
  left_join(item_data_2.5, 
            by = c("word" = "word", "class" = "class")) %>% 
  left_join(item_data_3.0, 
            by = c("word" = "word", "class" = "class")) 

nrow(item_data_combo_pt)

#merge together two z score calculations
item_data_combo_br_pt <- item_data_br_pt %>% 
  left_join(item_data_2.5, 
            by = c("word" = "word", "class" = "class")) %>% 
  left_join(item_data_3.0, 
            by = c("word" = "word", "class" = "class")) 

nrow(item_data_combo_br_pt)
```
...
write.csv(item_data_combo, "output_data/item_data/pt_combo_item_data.csv", row.names = F)
write.csv(item_data_combo_pt, "output_data/item_data/pt_pt_combo_item_data.csv", row.names = F)
write.csv(item_data_combo_br_pt, "output_data/item_data/br_pt_combo_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/pt_combo_answered_item_data.csv", row.names = F)
write.csv(item_data_combo_pt, "output_data/item_data/pt_pt_combo_answered_item_data.csv", row.names = F)
write.csv(item_data_combo_br_pt, "output_data/item_data/br_pt_combo_answered_item_data.csv", row.names = F)
```

## Priming Data

- Observation 6f7958710-02a7 is misaligned but did not answer a single trials
- Observation 1069f9080-918a is missing trials between 07_00_062_1 and 07_00_067_1 - leaving them in because it aligns everything by leaving them and it will exclude that fake "priming" trial. 
...
# figure out trial type ----
  ...
  stimuli_data_pt$word_combo <- paste0(stimuli_data_pt$pt_cue, stimuli_data_pt$pt_target)
  priming_focus_pt <- merge(priming_focus, stimuli_data_pt[ , c("type", "word_combo")], 
                    by = "word_combo", all.x = T)
  
  nrow(priming_focus)
  nrow(priming_focus_pt)
  table(priming_focus_pt$type, useNA = "ifany")
  
  stimuli_data_br_pt$word_combo <- paste0(stimuli_data_br_pt$pt_cue, stimuli_data_br_pt$pt_target)
  priming_focus_br_pt <- merge(priming_focus, stimuli_data_br_pt[ , c("type", "word_combo")], 
                    by = "word_combo", all.x = T)
  nrow(priming_focus)
  nrow(priming_focus_br_pt)
  table(priming_focus_br_pt$type, useNA = "ifany")
  
  # here the NA values are the cross listed stuff that occurs 
  # in one language but not the other 
  # remember this is where you check unlined up stuff
  temp <- unique(c(priming_focus_pt$word_combo[is.na(priming_focus_pt$type)], 
            priming_focus_br_pt$word_combo[is.na(priming_focus_br_pt$type)]))
  
  temp2 <- unique(c(stimuli_data_pt$word_combo[stimuli_data_pt$cue_type == "word" & stimuli_data_pt$target_type == "word"],
      stimuli_data_br_pt$word_combo[stimuli_data_br_pt$cue_type == "word" & stimuli_data_br_pt$target_type == "word"]))
  
  na_values <- setdiff(temp, temp2)
  temp_DF <- priming_focus %>% filter(word_combo %in% na_values) %>% 
    filter(word_combo != "NANA") %>% 
    filter(observation != "6f7958710-02a7") %>% 
    filter(observation != "1069f9080-918a")
  nrow(temp_DF)
  
  # subset out NAs at some point they will be practice trials
  priming_focus_pt <- subset(priming_focus_pt, !is.na(type))
  priming_focus_br_pt <- subset(priming_focus_br_pt, !is.na(type))
  
  nrow(priming_focus_pt)
  table(priming_focus_pt$type, useNA = "ifany")
  
  export(priming_focus_br_pt, "output_data/priming_data/br_pt_combo_prime_trials.csv", row.names = F)
  export(priming_focus_pt, "output_data/priming_data/pt_combo_prime_trials.csv", row.names = F)
  
  temp <- priming_focus_pt %>% 
...
```{r}
  ...
  pt_num_trials <- priming_focus_pt %>% 
    group_by(word_combo) %>% 
    summarize(
  ...
  pt_num_trials <- pt_num_trials %>% 
    full_join(pt_num_trials2, by = "word_combo")
  
  # subset out NAs they are test / practice trials
  # only correct answers and trials to keep
  priming_Z <- priming_focus_pt %>% 
    filter(!is.na(type)) %>% 
...
# Calculate Statistics ----------------------------------------------------
  ...
  priming_trial_paired <- priming_Z %>% 
  select(word_combo, cue_word, type, target_word) %>% 
  unique() %>% 
  ...
  pivot_wider(
    ...
    values_from = c("word_combo", "cue_word", "target_word")
  ) %>% 
  select(-target_word_related, -target_word_unrelated) %>% 
  export(., file = "../../03_Materials/matched_stimuli/pt_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(pt_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/pt_combo_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/pt_combo_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/pt_combo_prime_summary_no3.0.csv", row.names = F)
```

#### Calculate BR PT Only

```{r}
  # calculate the total N versus timeout N
  br_pt_num_trials <- priming_focus_br_pt %>% 
    group_by(word_combo) %>% 
    summarize(
              # target_correct = sum(target_correct, na.rm = T),
              target_answeredN = sum(target_end_of_trial == "response", na.rm = T), 
  ...
  br_pt_num_trials <- br_pt_num_trials %>% 
    full_join(br_pt_num_trials2, by = "word_combo")
  
  # subset out NAs they are test / practice trials
  # only correct answers and trials to keep
  priming_Z <- priming_focus_br_pt %>% 
    filter(!is.na(type)) %>% 
    filter(keep_target == "keep") %>% # to exclude correct trials too short
    filter(target_correct == TRUE) %>% # to make sure only correct trials 
    filter(keep_participant == "keep") # participants to keep
...
# Calculate Statistics ----------------------------------------------------
  ...
  select(-target_word_related, -target_word_unrelated) %>% 
  export(., file = "../../03_Materials/matched_stimuli/br_pt_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
            ...
            .groups = "keep") %>% 
    # merge with the number of trials data
    left_join(br_pt_num_trials, by = c("word_combo" = "word_combo")) %>% 
    # deal with duplicates because of translation
    arrange(target_word, type) %>% 
      group_by(type) %>% 
      mutate(target_word_unique = make.unique(target_word)) %>% 
...
#### Write out Original Scores BR PT
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/br_pt_combo_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/br_pt_combo_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/br_pt_combo_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus_br_pt, "output_data/priming_data/br_pt_combo_answered_prime_trials.csv", row.names = F)
  export(priming_focus_pt, "output_data/priming_data/pt_combo_answered_prime_trials.csv", row.names = F)
```

#### Calculate PT Only Redefined
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/pt_combo_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/pt_combo_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/pt_combo_answered_prime_summary_no3.0.csv", row.names = F)
```

#### Calculate BR PT Only Redefined
...
#### Write out Redefined Scores BR PT
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/br_pt_combo_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/br_pt_combo_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/br_pt_combo_answered_prime_summary_no3.0.csv", row.names = F)
```

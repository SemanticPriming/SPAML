...
# get all the files
fa_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
fa_files <- fa_files[grepl("fa", fa_files)]

fa_data <- list()

# open all files
for (i in 1:length(fa_files)){
  fa_data[[i]] <- processData(fa_files[i])
  fa_data[[i]]$url_lab <- as.character(fa_data[[i]]$url_lab)
}

# convert to data frame and make unique
fa_data_all <- bind_rows(fa_data) %>% unique()
...
fa_data_all <- fill(fa_data_all, uuid, .direction = "down")
...
fa_data_all$uuid[fa_data_all$observation == "ceae4"] <- "801fa290-adf2-4a90-8490-a32415b48544"

fa_data_all$observation <- paste0(fa_data_all$observation, substr(fa_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(fa_data_all$observation))

# remove ZPID, mturk, uuid 
fa_data_all <- fa_data_all %>% 
  select(-url_special_code, -uuid, -meta_location, 
         -x, -url, -url_fbclid, -url_lab_1990)
...
```{r write-full, eval = F}
write.csv(fa_data_all, 
       gzfile("output_data/full_data/fa_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
fa_data_all <- read.csv("output_data/full_data/fa_full_data.csv.gz")
```
...
fa_data_all$url_lab[is.na(fa_data_all$url_lab)] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/fa_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(fa_data_all)[ c(6, 8:37, 41:42) ]
...
fa_data_all[ fa_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials,
          gzfile("output_data/trial_data/fa_trial_data.csv.gz"),
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/fa/fa_words.csv")
...
stimuli_data %>% 
  select(target_type, fa_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, fa_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$fa_cue, stimuli_data$fa_target)))
...
length(unique(c(stimuli_data$fa_cue, stimuli_data$fa_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$fa_cue, stimuli_data$fa_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/fa_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/fa_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$fa_cue, stimuli_data$fa_target)
  ...
  export(priming_focus, "output_data/priming_data/fa_prime_trials.csv", row.names = F)
  ...
  fa_num_trials <- fa_num_trials %>% 
    full_join(fa_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/fa_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(fa_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/fa_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/fa_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/fa_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/fa_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/fa_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/fa_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/fa_answered_prime_summary_no3.0.csv", row.names = F)
```

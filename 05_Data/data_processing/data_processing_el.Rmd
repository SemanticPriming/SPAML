...
# get all the files
el_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
el_files <- el_files[grepl("el", el_files)]

el_data <- list()

# open all files
for (i in 1:length(el_files)){
  el_data[[i]] <- processData(el_files[i])
  el_data[[i]] <- el_data[[i]] %>% 
    mutate(across(matches(c("url_lab", "url_special_code")), as.character))
}

# convert to data frame and make unique
el_data_all <- bind_rows(el_data) %>% unique()
...
el_data_all <- fill(el_data_all, uuid, .direction = "down")
...
el_data_all$uuid[el_data_all$observation == "67589"] <- "6e7b6418-5b1c-465e-82d1-d5c303c92839"

el_data_all$uuid[el_data_all$observation == "f2b5f"] <- "f386cdee-0838-4a06-a8ae-9458ea08f9b4"

el_data_all$uuid[el_data_all$observation == "2713f"] <- "a4fb59ca-d0b0-4344-b906-b9212debfb0c"

# last observation is two different ones, so allow merge

el_data_all$observation <- paste0(el_data_all$observation, substr(el_data_all$uuid, 20, 30))

# make sure every row has an id number 
sum(is.na(el_data_all$observation))
...
```{r write-full, eval = F}
write.csv(el_data_all, 
       gzfile("output_data/full_data/el_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
el_data_all <- read.csv("output_data/full_data/el_full_data.csv.gz")
```
...
# no lab 13, unclear
el_data_all$url_lab[el_data_all$url_lab == "13"] <- "No Lab ID"
el_data_all$url_lab[el_data_all$url_lab == "999"] <- "No Lab ID"
el_data_all$url_lab[el_data_all$url_lab == "762"] <- "761"
...
export(participant_DF, "output_data/participant_data/el_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(el_data_all)[ c(6, 8:37, 41:42) ]
...
el_data_all[ el_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials,
          gzfile("output_data/trial_data/el_trial_data.csv.gz"),
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/el/el_words.csv")
...
stimuli_data %>% 
  select(target_type, el_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, el_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$el_cue, stimuli_data$el_target)))
...
length(unique(c(stimuli_data$el_cue, stimuli_data$el_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$el_cue, stimuli_data$el_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/el_item_data.csv", row.names = F)
```
...
##write out item level data 
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/el_answered_item_data.csv", row.names = F)
```

## Priming Data

- Checked 5/17: after mid-February 2023 several incorrect trials were fixed due to typos or other issues with the stimuli. Therefore, there are 545 trials that were discarded because they no longer matched the corrected stimuli.  
- Same results 12/14
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$el_cue, stimuli_data$el_target)
  ...
  export(priming_focus, "output_data/priming_data/el_prime_trials.csv", row.names = F)
  ...
  el_num_trials <- el_num_trials %>% 
    full_join(el_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/el_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(el_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/el_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/el_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/el_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/el_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/el_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/el_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/el_answered_prime_summary_no3.0.csv", row.names = F)
```

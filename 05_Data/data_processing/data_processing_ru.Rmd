...
# get all the files
ru_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
ru_files <- ru_files[grepl("ru", ru_files)]

ru_data <- list()

# open all files
for (i in 1:length(ru_files)){
  ru_data[[i]] <- processData(ru_files[i])
  ru_data[[i]]$url_lab <- as.character(ru_data[[i]]$url_lab)
}

# convert to data frame and make unique
ru_data_all <- bind_rows(ru_data) %>% unique()
...
ru_data_all <- fill(ru_data_all, uuid, .direction = "down")
...
ru_data_all$observation <- paste0(ru_data_all$observation, substr(ru_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(ru_data_all$observation))
...
```{r write-full, eval = F}
write.csv(ru_data_all, 
       gzfile("output_data/full_data/ru_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
ru_data_all <- read.csv("output_data/full_data/ru_full_data.csv.gz")
```
...
ru_data_all$url_lab[is.na(ru_data_all$url_lab)] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/ru_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(ru_data_all)[ c(6, 8:37, 41:42) ]
...
ru_data_all[ ru_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
real_trials_NA <- real_trials %>% #data frame
  filter(is.na(correct) | #grab time out trials OR
           is.na(duration) | #deal with weird data storage
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/ru_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/ru/ru_words.csv")
...
stimuli_data %>% 
  select(target_type, ru_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, ru_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$ru_cue, stimuli_data$ru_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$ru_cue, stimuli_data$ru_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$ru_cue, stimuli_data$ru_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/ru_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/ru_answered_item_data.csv", row.names = F)
```

## Priming Data

- 5/30: 125 rows included typos/misspelled original words 
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$ru_cue, stimuli_data$ru_target)
  ...
  export(priming_focus, "output_data/priming_data/ru_prime_trials.csv", row.names = F)
  ...
  ru_num_trials <- ru_num_trials %>% 
    full_join(ru_num_trials2)
...
priming_trial_paired <- priming_Z %>% 
  ...
  export(., file = "../../03_Materials/matched_stimuli/ru_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(ru_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/ru_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/ru_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/ru_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/ru_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/ru_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/ru_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/ru_answered_prime_summary_no3.0.csv", row.names = F)
```

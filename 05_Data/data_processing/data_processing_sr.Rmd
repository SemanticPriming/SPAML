...
```{r import_data, eval = F, messsage = F}
# get all the files
sr_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
sr_files <- sr_files[grepl("sr", sr_files)]

sr_data <- list()

# open all files
for (i in 1:length(sr_files)){
  sr_data[[i]] <- processData(sr_files[i])
  sr_data[[i]]$url_lab <- as.character(sr_data[[i]]$url_lab)
}

# convert to data frame and make unique
sr_data_all <- bind_rows(sr_data) %>% unique()
...
sr_data_all <- fill(sr_data_all, uuid, .direction = "down")
...
sr_data_all$observation <- paste0(sr_data_all$observation, substr(sr_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(sr_data_all$observation))
...
```{r write-full, eval = F}
write.csv(sr_data_all, 
       gzfile("output_data/full_data/sr_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
sr_data_all <- read.csv("output_data/full_data/sr_full_data.csv.gz")
```
...
sr_data_all$url_lab[is.na(sr_data_all$url_lab)] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/sr_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(sr_data_all)[ c(6, 8:37, 41:42) ]
...
sr_data_all[ sr_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/sr_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/sr/sr_words.csv")
...
stimuli_data %>% 
  select(target_type, sr_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, sr_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$sr_cue, stimuli_data$sr_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$sr_cue, stimuli_data$sr_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$sr_cue, stimuli_data$sr_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/sr_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/sr_answered_item_data.csv", row.names = F)
```

## Priming Data

- Checked 5/30: no misaligned trials 
- Checked 12/14: no misaligned trials 
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$sr_cue, stimuli_data$sr_target)
  ...
  export(priming_focus, "output_data/priming_data/sr_prime_trials.csv", row.names = F)
  ...
  sr_num_trials <- sr_num_trials %>% 
    full_join(sr_num_trials2, by = "word_combo")
...
# Calculate Statistics ----------------------------------------------------
 ...
 priming_trial_paired <- priming_Z %>% 
  ...
  export(., file = "../../03_Materials/matched_stimuli/sr_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(sr_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/sr_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/sr_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/sr_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/sr_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/sr_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/sr_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/sr_answered_prime_summary_no3.0.csv", row.names = F)
```

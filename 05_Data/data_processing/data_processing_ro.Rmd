---
title: "Data Processing - Romanian"
...
# get all the files
ro_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
ro_files <- ro_files[grepl("/ro.*", ro_files)]

ro_data <- list()

# open all files
for (i in 1:length(ro_files)){
  ro_data[[i]] <- processData(ro_files[i])
  ro_data[[i]]$url_lab <- as.character(ro_data[[i]]$url_lab)
  ro_data[[i]] <- ro_data[[i]] %>% 
    mutate(across(matches(c("url_special_code", "special_code")), as.character))
}

# convert to data frame and make unique
ro_data_all <- bind_rows(ro_data) %>% unique()
...
# fix misspelling/merge issue
ro_data_all$word <- gsub("răbdător, pacient", "pacient", ro_data_all$word)
```
...
ro_data_all <- fill(ro_data_all, uuid, .direction = "down")
...
ro_data_all$uuid[ro_data_all$observation == "a7d18"] <- "318975ee-f43e-46d4-95e3-03443c536eda"

ro_data_all$observation <- paste0(ro_data_all$observation, substr(ro_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(ro_data_all$observation))

# remove ZPID, mturk, uuid 
ro_data_all <- ro_data_all %>% 
  select(-url_special_code, -uuid, 
         -meta_location, -x, -url, -url_fbclid,
         -return, -special_code)
...
```{r write-full, eval = F}
write.csv(ro_data_all, 
       gzfile("output_data/full_data/ro_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
ro_data_all <- read.csv("output_data/full_data/ro_full_data.csv.gz")
```
...
ro_data_all$url_lab[is.na(ro_data_all$url_lab)] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/ro_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(ro_data_all)[ c(6, 8:37, 41:42) ]
...
ro_data_all[ ro_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/ro_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/ro/ro_words.csv")
...
stimuli_data %>% 
  select(target_type, ro_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, ro_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$ro_cue, stimuli_data$ro_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$ro_cue, stimuli_data$ro_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$ro_cue, stimuli_data$ro_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/ro_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/ro_answered_item_data.csv", row.names = F)
```

## Priming Data

```{r fix-trials}
# fix missing misalignment 
real_trials <- real_trials %>% 
  filter(!(observation == "3ffcda6e6-ec7a" &  fix_sender == "05_00_057_1"))
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$ro_cue, stimuli_data$ro_target)
  ...
  export(priming_focus, "output_data/priming_data/ro_prime_trials.csv", row.names = F)
  ...
  ro_num_trials <- ro_num_trials %>% 
    full_join(ro_num_trials2, by = "word_combo")
...
priming_trial_paired <- priming_Z %>% 
  ...
  export(., file = "../../03_Materials/matched_stimuli/ro_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(ro_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/ro_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/ro_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/ro_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/ro_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/ro_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/ro_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/ro_answered_prime_summary_no3.0.csv", row.names = F)
```

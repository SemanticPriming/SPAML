...
# get all the files
he_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
he_files <- he_files[grepl("he", he_files)]

he_data <- list()

# open all files
for (i in 1:length(he_files)){
  he_data[[i]] <- processData(he_files[i])
  he_data[[i]]$url_lab <- as.character(he_data[[i]]$url_lab)
}

# convert to data frame and make unique
he_data_all <- bind_rows(he_data) %>% unique()
...
he_data_all <- fill(he_data_all, uuid, .direction = "down")
...
he_data_all$observation <- paste0(he_data_all$observation, substr(he_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(he_data_all$observation))
...
```{r write-full, eval = F}
write.csv(he_data_all, 
       gzfile("output_data/full_data/he_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
he_data_all <- read.csv("output_data/full_data/he_full_data.csv.gz")
```
...
# no other labs
he_data_all$url_lab[is.na(he_data_all$url_lab)] <- "2412"
...
export(participant_DF, "output_data/participant_data/he_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(he_data_all)[ c(6, 8:37, 41:42) ]
...
he_data_all[ he_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/he_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/he/he_words.csv")
...
stimuli_data %>% 
  select(target_type, he_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, he_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$he_cue, stimuli_data$he_target)))
...
length(unique(c(stimuli_data$he_cue, stimuli_data$he_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$he_cue, stimuli_data$he_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/he_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/he_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$he_cue, stimuli_data$he_target)
  ...
  export(priming_focus, "output_data/priming_data/he_prime_trials.csv", row.names = F)
  ...
  he_num_trials <- he_num_trials %>% 
    full_join(he_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/he_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(he_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/he_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/he_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/he_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/he_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/he_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/he_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/he_answered_prime_summary_no3.0.csv", row.names = F)
```

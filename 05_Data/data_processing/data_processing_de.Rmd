...
# get all the files
de_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
...
for (i in 1:length(de_files)){
  de_data[[i]] <- processData(de_files[i])
  de_data[[i]]$url_lab <- as.character(de_data[[i]]$url_lab)
  de_data[[i]]$url_special_code <- as.character(de_data[[i]]$url_special_code)
}

# convert to data frame and make unique
de_data_all <- bind_rows(de_data) %>% unique()
...
de_data_all <- fill(de_data_all, uuid, .direction = "down")
...
# fix one duplicate
de_data_all$uuid[de_data_all$observation == "e6c28"] <- "28f51d36-56fd-4d9d-a038-46567e30c854"

de_data_all$observation <- paste0(de_data_all$observation, substr(de_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(de_data_all$observation))
...
```{r write-full, eval = F}
write.csv(de_data_all, 
       gzfile("output_data/full_data/de_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
de_data_all <- read.csv("output_data/full_data/de_full_data.csv.gz")
```
...
# 106 is italian
de_data_all$url_lab[de_data_all$url_lab == "106"] <- "1060"
de_data_all$url_lab[de_data_all$url_lab == "10604"] <- "1060"

# 2970 is a typo
de_data_all$url_lab[de_data_all$url_lab == "2970"] <- "2790"

# missing 
de_data_all$url_lab[is.na(de_data_all$url_lab)] <- "No Lab ID"
de_data_all$url_lab[de_data_all$url_lab == "9999"] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/de_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(de_data_all)[ c(6, 8:37, 41:42) ]
...
de_data_all[ de_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials,
          gzfile("output_data/trial_data/de_trial_data.csv.gz"),
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/de/de_words.csv")
...
stimuli_data %>% 
  select(target_type, de_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, de_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$de_cue, stimuli_data$de_target)))
...
length(unique(c(stimuli_data$de_cue, stimuli_data$de_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$de_cue, stimuli_data$de_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/de_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/de_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$de_cue, stimuli_data$de_target)
  ...
  export(priming_focus, "output_data/priming_data/de_prime_trials.csv", row.names = F)
  ...
  de_num_trials <- de_num_trials %>% 
    full_join(de_num_trials2)
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/de_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(de_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/de_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/de_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/de_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/de_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/de_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/de_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/de_answered_prime_summary_no3.0.csv", row.names = F)
```

...
# get all the files
sk_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
sk_files <- sk_files[grepl("sk", sk_files)]

sk_data <- list()

# open all files
for (i in 1:length(sk_files)){
  sk_data[[i]] <- processData(sk_files[i])
  sk_data[[i]]$url_lab <- as.character(sk_data[[i]]$url_lab)
}

# convert to data frame and make unique
sk_data_all <- bind_rows(sk_data) %>% unique()
...
sk_data_all <- sk_data_all %>%
  filter(timestamp > as.POSIXct("2023-03-19"))
...
sk_data_all <- fill(sk_data_all, uuid, .direction = "down")
...
sk_data_all$observation <- paste0(sk_data_all$observation, substr(sk_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(sk_data_all$observation))
...
```{r write-full, eval = F}
write.csv(sk_data_all, 
       gzfile("output_data/full_data/sk_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
sk_data_all <- read.csv("output_data/full_data/sk_full_data.csv.gz")
```
...
sk_data_all$url_lab[sk_data_all$url_lab == "245ň"] <- "245"
...
export(participant_DF, "output_data/participant_data/sk_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(sk_data_all)[ c(6, 8:37, 41:42) ]
...
sk_data_all[ sk_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/sk_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/sk/sk_words.csv")
...
stimuli_data %>% 
  select(target_type, sk_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, sk_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$sk_cue, stimuli_data$sk_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$sk_cue, stimuli_data$sk_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$sk_cue, stimuli_data$sk_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/sk_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/sk_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$sk_cue, stimuli_data$sk_target)
  ...
  export(priming_focus, "output_data/priming_data/sk_prime_trials.csv", row.names = F)
  ...
  sk_num_trials <- sk_num_trials %>% 
    full_join(sk_num_trials2)
...
priming_trial_paired <- priming_Z %>% 
  ...
  export(., file = "../../03_Materials/matched_stimuli/sk_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(sk_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/sk_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/sk_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/sk_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/sk_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/sk_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/sk_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/sk_answered_prime_summary_no3.0.csv", row.names = F)
```

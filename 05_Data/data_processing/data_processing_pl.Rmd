...
# get all the files
pl_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
pl_files <- pl_files[grepl("pl", pl_files)]

pl_data <- list()

# open all files
for (i in 1:length(pl_files)){
  pl_data[[i]] <- processData(pl_files[i])
  pl_data[[i]]$url_lab <- as.character(pl_data[[i]]$url_lab)
}

# convert to data frame and make unique
pl_data_all <- bind_rows(pl_data) %>% unique()
...
pl_data_all <- fill(pl_data_all, uuid, .direction = "down")
...
pl_data_all$uuid[pl_data_all$observation == "e2328"] <- "ca32f2f8-b3fd-4b62-843d-2b0cbcb2ed28"

pl_data_all$observation <- paste0(pl_data_all$observation, substr(pl_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(pl_data_all$observation))
...
```{r write-full, eval = F}
write.csv(pl_data_all, 
       gzfile("output_data/full_data/pl_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
pl_data_all <- read.csv("output_data/full_data/pl_full_data.csv.gz")
```
...
pl_data_all$url_lab <- gsub("[[:punct:]]", "", pl_data_all$url_lab)
pl_data_all$url_lab[pl_data_all$url_lab == "10"] <- "101"
pl_data_all$url_lab[pl_data_all$url_lab == "299"] <- "2998"
pl_data_all$url_lab[is.na(pl_data_all$url_lab)] <- "No Lab ID"
...
export(participant_DF, "output_data/participant_data/pl_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(pl_data_all)[ c(6, 8:37, 41:42) ]
...
pl_data_all[ pl_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/pl_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/pl/pl_words.csv")
...
stimuli_data %>% 
  select(target_type, pl_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, pl_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$pl_cue, stimuli_data$pl_target)))
...
nrow(item_data)
length(unique(c(stimuli_data$pl_cue, stimuli_data$pl_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$pl_cue, stimuli_data$pl_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/pl_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/pl_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$pl_cue, stimuli_data$pl_target)
  ...
  export(priming_focus, "output_data/priming_data/pl_prime_trials.csv", row.names = F)
  ...
  pl_num_trials <- pl_num_trials %>% 
    full_join(pl_num_trials2)
...
priming_trial_paired <- priming_Z %>% 
  ...
  export(., file = "../../03_Materials/matched_stimuli/pl_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(pl_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/pl_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/pl_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/pl_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/pl_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/pl_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/pl_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/pl_answered_prime_summary_no3.0.csv", row.names = F)
```

...
# get all the files
cs_files <- list.files("../../04_Procedure_real",
                       pattern = "*.sqlite",
                       full.names = TRUE,
                       recursive = TRUE)
cs_files <- cs_files[grepl("cs", cs_files)]

cs_data <- list()

# open all files
for (i in 1:length(cs_files)){
  cs_data[[i]] <- processData(cs_files[i])
  cs_data[[i]]$url_lab <- as.character(cs_data[[i]]$url_lab)
}

# convert to data frame and make unique
cs_data_all <- bind_rows(cs_data) %>% unique()
...
# fix the duplication from below 
cs_data_all$uuid[cs_data_all$uuid == "a5fbf8f2-3684-4615-8b70-dd172f5c2a01" ] <- "6e9eacef-fc7e-481f-b13b-7f17ac41cfde"

cs_data_all$uuid[cs_data_all$uuid == "a7c44020-8aaf-41b8-9544-9ecb2a4d836d"] <- "cd3f8812-63ef-4b86-a19d-91a810091c5f"

# fill in uuid for each row
cs_data_all <- fill(cs_data_all, uuid, .direction = "down")
...
# checked duplicated UUIDs, they are different

cs_data_all$observation <- paste0(cs_data_all$observation, substr(cs_data_all$uuid, 20, 28))

# make sure every row has an id number 
sum(is.na(cs_data_all$observation))

# remove ZPID, mturk, uuid 
cs_data_all <- cs_data_all %>% 
  select(-url_special_code, -uuid, -x, -url, 
         -url_fbclid, -meta_location)

ncol(cs_data_all)
```

In this section, we remove a part that showed something to a participant twice for unknown reasons.

```{r eval = F}
cs_data_all <- cs_data_all %>% 
filter(!(observation == "c3b6e91ed-385a" & fix_sender == "13_00_000_1" & is.na(time_switch)))
...
```{r write-full, eval = F}
write.csv(cs_data_all, 
       gzfile("output_data/full_data/cs_full_data.csv.gz"),
       row.names = F)
```
...
```{r read-full}
cs_data_all <- read.csv("output_data/full_data/cs_full_data.csv.gz")
```
...
export(participant_DF, "output_data/participant_data/cs_participant_data.csv", row.names = F)
```
...
# censor out the data
  ...
  all_censor <- colnames(cs_data_all)[ c(6, 8:37, 41:42) ]
...
cs_data_all[ cs_data_all$observation %in% too_young , 
             all_censor ] <- NA
...
write.csv(real_trials, 
          gzfile("output_data/trial_data/cs_trial_data.csv.gz"), 
          row.names = F)
```
...
stimuli_data <- import("../../04_Procedure/cs/cs_words.csv")
...
stimuli_data %>% 
  select(target_type, cs_target) %>% 
  group_by(target_type) %>% 
  unique() %>% 
  summarize(n = n())

stimuli_data %>% 
  select(cue_type, cs_cue) %>% 
  group_by(cue_type) %>% 
  unique() %>% 
  summarize(n = n())
...
temp <- item_data %>% 
  filter(!(word %in% c(stimuli_data$cs_cue, stimuli_data$cs_target)))
...
length(unique(c(stimuli_data$cs_cue, stimuli_data$cs_target)))
# what word forms aren't there
setdiff(unique(c(stimuli_data$cs_cue, stimuli_data$cs_target)), item_data$word)
```
...
write.csv(item_data_combo, "output_data/item_data/cs_item_data.csv", row.names = F)
```
...
##write out item level data 
write.csv(item_data_combo, "output_data/item_data/cs_answered_item_data.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  stimuli_data$word_combo <- paste0(stimuli_data$cs_cue, stimuli_data$cs_target)
  ...
  export(priming_focus, "output_data/priming_data/cs_prime_trials.csv", row.names = F)
  ...
  cs_num_trials <- cs_num_trials %>% 
    full_join(cs_num_trials2, 
              by = "word_combo")
...
# Calculate Statistics ----------------------------------------------------
  ...
  export(., file = "../../03_Materials/matched_stimuli/cs_trial_unique.csv")
  ...
  dplyr::summarize(avgRT = mean(target_duration, na.rm = T),
    ...
    left_join(cs_num_trials, by = c("word_combo" = "word_combo")) %>% 
...
write.csv(priming_Z_summary, "output_data/priming_data/cs_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/cs_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/cs_prime_summary_no3.0.csv", row.names = F)
```
...
# figure out trial type ----
  ...
  export(priming_focus, "output_data/priming_data/cs_answered_prime_trials.csv", row.names = F)
...
##write out the priming data 
write.csv(priming_Z_summary, "output_data/priming_data/cs_answered_prime_summary.csv", row.names = F)
write.csv(priming_Z_summary_no2.5, "output_data/priming_data/cs_answered_prime_summary_no2.5.csv", row.names = F)
write.csv(priming_Z_summary_no3.0, "output_data/priming_data/cs_answered_prime_summary_no3.0.csv", row.names = F)
```

...
calc_frequency <- function(root, model, file) {
  ...
  # Annotate the Text
  annotate <- udpipe(data.text, model, parser = "none") 
  ...
  file <- gsub(root, "data/", "frequency/", file, fixed = T)
  file <- paste0(file, "_frequency.csv")
  
  export(count_df, file = file)
